...
import argparse
from scripts.ci.verify_codeowners import main as _verify_codeowners
from scripts.ci.verify_license import main as _verify_license
...
def _get_sdk_file_list(root_dir):
    dir_paths = [os.path.join(root_dir, path) for path in os.listdir(root_dir)]
    sdk_files = []

    for path in dir_paths:
        if os.path.isdir(path):
            sdk_files.extend(_get_sdk_file_list(path))
        elif _is_sdk_file(path):
            sdk_files.append(os.path.basename(path))
...
def main():
    ...
    sdk_files = _get_sdk_file_list(root_dir)
    sdk_files.append("vendored_sdks")
    ...
    _run_pylint(module_paths, ",".join(sdk_files), rcfile, cpu_count)
    print("Pylint OK.\n")
