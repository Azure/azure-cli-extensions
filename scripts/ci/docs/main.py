...
ADO_BUILD_ID = os.environ.get('ADO_BUILD_ID')
ADO_REPO_SLUG = os.environ.get('ADO_REPO_SLUG')
ADO_COMMIT = os.environ.get('ADO_COMMIT')
...
from util import get_index_data, INDEX_PATH
...
def commit_update(doc_repo):
    ...
    branch_name = 'az-ext-list-ci-{}'.format(ADO_BUILD_ID)
    local_branch = doc_repo.create_head(branch_name)
    local_branch.checkout()
    commit_url = 'https://github.com/{}/commit/{}'.format(ADO_REPO_SLUG, ADO_COMMIT)
    commit_msg = 'Update CLI extensions available doc.\n Triggered by {} - ' \
                 'ADO_BUILD_ID={}\n{}'.format(ADO_REPO_SLUG, ADO_BUILD_ID, commit_url)
