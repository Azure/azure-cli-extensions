Describe 'Extension Troubleshoot Testing' {
    BeforeAll {
        $extensionType = "microsoft.monitor.pipelinecontroller"
        $extensionName = "amp-controller"
    ...
    It 'Creates the extension and checks that it onboards correctly' {
        $output = az $Env:K8sExtensionName create -c $($ENVCONFIG.arcClusterName) -g $($ENVCONFIG.resourceGroup) --cluster-type $clusterType -n $extensionName --extension-type $extensionType --scope cluster --release-namespace $namespace --no-wait
        $? | Should -BeTrue
    ...
    It "Executes the troubleshoot command on the extension" {
        $output = az $Env:K8sExtensionName troubleshoot --name $extensionName --namespace-list $namespace
        $? | Should -BeTrue
        $output | Should -Not -BeNullOrEmpty
    }
