Describe 'Auto Upgrade Scenario' {
    BeforeAll {
        . $PSScriptRoot/../helper/Constants.ps1
    }

    It 'Check if basic onboarding works with auto-upgrade disabled' {
        az connectedk8s connect -n $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup -l $ARC_LOCATION --disable-auto-upgrade --no-wait
        $? | Should -BeTrue
    ...
    It 'Enable auto-upgrade using update cmd' {
        az connectedk8s update -n $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --auto-upgrade true
        $? | Should -BeTrue
        Start-Sleep -Seconds 10
