...
def load_arguments(self, _):
    ...
    with self.argument_context('connectedk8s connect') as c:
        ...
        c.argument('disable_auto_upgrade', options_list=['--disable-auto-upgrade'], action='store_true', help='Flag to disable auto upgrade of arc agents. This flag must be provided when using custom registry repository')
        c.argument('cl_oid', options_list=['--custom-locations-oid'], help="OID of 'custom-locations' app")
        c.argument('container_registry_repository', options_list=['--registry-repository'], arg_group='CustomContainerRegistry', help='Custom container registry repository.')
        c.argument('container_registry_username', options_list=['--registry-username'], arg_group='CustomContainerRegistry', help='Custom container registry repository username.')
        c.argument('container_registry_password', options_list=['--registry-password'], arg_group='CustomContainerRegistry', help='Custom container registry repository password.')
        c.argument('arc_agent_version', options_list=['--agent-version'], arg_group='CustomContainerRegistry', help='Version of agent to update the helm charts to. Required for private container registry')
    ...
    with self.argument_context('connectedk8s update') as c:
        ...
        c.argument('auto_upgrade', options_list=['--auto-upgrade'], help='Flag to enable/disable auto upgrade of arc agents. By default, auto upgrade of agents is enabled. This flag must be false when using custom registry repository', arg_type=get_enum_type(["true", "false"]))
        c.argument('container_registry_repository', options_list=['--registry-repository'], arg_group='CustomContainerRegistry', help='Custom container registry repository.')
        c.argument('container_registry_username', options_list=['--registry-username'], arg_group='CustomContainerRegistry', help='Custom container registry repository username.')
        c.argument('container_registry_password', options_list=['--registry-password'], arg_group='CustomContainerRegistry', help='Custom container registry repository password.')
        c.argument('default_registry', options_list=['--default-registry'], arg_group='CustomContainerRegistry', help='Flag to move from custom container registry repository to default registry i.e., Microsoft container registry (MCR).')
