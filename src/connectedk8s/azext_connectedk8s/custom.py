...
def create_connectedk8s(cmd, client, resource_group_name, cluster_name, correlation_id=None, https_proxy="", http_proxy="", no_proxy="", proxy_cert="", location=None,
    ...
    if enable_private_link is not None or distribution_version is not None or azure_hybrid_benefit is not None or enable_workload_identity or enable_oidc_issuer or enable_gateway:
        client = cf_connected_cluster_prev_2024_07_01(cmd.cli_ctx, None)
    ...
    if not required_node_exists:
        ...
        logger.warning("Please ensure that this Kubernetes cluster has any nodes with OS 'linux', for scheduling the Arc-Agents onto and connecting to Azure. "
                       "Learn more at %s", "https://aka.ms/ArcK8sSupportedOSArchitecture")
...
def validate_env_file_dogfood(values_file):
    if not values_file:
        ...
        raise ValidationError("Helm environment file is required when using Dogfood environment for onboarding the cluster.",
    ...
    with open(values_file, 'r') as f:
        ...
        try:
            ...
            raise FileOperationError("The required helm environment variables for dogfood onboarding are either not present in the file or incorrectly set.",
                                     recommendation="Please check the values 'global.azureEnvironment' and 'systemDefaultValues.azureArcAgents.config_dp_endpoint_override' in the file.")
...
def check_kube_connection():
    ...
    try:
        ...
        utils.kubernetes_exception_handler(e, consts.Kubernetes_Connectivity_FaultType,
...
def install_helm_client():
    ...
    if not os.path.isfile(install_location):
        ...
        try:
            ...
            raise ClientRequestError("Failed to extract helm executable." + str(
                e), recommendation="Please ensure that you delete the directory '{}' before trying again.".format(download_dir))
...
def connected_cluster_exists(client, resource_group_name, cluster_name):
    try:
        ...
        utils.arm_exception_handler(e, consts.Get_ConnectedCluster_Fault_Type,
...
def delete_connectedk8s(cmd, client, resource_group_name, cluster_name,
    ...
    if (preview_cluster_resource is not None) and (preview_cluster_resource.kind is not None) and (preview_cluster_resource.kind.lower() == consts.Provisioned_Cluster_Kind):
        raise InvalidArgumentValueError(
            "Deleting a Provisioned Cluster is not supported from the Connected Cluster CLI. Please use the 'az aksarc delete' CLI command.\nhttps://learn.microsoft.com/en-us/cli/azure/aksarc?view=azure-cli-latest#az-aksarc-delete")
    ...
    if (configmap.data["AZURE_RESOURCE_GROUP"].lower() == resource_group_name.lower() and
        ...
        if check_proxy_kubeconfig(kube_config, kube_context, arm_hash):
            ...
            raise ClientRequestError("az connectedk8s delete is not supported when using the Cluster Connect kubeconfig.",
                                     recommendation="Run the az connectedk8s delete command with your kubeconfig file pointing to the actual Kubernetes cluster to ensure that the agents are cleaned up successfully as part of the delete command.")
...
def upgrade_agents(cmd, client, resource_group_name, cluster_name, kube_config=None, kube_context=None, skip_ssl_verification=False, arc_agent_version=None, upgrade_timeout="600"):
    ...
    if (connected_cluster is not None) and (connected_cluster.kind is not None) and (connected_cluster.kind.lower() == consts.Provisioned_Cluster_Kind):
        raise InvalidArgumentValueError(
            "Upgrading a Provisioned Cluster is not supported from the Connected Cluster CLI. Please use the 'az aksarc upgrade' CLI command. https://learn.microsoft.com/en-us/cli/azure/aksarc?view=azure-cli-latest#az-aksarc-upgrade")
...
def enable_features(cmd, client, resource_group_name, cluster_name, features, kube_config=None, kube_context=None,
    ...
    if (connected_cluster is not None) and (connected_cluster.kind is not None) and (connected_cluster.kind.lower() == consts.Provisioned_Cluster_Kind):
        raise InvalidArgumentValueError(
            "Enable feature of a Provisioned Cluster is not supported from the Connected Cluster CLI. For information on how to enable a feature on a Provisioned Cluster using a cluster extension, please refer to: https://learn.microsoft.com/en-us/azure/aks/deploy-extensions-az-cli")
    ...
    if connected_cluster.private_link_state.lower() == "enabled" and (enable_cluster_connect or enable_cl):
        ...
        raise InvalidArgumentValueError(
            "The features 'cluster-connect' and 'custom-locations' cannot be enabled for a private link enabled connected cluster.")
    ...
    if enable_azure_rbac:
        ...
        logger.warning(
            "Please use the kubelogin version v0.0.32 or higher which has support for generating PoP token(s). This is needed by guard running in 'arc' authN mode.")
...
def disable_features(cmd, client, resource_group_name, cluster_name, features, kube_config=None, kube_context=None,
    ...
    if (connected_cluster is not None) and (connected_cluster.kind is not None) and (connected_cluster.kind.lower() == consts.Provisioned_Cluster_Kind):
        raise InvalidArgumentValueError(
            "Disable feature of a Provisioned Cluster is not supported from the Connected Cluster CLI. For information on how to disable a feature on a Provisioned Cluster using a cluster extension, please refer to: https://learn.microsoft.com/en-us/azure/aks/deploy-extensions-az-cli")
    ...
    if disable_cl:
        logger.warning(
            "Disabling 'custom-locations' feature might impact some dependent resources. Learn more about this at https://aka.ms/ArcK8sDependentResources.")
...
def client_side_proxy_wrapper(cmd,
    ...
    if (clientproxyutils.check_process(proc_name)) and clientproxyutils.check_if_port_is_open(api_server_port):
        raise ClientRequestError('The proxy port is already in use, potentially by another proxy instance.',
                                 recommendation='Please stop the existing proxy instance or pass a different port through --port option.')
    ...
    client_side_proxy_main(cmd, tenant_id, client, resource_group_name, cluster_name, 0, args, client_proxy_port, api_server_port,
                           operating_system, creds, user_type, debug_mode, token=token, path=path, context_name=context_name, clientproxy_process=None)
...
def client_side_proxy_main(cmd,
    ...
    expiry, clientproxy_process = client_side_proxy(cmd, tenant_id, client, resource_group_name, cluster_name, 0, args, client_proxy_port,
                                                    api_server_port, operating_system, creds, user_type, debug_mode, token=token, path=path, context_name=context_name, clientproxy_process=None)
    ...
    while (True):
        ...
        if (clientproxyutils.check_if_csp_is_running(clientproxy_process)):
            if time.time() >= next_refresh_time:
                expiry, clientproxy_process = client_side_proxy(cmd, tenant_id, client, resource_group_name, cluster_name, 1, args, client_proxy_port, api_server_port,
                                                                operating_system, creds, user_type, debug_mode, token=token, path=path, context_name=context_name, clientproxy_process=clientproxy_process)
                next_refresh_time = expiry - consts.CSP_REFRESH_TIME
        else:
    ...
    try:
        ...
        if len(app_object) != 0:
            ...
            if cl_oid is not None and cl_oid != app_object[0]['id']:
                logger.debug(
                    "The 'Custom-locations' OID passed is different from the actual OID(%s) of the Custom Locations RP app. Proceeding with the correct one...", app_object[0]['id'])
...
def troubleshoot(cmd, client, resource_group_name, cluster_name, kube_config=None, kube_context=None, skip_ssl_verification=False, no_wait=False, tags=None):
    ...
    try:
        ...
        diagnostic_checks[consts.Fetch_Kubectl_Cluster_Info], storage_space_available = troubleshootutils.fetch_kubectl_cluster_info(
        ...
        diagnostic_checks[consts.Fetch_Connected_Cluster_Resource], storage_space_available = troubleshootutils.fetch_connected_cluster_resource(
        ...
        if arc_agents_pod_list.items:
            ...
            diagnostic_checks[consts.Retrieve_Arc_Agents_Logs], storage_space_available = troubleshootutils.retrieve_arc_agents_logs(
                corev1_api_instance, filepath_with_timestamp, storage_space_available)

            # For storing all arc agents events logs
            diagnostic_checks[consts.Retrieve_Arc_Agents_Event_Logs], storage_space_available = troubleshootutils.retrieve_arc_agents_event_logs(
                filepath_with_timestamp, storage_space_available, kubectl_client_location, kube_config, kube_context)

            # For storing all the deployments logs using the AppsV1Api
            appv1_api_instance = kube_client.AppsV1Api()
            diagnostic_checks[consts.Retrieve_Deployments_Logs], storage_space_available = troubleshootutils.retrieve_deployments_logs(
                appv1_api_instance, filepath_with_timestamp, storage_space_available)

            # Check for the azure arc agent states
            diagnostic_checks[consts.Arc_Agent_State_Check], storage_space_available, all_agents_stuck, probable_sufficient_resource_for_agents = troubleshootutils.check_agent_state(
                corev1_api_instance, filepath_with_timestamp, storage_space_available)
            ...
            if diagnostic_checks[consts.MSI_Cert_Check] == consts.Diagnostic_Check_Passed:
                diagnostic_checks[consts.KAP_Security_Policy_Check] = troubleshootutils.check_probable_cluster_security_policy(
            ...
            logger.warning(
                "Error: Azure Arc agents are not present on the cluster. Please verify whether Arc onboarding of the Kubernetes cluster has been attempted.\n")
        ...
        diagnostic_checks[consts.Diagnoser_Check], storage_space_available = troubleshootutils.check_diagnoser_container(
            corev1_api_instance, batchv1_api_instance, filepath_with_timestamp, storage_space_available, absolute_path, probable_sufficient_resource_for_agents, helm_client_location, kubectl_client_location, release_namespace, diagnostic_checks[consts.KAP_Security_Policy_Check], kube_config, kube_context)

        # saving secrets in azure-arc namespace
        storage_space_available = troubleshootutils.get_secrets_azure_arc(
            corev1_api_instance, kubectl_client_location, kube_config, kube_context, filepath_with_timestamp, storage_space_available)

        # saving helm values of azure-arc release
        storage_space_available = troubleshootutils.get_helm_values_azure_arc(
            corev1_api_instance, helm_client_location, release_namespace, kube_config, kube_context, filepath_with_timestamp, storage_space_available)
        ...
        if cluster_connectivity_status != "Connected":
            logger.warning(
                "Cluster connectivity status is not connected. The current state of the cluster is : %s", cluster_connectivity_status)
        ...
        if storage_space_available:
            ...
            logger.warning(
                "The diagnoser was unable to save logs to your machine. Please check whether sufficient storage is available and run the troubleshoot command again.")
