...
class Connectedk8sScenarioTest(ScenarioTest):
    ...
    def test_connectedk8s(self, resource_group):
        ...
        self.cmd('az connectedk8s connect -g {rg} -n {name} -l eastus2euap --tags foo=doo', checks=[
            self.check('tags.foo', 'doo'),
            self.check('name', '{name}')
        ])
        count = len(self.cmd('az connectedk8s list').get_output_in_json())
        self.cmd('az connectedk8s show -g {rg} -n {name}', checks=[
            self.check('name', '{name}'),
            self.check('resourceGroup', '{rg}')
        ])
        self.cmd('az connectedk8s delete -g {rg} -n {name} -y')
