    ...
    try:
        ...
        if(cluster_diagnostic_checks_container_log is not None and cluster_diagnostic_checks_container_log != ""):
            ...
            for outputs in cluster_diagnostic_checks_container_log_list:
                if consts.Outbound_Connectivity_Check_String in outputs:
                    counter_container_logs = 1
                    if outbound_check_log == "":
                        outbound_check_log += outputs
                    else:
                        outbound_check_log += "  " + outputs
            ...
            outbound_connectivity_check, storage_space_available = azext_utils.check_cluster_outbound_connectivity(outbound_check_log, filepath_with_timestamp, storage_space_available, diagnoser_output)
        else:
