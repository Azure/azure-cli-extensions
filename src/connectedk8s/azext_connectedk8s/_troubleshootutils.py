...
def check_agent_version(connected_cluster,azure_arc_agent_version):
    ...
    if(connected_cluster.agent_version==None):
        return True
...
def check_msi_certificate(connected_cluster,corev1_api_instance):
    ...
    if not msi_cert_present:
        print("Error: Unable to pull MSI certificate.\n")
        return False
...
def check_cluster_security_policy(corev1_api_instance):
    ...
    if(cluster_connect_feature==True and kap_pod_present==False):
...
def check_msi_expiry(connected_cluster,corev1_api_instance):
