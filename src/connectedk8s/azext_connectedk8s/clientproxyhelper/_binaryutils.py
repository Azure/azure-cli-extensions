...
def _get_client_architeture():
    ...
    if "arm64" in machine.lower() or "aarch64" in machine.lower():
        ...
        raise azclierror.ClientRequestError("Couldn't identify the platform architecture.")
    else:
        raise azclierror.ClientRequestError(f"Unsuported architecture: {machine} is not currently supported")
...
def _get_client_operating_system():
    ...
    if operating_system.lower() not in ('linux', 'darwin', 'windows'):
        ...
        raise azclierror.ClientRequestError(f"The {operating_system} platform is not currently supported.")
