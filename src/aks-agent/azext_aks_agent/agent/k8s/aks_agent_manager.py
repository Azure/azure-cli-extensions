...
from azext_aks_agent._consts import AGENT_LABEL_SELECTOR, AGENT_NAMESPACE
    ...
    def get_agent_pods(self) -> Tuple[bool, Union[List[str], str]]:
        ...
        try:
            ...
            logger.debug("Searching for pods with label selector '%s' in namespace '%s'",
                         AGENT_LABEL_SELECTOR, self.namespace)
            ...
            if not pod_list.items:
                error_msg = (
                    f"No pods found with label selector '{AGENT_LABEL_SELECTOR}' in namespace '{self.namespace}'. "
                    f"This could mean:\n"
            ...
            error_msg = (
                f"No running pods found with label selector '{AGENT_LABEL_SELECTOR}' in namespace '{self.namespace}'. "
