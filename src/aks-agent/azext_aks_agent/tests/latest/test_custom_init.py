...
class TestAksAgentInit(unittest.TestCase):
    ...
    @patch('azext_aks_agent.custom.get_subscription_id')
    @patch('azext_aks_agent.custom.get_console')
    def test_init_helm_status_failed_uninstalls_first(
            self, mock_get_console, mock_get_subscription_id, mock_aks_manager_class,
            mock_get_aks_creds, mock_setup_llm):
        """Test initialization uninstalls when helm status is failed."""
        # Setup mocks
        mock_console = MagicMock()
        mock_get_console.return_value = mock_console
        mock_get_subscription_id.return_value = self.subscription_id
        ...
        mock_agent_manager.check_llm_config_exists.return_value = False
        mock_agent_manager.get_agent_status.side_effect = [
            {"helm_status": "failed", "ready": False},
            {"helm_status": "deployed", "ready": True}
        ]
        ...
        # Assert
        mock_agent_manager.uninstall_agent.assert_called_once_with(delete_secret=False)
        mock_agent_manager.deploy_agent.assert_called_once()
    ...
    @patch('azext_aks_agent.custom.get_subscription_id')
    @patch('azext_aks_agent.custom.get_console')
    def test_init_uninstall_failure_logs_error(
            self, mock_get_console, mock_get_subscription_id, mock_aks_manager_class,
            mock_get_aks_creds, mock_setup_llm):
        """Test initialization logs error when uninstall of failed deployment fails."""
        # Setup mocks
        mock_console = MagicMock()
        mock_get_console.return_value = mock_console
        mock_get_subscription_id.return_value = self.subscription_id
        ...
        mock_agent_manager = MagicMock()
        mock_agent_manager.check_llm_config_exists.return_value = False
        mock_agent_manager.get_agent_status.return_value = {"helm_status": "failed"}
        mock_agent_manager.uninstall_agent.side_effect = Exception("Uninstall failed")
        mock_aks_manager_class.return_value = mock_agent_manager
        ...
        # Verify error message was printed to console
        error_calls = [call for call in mock_console.print.call_args_list
                       if 'Failed to uninstall previous deployment' in str(call)]
        self.assertTrue(len(error_calls) > 0, "Expected error message about uninstall failure to be printed")
