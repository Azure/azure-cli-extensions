...
def get_log_workspace_credentials_from_env():
    """
    ...
    if get_workspace_from_env:
        ...
        succeed = False
        while not succeed:
            print("Please provide Azure Log Analytics workspace information")
...
def update_azure_upload_status(
    data_controller, export_type, last_upload_time, ex
):
    """
    ...
    msg = "Success"
    if ex is not None:
        previousSuccessTimestamp = data_controller["k8sRaw"]["status"]["azure"][
            "uploadStatus"
        ]["{export_type}".format(export_type=export_type)]["lastUploadTime"]
        msg = ex
        upload_status = {
            "lastUploadTime": previousSuccessTimestamp,
            "message": str(msg)
            + " "
            + last_upload_time.strftime("%Y-%m-%dT%H:%M:%SZ"),
        }
    else:
        upload_status = {
            "lastUploadTime": last_upload_time.strftime("%Y-%m-%dT%H:%M:%SZ"),
            "message": str(msg),
        }
