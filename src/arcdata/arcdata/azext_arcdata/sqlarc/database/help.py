...
helps["sql db-arc backups-policy set"] = HELP_BACKUPS_POLICY_SET.format(
    example=(
        "az sql db-arc backups-policy set --name myDatabase --server myInstance --resource-group myResourceGroup",
        " --retention-days 7 --full-backup-days 7 --diff-backup-hours 12 --tlog-backup-mins 5"
    ),
    example2=(
        "az sql db-arc backups-policy set --name myDatabase --server myInstance --resource-group myResourceGroup",
        " --default-policy"
...
helps["sql db-arc restore"] = help_format_example.format(
    ...
    example=(
        'az sql db-arc restore --server myInstance --resource-group myResourceGroup --name mySourceDb ',
        '--dest-name myNewDb --time "2021-10-20T05:34:22Z"'
    ),
)
