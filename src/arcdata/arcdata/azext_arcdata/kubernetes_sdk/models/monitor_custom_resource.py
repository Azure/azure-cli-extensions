...
@validatedclass
class MonitorCustomResource(CustomResource):
    """
    ...
    def name(self, n: str):
        ...
        if not n:
            raise ValueError("Monitor custom resource name cannot be empty")

        if len(n) > self.monitor_name_max_length:
            raise ValueError(
                "Export custom resource name '{}' exceeds {} character length limit".format(
                    n, self.monitor_name_max_length
                )
            )

        # Default to monitorstack for now
        self._name = "monitorstack"
    ...
    def namespace(self, ns: str):
        ...
        if len(ns) > self.monitor_namespace_max_length:
            raise ValueError(
                "Kubernetes namespace '{}' exceeds {} character name length limit".format(
                    ns, self.monitor_namespace_max_length
                )
            )
