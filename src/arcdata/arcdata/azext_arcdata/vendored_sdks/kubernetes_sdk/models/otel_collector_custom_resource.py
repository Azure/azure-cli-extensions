...
@validatedclass
class TelemetryCollectorCustomResource(CustomResource):
    """
    ...
    def name(self, n: str):
        ...
        if len(n) > self.otel_collector_name_max_length:
            raise ValueError(
                "Export custom resource name '{}' exceeds {} character length limit".format(
                    n, self.otel_collector_name_max_length
                )
            )

        # Default to telemetrycollector for now
        self._name = "telemetrycollector"
    ...
    def namespace(self, ns: str):
        ...
        if len(ns) > self.otel_collector_namespace_max_length:
            raise ValueError(
                "Kubernetes namespace '{}' exceeds {} character name length limit".format(
                    ns, self.otel_collector_namespace_max_length
                )
            )

        if not name_meets_dns_requirements(ns):
            raise ValueError(
                "Kubernetes namespace '{}' does not follow DNS requirements: {}".format(
                    ns, DNS_NAME_REQUIREMENTS
                )
            )
