...
def devcenter_project_show_dp(cmd, dev_center,
                           project_name):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
    return cf_fidalgo.project.get(
                      project_name=project_name)


def devcenter_pool_list_dp(cmd, dev_center,
                        project_name,
                        top=None,
                        filter_=None):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
    return cf_fidalgo.pool.list(
                       top=top,
                       filter=filter_,
                       project_name=project_name)


def devcenter_pool_show_dp(cmd, dev_center,
                        project_name,
                        pool_name):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
    return cf_fidalgo.pool.get(
                      project_name=project_name,
                      pool_name=pool_name)


def devcenter_schedule_list_dp(cmd, dev_center,
                            project_name,
                            pool_name,
                            top=None,
                            filter_=None):
...
def devcenter_schedule_show_dp(cmd, dev_center,
                            project_name,
                            pool_name,
                            schedule_name):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
...
def devcenter_dev_box_list(cmd, dev_center,
                           filter_=None,
                           top=None,
                           project_name=None,
                           user_id=None):
    ...
    if project_name is not None and user_id is not None:
        ...
        return cf_fidalgo.dev_box.list_by_user(
                                   filter=filter_,
                                   top=top,
                                   user_id=user_id)
    return cf_fidalgo.dev_box.list(
                       filter=filter_,
                       top=top)


def devcenter_dev_box_show(cmd, dev_center,
                           project_name,
                           user_id,
                           dev_box_name):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
...
def devcenter_dev_box_delete(cmd, dev_center,
                             project_name,
                             user_id,
                             dev_box_name,
                             no_wait=False):
...
def devcenter_dev_box_get_remote_connection(cmd, dev_center,
                                            project_name,
                                            user_id,
                                            dev_box_name):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
...
def devcenter_dev_box_start(cmd, dev_center,
                            project_name,
                            user_id,
                            dev_box_name,
                            no_wait=False):
...
def devcenter_dev_box_stop(cmd, dev_center,
                           project_name,
                           user_id,
                           dev_box_name,
                           no_wait=False):
...
def devcenter_environment_list(cmd, dev_center,
                               project_name,
                               top=None):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
    return cf_fidalgo.environments.list_by_project(
                                  top=top,
                                  project_name=project_name)


def devcenter_environment_show(cmd, dev_center,
                               project_name,
                               user_id,
                               environment_name):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
...
def devcenter_environment_update(cmd, dev_center,
                                 project_name,
                                 user_id,
                                 environment_name,
                                 description=None,
...
def devcenter_environment_delete(cmd, dev_center,
                                 project_name,
                                 user_id,
                                 environment_name,
                                 no_wait=False):
...
def devcenter_environment_custom_action(cmd, dev_center,
                                        project_name,
                                        user_id,
                                        environment_name,
                                        action_id,
...
def devcenter_environment_delete_action(cmd, dev_center,
                                        project_name,
                                        user_id,
                                        environment_name,
                                        action_id,
...
def devcenter_environment_deploy_action(cmd, dev_center,
                                        project_name,
                                        user_id,
                                        environment_name,
                                        action_id,
...
def devcenter_environment_list_by_project(cmd, dev_center,
                                          project_name,
                                          user_id,
                                          top=None):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
...
def devcenter_artifact_list(cmd, dev_center,
                            project_name,
                            user_id,
                            environment_name,
                            artifact_path=None):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
    if project_name is not None and user_id is not None and environment_name is not None and artifact_path is not None:
...
def devcenter_catalog_item_list(cmd, dev_center,
                                project_name,
                                top=None):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)                            
    return cf_fidalgo.catalog_item.list_by_project(
                                  project_name=project_name,
                                  top=top)


def devcenter_catalog_item_show(cmd, dev_center,
                                project_name,
                                catalog_item_id,
                                top=None):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)                              
...
def devcenter_catalog_item_version_list(cmd, dev_center,
                                        project_name,
                                        catalog_item_id,
                                        top=None):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)                                                                      
...
def devcenter_catalog_item_version_show(cmd, dev_center,
                                        project_name,
                                        catalog_item_id,
                                        version,
                                        top=None):
...
def devcenter_environment_type_list_dp(cmd, dev_center,
                                    project_name,
                                    top=None):
    cf_fidalgo =  cf_devcenter_dataplane(cmd.cli_ctx, dev_center)
    return cf_fidalgo.environment_type.list_by_project(project_name=project_name,
                                  top=top)
