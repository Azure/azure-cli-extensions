...
class DelayAllActions(AAZCommand):
    """Delays all actions.

    :example: Delay all actions
    ...
    _aaz_info = {
        ...
        "resources": [
            ["data-plane:microsoft.devcenter", "/projects/{}/users/{}/devboxes/{}/actions:delay", "2025-08-01-preview"],
        ]
    }
    ...
    def _execute_operations(self):
        self.pre_operations()
        self.DevBoxesDelayActions(ctx=self.ctx)()
    ...
    class DevBoxesDelayActions(AAZHttpOperation):
        ...
        def _build_schema_on_200(cls):
            ...
            _DelayAllActionsHelper._build_schema_azure_core_foundations_error_read(_element.error)
            ...
            action = cls._schema_on_200.value.Element.action
            action.action_type = AAZStrType(
                serialized_name="actionType",
                flags={"required": True},
            )
            action.name = AAZStrType(
                flags={"read_only": True},
            )
            action.next = AAZObjectType()
            action.source_id = AAZStrType(
                serialized_name="sourceId",
                flags={"required": True},
            )
            action.source_type = AAZStrType(
                serialized_name="sourceType",
                flags={"required": True},
            )
            action.source_uri = AAZStrType(
                serialized_name="sourceUri",
                flags={"required": True},
            )
            action.suspended_until = AAZStrType(
                serialized_name="suspendedUntil",
            )
            action.uri = AAZStrType(
                flags={"required": True},
            )

            next = cls._schema_on_200.value.Element.action.next
...
class _DelayAllActionsHelper:
    """Helper class for DelayAllActions"""
