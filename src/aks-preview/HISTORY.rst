...
* `az aks create`, `az aks update`: Add `--enable-node-hardening` to enable cluster-level node hardening and `az aks update --disable-node-hardening` to disable it. Applies hardened defaults for soft eviction thresholds, kube-reserved, and system-reserved on Linux node pools. Requires AFEC registration `Microsoft.ContainerService/CustomNodeConfigPreview`.
