...
if [[ ${TEST_MODE} == "record" || ${TEST_MODE} == "all" ]]; then
    ...
    run_flags+=" --json-report-file=ext_recording_report.json"
    run_flags+=" --xml-file=ext_recording_result.xml"
...
if [[ ${TEST_MODE} == "live" || ${TEST_MODE} == "all" ]]; then
    ...
    run_flags+=" --json-report-file=ext_re_recording_report.json"
    run_flags+=" --xml-file=ext_re_recording_result.xml"
    echo "run flags: ${run_flags}"
    azaks ${run_flags}
fi
