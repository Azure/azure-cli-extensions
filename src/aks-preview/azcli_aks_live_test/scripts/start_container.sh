...
[[ -z "${MAPPED_AZCLI_ALT_CLIENT_SECRET}" ]] && (echo "MAPPED_AZCLI_ALT_CLIENT_SECRET is empty")
...
# pass secrects as environment variables directly if specified (instead of storing in env.list)
# pass other environment variables with env.list
if [[ -n ${MAPPED_AZCLI_ALT_CLIENT_SECRET} ]]; then
