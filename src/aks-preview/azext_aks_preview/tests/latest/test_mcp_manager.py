...
class TestMCPManager(unittest.TestCase):
    ...
    @patch('azext_aks_preview.agent.mcp_manager.get_config_dir')
    ...
    def test_binary_manager_initialization(self):
        """Test that binary manager is properly initialized with correct path."""
        manager = MCPManager(config_dir=self.test_config_dir)
        
        # Check that binary manager install_dir is set correctly
        expected_install_dir = os.path.join(self.test_config_dir, 'bin')
        self.assertEqual(manager.binary_manager.install_dir, expected_install_dir)
        
        # Check that binary path is constructed correctly
        expected_binary_path = os.path.join(expected_install_dir, 'aks-mcp')
        if os.name == 'nt':  # Windows
            expected_binary_path += '.exe'
        
        actual_binary_path = manager.binary_manager.get_binary_path()
        self.assertEqual(actual_binary_path, expected_binary_path)
    
    def test_verbose_mode(self):
        """Test verbose mode setting."""
        manager_quiet = MCPManager(config_dir=self.test_config_dir, verbose=False)
        manager_verbose = MCPManager(config_dir=self.test_config_dir, verbose=True)
        
        self.assertFalse(manager_quiet.verbose)
        self.assertTrue(manager_verbose.verbose)
    
    def test_integration_with_binary_manager(self):
        """Test integration between MCP manager and binary manager."""
        manager = MCPManager(config_dir=self.test_config_dir)
        
        # Create a mock binary file to simulate real scenario
        binary_path = manager.get_binary_path()
        os.makedirs(os.path.dirname(binary_path), exist_ok=True)
        
        # Initially, binary should not be available
        self.assertFalse(manager.is_binary_available())
        self.assertIsNone(manager.get_binary_version())
        self.assertFalse(manager.validate_binary_version())
        ...
        # Now binary should be available
        self.assertTrue(manager.is_binary_available())
        
        # Mock the version command for testing
...
class TestMCPManagerServerLifecycle(IsolatedAsyncioTestCase):
    ...
    async def test_start_server_binary_not_available(self):
        """Test start_server fails when binary is not available."""
        manager = MCPManager(config_dir=self.test_config_dir)
        
        with patch.object(manager, 'is_binary_available', return_value=False):
            with self.assertRaises(Exception) as cm:
                await manager.start_server()
            
            self.assertIn("MCP binary is not available", str(cm.exception))
    
    @patch('azext_aks_preview.agent.mcp_manager.asyncio.create_subprocess_exec')
    @patch('azext_aks_preview.agent.mcp_manager.asyncio.sleep')
    ...
    async def test_start_server_already_running_and_healthy(self, mock_sleep, mock_create_subprocess):
        ...
        with patch.object(manager, 'is_binary_available', return_value=True):
            with patch.object(manager, 'is_server_running', return_value=True):
                with patch.object(manager, 'is_server_healthy', return_value=True):
                    with patch('azext_aks_preview.agent.user_feedback.ProgressReporter.show_status_message') as mock_progress:
        ...
        with patch.object(manager, 'is_binary_available', return_value=True):
            with patch.object(manager, 'is_server_running', return_value=True):
                with patch.object(manager, 'is_server_healthy', return_value=False):
                    with patch.object(manager, 'stop_server') as mock_stop:  # Changed from AsyncMock to regular Mock
                        with patch.object(manager, '_find_available_port', return_value=8003):
                            with patch.object(manager, 'get_binary_path', return_value='/fake/aks-mcp'):
                                with patch('azext_aks_preview.agent.mcp_manager.asyncio.create_subprocess_exec') as mock_create:
                                    with patch('azext_aks_preview.agent.mcp_manager.asyncio.sleep'):
                                        
                                        # Mock the health check to fail first time, succeed second time
                                        health_calls = [False, True]  # First call (after restart check), second call (after start)
    ...
    def test_stop_server_no_process(self):
        ...
        manager.stop_server()  # Changed from await to synchronous call
    ...
    def run_async_test(self, test_func):
        """Helper to run async test functions (not needed with IsolatedAsyncioTestCase)."""
        pass  # Not needed with IsolatedAsyncioTestCase
