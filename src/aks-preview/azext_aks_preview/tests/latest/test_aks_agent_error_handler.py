...
class TestAksAgentErrorHandler:
    ...
    def test_agent_error_base_class(self):
        ...
        from azext_aks_preview.agent.error_handler import AgentError
    ...
    def test_configuration_error_inheritance(self):
        ...
        from azext_aks_preview.agent.error_handler import ConfigurationError
    ...
    def test_error_message_formatting(self):
        ...
        from azext_aks_preview.agent.error_handler import AgentErrorHandler, MCPError
    ...
    def test_mcp_setup_error_handling(self):
        ...
        mcp_error = AgentErrorHandler.handle_mcp_setup_error(network_error, "server startup")
        
        assert isinstance(mcp_error, MCPError)
        assert "MCP setup failed during server startup" in str(mcp_error)
        assert mcp_error.error_code == "MCP_SETUP"
        assert any("internet connection" in suggestion.lower() for suggestion in mcp_error.suggestions)
        
        # Test permission-related error
        permission_error = PermissionError("Access denied")
        mcp_error_perm = AgentErrorHandler.handle_mcp_setup_error(permission_error, "binary download")
        
        assert any("permissions" in suggestion.lower() for suggestion in mcp_error_perm.suggestions)
        
        # Test port-related error
        port_error = OSError("Address already in use")
        mcp_error_port = AgentErrorHandler.handle_mcp_setup_error(port_error, "server binding")
        
        assert any("port" in suggestion.lower() for suggestion in mcp_error_port.suggestions)

    def test_binary_error_handling(self):
        """Test binary error handling with operation-specific guidance."""
        from azext_aks_preview.agent.error_handler import AgentErrorHandler, BinaryError
    ...
    def test_server_error_handling(self):
        ...
        from azext_aks_preview.agent.error_handler import AgentErrorHandler, ServerError
    ...
    def test_context_error_creation(self):
        ...
        from azext_aks_preview.agent.error_handler import AgentErrorHandler, AgentError
    ...
    def test_error_codes_are_unique_and_descriptive(self):
        ...
        # All codes should be different (except binary/server which inherit from MCP but have different defaults)
        assert len(error_codes) >= 3  # At least MCP, BINARY, SERVER, CONFIG should be different
        
        # Test that handler-generated errors have specific codes
        ...
        assert binary_download.error_code != binary_validation.error_code
