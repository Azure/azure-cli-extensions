...
from azext_aks_preview.agent.config_generator import ConfigurationGenerator
...
class TestConfigurationGenerator(unittest.TestCase):
    ...
    def test_generate_traditional_config_basic(self):
        ...
        toolsets = result["toolsets"]
        self.assertTrue(toolsets["aks/node-health"]["enabled"])
        self.assertTrue(toolsets["aks/core"]["enabled"])
        self.assertTrue(toolsets["kubernetes/core"]["enabled"])
        self.assertTrue(toolsets["kubernetes/logs"]["enabled"])
        self.assertTrue(toolsets["kubernetes/live-metrics"]["enabled"])
        self.assertTrue(toolsets["bash"]["enabled"])
        
        # Should preserve custom toolsets
        self.assertTrue(toolsets["some-custom-toolset"]["enabled"])

    def test_generate_traditional_config_with_mcp_servers(self):
        """Test traditional config generation removes existing MCP servers."""
    ...
    def test_validate_mcp_config_enabled_conflicting_toolsets(self):
        ...
        config = {
            ...
            "toolsets": {
                "aks/core": {"enabled": True}  # This should cause validation to fail
            }
        }
    ...
    def test_validate_traditional_config_disabled_toolsets(self):
        ...
        config = {
            "toolsets": {
                "aks/core": {"enabled": False}  # Should be enabled for traditional mode
            }
        }
