...
from azext_aks_preview.agent.status import AgentStatusManager
from azext_aks_preview.agent.status_models import AgentStatus, BinaryStatus, ServerStatus, ConfigStatus
...
class TestAgentStatusManager:
    ...
    def test_init_with_default_config_dir(self):
        ...
        with patch('azext_aks_preview.agent.status.get_config_dir') as mock_get_config_dir:
    ...
    @pytest.mark.asyncio
    @patch('azext_aks_preview.agent.status.psutil')
    ...
    def test_get_mcp_binary_status_available(self):
        ...
        with patch.object(self.status_manager.binary_manager, 'get_binary_path') as mock_path, \
             ...
             patch.object(self.status_manager.binary_manager, 'validate_version') as mock_validate, \
             patch('os.path.exists') as mock_exists, \
             patch('azext_aks_preview.agent.status_models.BinaryStatus.from_file_path') as mock_from_path:
    ...
    def test_get_mcp_binary_status_not_available(self):
        ...
        with patch.object(self.status_manager.binary_manager, 'get_binary_path') as mock_path, \
            ...
            assert result.path == '/mock/binary/path'
            assert result.error_message == "Binary not found"
    
    def test_get_mcp_binary_status_with_exception(self):
        """Test MCP binary status collection with exception."""
        with patch.object(self.status_manager.binary_manager, 'get_binary_path', side_effect=Exception("Test error")):
            
            result = self.status_manager._get_mcp_binary_status()
            
            assert not result.available
            assert "Binary status check failed" in result.error_message
    
    @pytest.mark.asyncio
    @patch('azext_aks_preview.agent.status.MCPManager')
    @patch('azext_aks_preview.agent.status.psutil')
    ...
    def test_get_configuration_status_mcp_mode(self):
        ...
        with patch('azext_aks_preview.agent.status.ConfigurationGenerator.validate_mcp_config') as mock_validate:
            mock_validate.return_value = True
    ...
    def test_get_configuration_status_traditional_mode(self):
        ...
        with patch('azext_aks_preview.agent.status.ConfigurationGenerator.validate_traditional_config') as mock_validate:
            mock_validate.return_value = True
