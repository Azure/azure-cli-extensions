...
class TestAksAgentMCPIntegration:
    ...
    def test_initialize_mcp_manager_success(self):
        ...
        from azext_aks_preview.agent.agent import _initialize_mcp_manager
        
        with patch('azext_aks_preview.agent.mcp_manager.MCPManager') as mock_mcp_class:
            mock_manager = Mock()
    ...
    def test_initialize_mcp_manager_import_error(self):
        ...
        from azext_aks_preview.agent.agent import _initialize_mcp_manager
        from azext_aks_preview.agent.error_handler import MCPError
        
        with patch('azext_aks_preview.agent.mcp_manager.MCPManager', side_effect=ImportError("Module not found")):
    ...
    async def test_setup_mcp_mode_basic_workflow(self):
        ...
        from azext_aks_preview.agent.agent import _setup_mcp_mode
        ...
        with patch('pathlib.Path.exists', return_value=False), \
            ...
            try:
                ...
                # Expected to fail at Config.load_from_file, which means our logic is working
        ...
        if mock_yaml_dump.call_count > 0:
            config_data = mock_yaml_dump.call_args[0][0]  # First argument to yaml.dump
            print(f"Config data passed to yaml.dump: {config_data}")
            ...
            assert toolsets["aks/core"]["enabled"] == False
            assert toolsets["kubernetes/core"]["enabled"] == False
            assert toolsets["kubernetes/live-metrics"]["enabled"] == False
            assert toolsets["bash"]["enabled"] == False

    def test_config_content_verification(self):
        """Test to examine detailed configuration content generation."""
        from azext_aks_preview.agent.config_generator import ConfigurationGenerator
        
        # Test with a realistic base configuration
        base_config = {
            "model": "gpt-4",
            "max_steps": 20,
            "toolsets": {
                "custom/monitoring": {"enabled": True},
                "kubernetes/core": {"enabled": True, "config": {"timeout": 30}}
            },
            "other_settings": {"timeout": 60}
        }
        
        # Generate MCP configuration
        mcp_config = ConfigurationGenerator.generate_mcp_config(
            base_config, "http://localhost:8003/sse"
        )
        
        print(f"Generated MCP config: {mcp_config}")
        
        # Detailed verification of the generated configuration
        assert mcp_config["model"] == "gpt-4"
        assert mcp_config["max_steps"] == 20
        assert mcp_config["other_settings"]["timeout"] == 60
        
        # Verify MCP server configuration
        assert "mcp_servers" in mcp_config
        mcp_server_config = mcp_config["mcp_servers"]["aks-mcp"]
        assert mcp_server_config["description"] == "AKS MCP server"
        assert mcp_server_config["url"] == "http://localhost:8003/sse"
        
        # Verify toolset configurations
        toolsets = mcp_config["toolsets"]
        
        # Custom toolsets should be preserved
        assert toolsets["custom/monitoring"]["enabled"] == True
        
        # Conflicting built-in toolsets should be disabled
        assert toolsets["aks/core"]["enabled"] == False
        assert toolsets["kubernetes/core"]["enabled"] == False  # Should be disabled even though originally enabled
        assert toolsets["kubernetes/live-metrics"]["enabled"] == False
        assert toolsets["bash"]["enabled"] == False
        
        # Verify all expected conflicting toolsets are present
        expected_disabled = ["aks/node-health", "aks/core", "kubernetes/core", "kubernetes/logs", "kubernetes/live-metrics", "bash"]
        for toolset in expected_disabled:
            assert toolset in toolsets
            assert toolsets[toolset]["enabled"] == False
        
        print("✓ All configuration content verified successfully")
    ...
    async def test_setup_mcp_mode_binary_not_available(self):
        ...
        from azext_aks_preview.agent.agent import _setup_mcp_mode
        from azext_aks_preview.agent.binary_manager import BinaryStatus
        from azext_aks_preview.agent.error_handler import BinaryError
    ...
    async def test_setup_mcp_mode_server_start_failure(self):
        ...
        from azext_aks_preview.agent.agent import _setup_mcp_mode
        from azext_aks_preview.agent.binary_manager import BinaryStatus
        from azext_aks_preview.agent.error_handler import ServerError
    ...
    def test_error_handler_functionality(self):
        ...
        from azext_aks_preview.agent.error_handler import (
    ...
    def test_setup_traditional_mode_config_loading(self):
        ...
        try:
            ...
            assert toolsets["aks/core"]["enabled"] == True
            assert toolsets["kubernetes/core"]["enabled"] == True
            assert toolsets["kubernetes/live-metrics"]["enabled"] == True
            assert toolsets["custom/toolset"]["enabled"] == True  # Existing should be preserved
    ...
    def test_aks_agent_calls_sync_implementation(self, mock_stdin):
        ...
        from azext_aks_preview.agent.agent import aks_agent
        ...
        try:
            ...
            # This test primarily verifies no asyncio conflicts occur
    ...
    def test_python_version_check(self, mock_stdin):
        ...
        with patch('azext_aks_preview.agent.agent.sys') as mock_sys:
