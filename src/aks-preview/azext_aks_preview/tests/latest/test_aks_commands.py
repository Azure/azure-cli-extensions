...
class AzureKubernetesServiceScenarioTest(ScenarioTest):
    ...
    # gangams: commenting this test for now since it has dependency on azure-cli 2.62.0 which is not available in the current release
    # @live_only()
    # @AllowLargeResponse()
    # @AKSCustomResourceGroupPreparer(random_name_length=17, name_prefix='clitest', location='westus2')
    ...
    # gangams: commenting this test since it has dependency on azure-cli 2.62.0 which is not available in the current release
    # @live_only()
    # @AllowLargeResponse()
    # @AKSCustomResourceGroupPreparer(random_name_length=17, name_prefix='clitest', location='westus2')
    ...
    def test_aks_create_with_azuremonitorappmonitoring(
        ...
        create_cmd = (
            "aks create --resource-group={resource_group} --name={name} --location={location} --ssh-key-value={ssh_key_value} --node-vm-size={node_vm_size} "
            "--node-count 3 --enable-azure-monitor-app-monitoring --output=json"
        )
        self.cmd(
            create_cmd,
            checks=[
