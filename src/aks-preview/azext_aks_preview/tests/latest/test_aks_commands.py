...
class AzureKubernetesServiceScenarioTest(ScenarioTest):
    ...
    def test_aks_create_normal_cluster_then_add_managed_system_pool(
        ...
        # Update cluster should succeed (this validates the cluster is in a good state)
        update_cmd = (
            "aks update --resource-group={resource_group} --name={name}"
        )
        self.cmd(
    ...
    def test_aks_create_with_managed_system_pool_multiple_fails(
        ...
        add_nodepool_cmd = (
            "aks nodepool add --resource-group={resource_group} --cluster-name={name} "
            "--name={second_nodepool_name} --mode ManagedSystem --node-count 1"
        )
        # This should fail because only one ManagedSystem pool is allowed per cluster
        with self.assertRaisesRegex(
            (CLIError, ClientRequestError, HttpResponseError),
            r"(only.*one.*ManagedSystem.*pool.*allowed|ManagedSystem.*pool.*already.*exists|cannot.*add.*multiple.*ManagedSystem|A cluster cannot have more than one ManagedSystem|ManagedSystem.*pool.*limit.*exceeded)"
        ):
            self.cmd(add_nodepool_cmd)

        # Update the ManagedSystem pool to normal system pool should fail
        update_nodepool_cmd = (
            "aks nodepool update --resource-group={resource_group} --cluster-name={name} "
            "--name=nodepool1 --mode System"
        )
        # This should fail because ManagedSystem pool cannot be changed to System mode
        with self.assertRaisesRegex(
            (CLIError, ClientRequestError, HttpResponseError),
            r"(cannot.*update.*ManagedSystem.*pool.*mode|ManagedSystem.*pool.*mode.*cannot.*be.*changed|invalid.*mode.*change|ManagedSystem.*mode.*cannot.*be.*modified|Cannot change.*ManagedSystem.*pool.*mode)"
        ):
            self.cmd(update_nodepool_cmd)
