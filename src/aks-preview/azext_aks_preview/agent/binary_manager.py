...
class AksMcpBinaryManager:
                ...
                for line in attestation_content.strip().split('\n'):  # pylint: disable=too-many-nested-blocks
                    if line.strip():
                        try:
                            ...
                            # Check if binary is listed in materials
                            for material_path, material_info in materials.items():
                                if binary_filename in material_path:
                                    if isinstance(material_info, dict):
                                        digest = material_info.get("digest", {})
                                        binary_hash = digest.get("sha256")
                                    break

                            # Also check subjects array (common in SLSA attestations)
                            ...
                            if binary_hash:
                                # Calculate SHA256 hash of the downloaded binary
                                sha256_hash = hashlib.sha256()
                                with open(file_path, 'rb') as f:
                                    for chunk in iter(lambda: f.read(8192), b""):  # pylint: disable=cell-var-from-loop
    ...
    async def download_binary(
        ...
        :param version: Specific version to download (latest if None)
        :type version: str
        ...
        try:
            ...
            # TODO: For specific version, we'd need to fetch all releases and find the matching one
            # For now, we always use latest release regardless of version parameter
