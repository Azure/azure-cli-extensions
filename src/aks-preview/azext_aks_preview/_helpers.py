...
def _merge_kubernetes_configurations(
...
def get_nodepool_snapshot(cli_ctx, subscription_id, resource_group_name, snapshot_name):
    snapshot_client = get_nodepool_snapshots_client(
    ...
    # track 2 sdk raise exception from azure.core.exceptions
        ...
        if "not found" in ex.message:
            ...
            raise ResourceNotFoundError(
                f"Managed cluster snapshot '{snapshot_name}' not found."
            )
        raise map_azure_error_to_cli_error(ex) from ex
    return snapshot
...
def check_is_private_link_cluster(mc: ManagedCluster) -> bool:
    ...
    :return: bool
    """
    return check_is_private_cluster(
        mc
    ) and not check_is_apiserver_vnet_integration_cluster(mc)
...
def setup_common_safeguards_profile(
    level, version, excludedNamespaces, mc: ManagedCluster, models
) -> ManagedCluster:
    if (
        level is not None or version is not None or excludedNamespaces is not None
    ) and mc.safeguards_profile is None:
        mc.safeguards_profile = models.SafeguardsProfile(level=level, version=version)
