...
if [[ $TEST_MODE == "live" || $TEST_MODE == "all" ]]; then
    ...
    if [[ $TEST_DIFF == true && $BUILD_REASON == "PullRequest" ]]; then
        azdev test aks-preview --live --no-exitfirst --repo=azure-cli/ --src=HEAD --tgt=origin/$SYSTEM_PULLREQUEST_TARGETBRANCH --cli-ci --xml-path ext_live_test.xml --discover -a "-n $PARALLELISM --json-report --json-report-file=ext_live_report.json --reruns 3 --capture=sys"
    else
