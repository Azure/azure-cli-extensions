...
class AzureVWanVHubScenario(ScenarioTest):
    ...
    def test_azure_vwan_vpn_gateway_connection_vpn_site_link_conn(self):
        ...
        self.cmd('network vpn-gateway connection ipsec-policy remove -g {rg} --gateway-name {vpngateway} --connection-name {connection}')
        ...
        self.cmd('network vpn-site link remove -g {rg} -n {vpn_site} --index 1')
        ...
        self.cmd('network vpn-gateway connection vpn-site-link-conn ipsec-policy add -g {rg} --gateway-name {vpngateway} -n {connection} '
                 '--vpn-site-link-conn-name {vpn_site_link_conn} --ipsec-encryption AES256 --ipsec-integrity SHA256 --sa-lifetime 86471 '
                 '--sa-data-size 429496 --ike-encryption AES256 --ike-integrity SHA384 --dh-group DHGroup14 --pfs-group PFS14')
        self.cmd('network vpn-gateway connection vpn-site-link-conn ipsec-policy list -g {rg} --gateway-name {vpngateway} -n {connection} --vpn-site-link-conn-name {vpn_site_link_conn}')
        self.cmd('network vpn-gateway connection vpn-site-link-conn ipsec-policy remove -g {rg} --gateway-name {vpngateway} -n {connection} --vpn-site-link-conn-name {vpn_site_link_conn} --index 1')
