...
class AzureVWanRouteTableScenario(ScenarioTest):
    ...
    def test_azure_vwan_route_table(self, resource_group):
        ...
        self.kwargs.update({
            'vwan': 'testvwan',
            'vhub': 'testvhub',
            'routetable': 'testrtable',
            'rg': resource_group
        })
        self.cmd('network vwan create -n {vwan} -g {rg}')
        self.cmd('network vhub create -g {rg} -n {vhub} --vwan {vwan}  --address-prefix 10.0.0.0/24')
        time.sleep(120)
        ...
        self.cmd('network vhub route-table list -n -g {rg} --vhub-name {vhub}', checks=[
            self.check('@[0].name', self.kwargs['routetable']),
            self.check('length(@)', 1)
        ])

        self.cmd('network vhub route-table update -n -g {rg} --vhub-name {vhub} --attachments All_Branches', checks=[
        ...
        self.cmd('network vhub route-table route add -n {routetable} -g {rg} --vhub-name {vhub} --index 1', checks=[
