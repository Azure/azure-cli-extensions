...
@live_only()
class CodesigningScenario(ScenarioTest):
    
    @ResourceGroupPreparer(name_prefix= 'cli_test_acs_', location='centraluseuap')
    def test_codesigning_account(self, resource_group):
        ...
        self.cmd('codesigning certificate-profile create ' 
                 '-g {rg} ' 
                 '--account-name {name} '
        ...
        # self.cmd('codesigning certificate-profile delete ' 
        #          '-g {rg} ' 
        #          '--account-name {name} '
        #          '-n {private_profile_name} -y')
        
        profilesList = self.cmd('codesigning certificate-profile list '
                                '-g {rg} '
                                '--account-name {name}').get_output_in_json()
        
        assert len(profilesList) == 2 

        # self.cmd('codesigning delete -g {rg} -n {name} -y').get_outputin_json()
