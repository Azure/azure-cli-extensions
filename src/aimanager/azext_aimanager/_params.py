...
def load_arguments(self, _):
    ...
    for scope in ['aimanager modelsource add', 'aimanager modelsource update']:
        with self.argument_context(scope) as c:
            ...
            c.argument('credential', options_list=['--credential'],
                       help='Inline secret (access token or password) the platform uses to '
                            'authenticate to the source. Optional for public sources. '
                            'Update is a full-replace operation and the credential is write-only, '
                            'so omitting --credential clears any previously stored credential; '
                            're-supply it to keep authentication.')
            c.argument('aks_custom_headers', options_list=['--aks-custom-headers'],
                       help='Comma-separated key=value pairs to specify custom headers.')
