...
class TestChaosDiscoveredResourceBrowsing(ScenarioTest):
    ...
    @ResourceGroupPreparer(name_prefix='cli_test_chaos_dr', location='eastus')
    def test_discovered_resource_list_and_show(self, resource_group):
        ...
        # ── prerequisite: create workspace + refresh-recommendations ────
        ...
        list_result = self.cmd(
            'chaos discovered-resource list -w {ws} -g {rg}',
            checks=[
        ...
        self.cmd(
            'chaos discovered-resource show -w {ws} -g {rg} -n {dr_name}',
            checks=[
