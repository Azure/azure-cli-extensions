    ...
    resourceElementTemplates: [
      {
        ...
        configuration: {
          ...
          // If Configuration resource elements (SDFs, Perimeta config) are added, the configMapping
          // must be edited to have these lines (instead of blank values. SNSSelf is null if there are no Configuration elements)
          // "<name of service bus param>": "{configurationparameters('SNSSelf').nfAgentConfiguration.resourceNamespace}",
          // "<name of nf agent UAMI param>": "{configurationparameters('SNSSelf').nfAgentConfiguration.userAssignedIdentityResourceId}",
