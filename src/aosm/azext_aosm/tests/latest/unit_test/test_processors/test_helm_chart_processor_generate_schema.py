...
EXPOSE_TRUE_EXPECTED_SCHEMA = os.path.join(parameter_exposure_directory, "expose-all-true-expected-schema.json")
EXPOSE_FALSE_EXPECTED_SCHEMA = os.path.join(parameter_exposure_directory, "expose-all-false-expected-schema.json")
...
class TestGenerateSchemaExposeAllMock(BaseExposeAllTrue):
    def test_catch_all_yaml(self):
        output_schema = self.helm_chart_processor.generate_schema()
        with open(EXPOSE_TRUE_EXPECTED_SCHEMA, "r", encoding="utf-8") as _file:
            expected_schema = json.load(_file)
        self.assertEqual(output_schema, expected_schema)


class TestGenerateSchemaExposeNoneMock(BaseExposeAllFalse):
    def test_catch_all_yaml(self):
        output_schema = self.helm_chart_processor.generate_schema()
        with open(EXPOSE_FALSE_EXPECTED_SCHEMA, "r", encoding="utf-8") as _file:
            expected_schema = json.load(_file)
        self.assertEqual(output_schema, expected_schema)


class TestGenerateSchemaExposeAllTrueInteger(BaseExposeAllTrue):
    def setUp(self):
        super().setUp()
    ...
    def test_integer_hardcode_nfdv(self):
        # If expose all is true, and hardcode-nfdv, do not expose the parameter
...
class TestGenerateSchemaExposeAllTrueObject(BaseExposeAllTrue):
    def setUp(self):
        """
    ...
    def test_object_no_comment(self):
        ...
        expected_schema = {
            "test-generate-schema-cnf": {
                "type": "object",
                "properties": {
                    "testParameter_object1": {
                        "type": "string",
                        "default": "object1"
                    },
                    "testParameter_object2": {
                        "type": "string",
                        "default": "object2"
                    }
                },
...
class TestGenerateSchemaExposeAllTrueArray(BaseExposeAllTrue):
    def setUp(self):
        """
        This mock looks like:
        testParameter:
...
class TestGenerateSchemaExposeAllTrueEmptyArray(BaseExposeAllTrue):
    def setUp(self):
        super().setUp()
...
class TestGenerateSchemaExposeAllTrueNestedArray(BaseExposeAllTrue):
    def setUp(self):
            ...
            "test-generate-schema-cnf": {
                ...
                "properties": {
                    "testParameter": {
                        ...
                        "default": [{'item1object1': 1, 'item1object2': [{'nestedobject': 2}]}]
                    }
                },
                "required": []
            }
...
class TestGenerateSchemaExposeAllFalseInteger(BaseExposeAllFalse):
    def setUp(self):
        super().setUp()
        self.mocked_yaml = CommentedMap()
        self.mocked_yaml['testParameter'] = 1
...
class TestGenerateSchemaExposeAllFalseObject(BaseExposeAllFalse):
    def setUp(self):
        """
        This mock looks like:
        testParameter:
...
class TestGenerateSchemaExposeAllFalseArray(BaseExposeAllFalse):
    def setUp(self):
        """
        This mock looks like:
        testParameter:
...
class TestGenerateSchemaExposeAllFalseEmptyArray(BaseExposeAllFalse):
    def setUp(self):
        super().setUp()
        self.mocked_yaml = CommentedSeq()
        self.mocked_map = CommentedMap()
...
class TestGenerateSchemaExposeAllFalseNestedArray(BaseExposeAllFalse):
    def setUp(self):
