...
def update_resource_group_in_input_file(input_template_name: str, output_file_name: str, resource_group: str) -> str:
    ...
    rendered_template = jinja_template.render(publisher_resource_group_name=resource_group)
...
class VnfNsdTest(ScenarioTest):
    ...
    def test_vnf_nsd_publish_and_delete(self, resource_group):
        ...
        nfd_input_file_path = update_resource_group_in_input_file(NFD_INPUT_TEMPLATE_NAME, NFD_INPUT_FILE_NAME, resource_group)

        self.cmd(f'az aosm nfd build -f "{nfd_input_file_path}" --definition-type vnf --force')
                
        self.cmd(f'az aosm nfd publish -f "{nfd_input_file_path}" --definition-type vnf --debug')

        # TODO: should I run gets on things to make sure they exist?
    
        nsd_input_file_path = update_resource_group_in_input_file(NSD_INPUT_TEMPLATE_NAME, NSD_INPUT_FILE_NAME, resource_group)

        self.cmd(f'az aosm nsd build -f "{nsd_input_file_path}" --debug --force')
        self.cmd(f'az aosm nsd publish -f "{nsd_input_file_path}" --debug')

        self.cmd(f'az aosm nfd delete --definition-type vnf -f "{nfd_input_file_path}" --debug --force')
