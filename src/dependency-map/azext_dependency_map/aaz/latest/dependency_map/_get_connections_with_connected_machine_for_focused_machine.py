...
class GetConnectionsWithConnectedMachineForFocusedMachine(AAZCommand):
    ...
    :example: Get network connections between machines
        az dependency-map get-connections-with-connected-machine-for-focused-machine --resource-group rgdependencyMap --map-name mapsTest1 --focused-machine-id gagovctcfgocievqwq --connected-machine-id enaieiloylabljxzvmyrshp --start-date-time-utc 2024-03-29T07:35:15.336Z --end-date-time-utc 2024-03-29T07:35:15.336Z
    """
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        _args_schema.connected_machine_id = AAZStrArg(
            options=["--connected-machine-id"],
            arg_group="Body",
            help="Destination machine arm id",
            required=True,
        )
        _args_schema.focused_machine_id = AAZStrArg(
            options=["--focused-machine-id"],
            arg_group="Body",
            help="Source machine arm id",
            required=True,
        )
    ...
    def _execute_operations(self):
        self.pre_operations()
        yield self.MapsGetConnectionsWithConnectedMachineForFocusedMachine(ctx=self.ctx)()
    ...
    class MapsGetConnectionsWithConnectedMachineForFocusedMachine(AAZHttpOperation):
        ...
        def content(self):
            ...
            _builder.set_prop("connectedMachineId", AAZStrType, ".connected_machine_id", typ_kwargs={"flags": {"required": True}})
            ...
            if date_time is not None:
                date_time.set_prop("endDateTimeUtc", AAZStrType, ".end_date_time_utc")
                date_time.set_prop("startDateTimeUtc", AAZStrType, ".start_date_time_utc")

            process_name_filter = _builder.get(".filters.processNameFilter")
            if process_name_filter is not None:
                process_name_filter.set_prop("operator", AAZStrType, ".operator", typ_kwargs={"flags": {"required": True}})
                process_name_filter.set_prop("processNames", AAZListType, ".process_names", typ_kwargs={"flags": {"required": True}})

            process_names = _builder.get(".filters.processNameFilter.processNames")
            if process_names is not None:
                process_names.set_elements(AAZStrType, ".")

            return self.serialize_content(_content_value)


class _GetConnectionsWithConnectedMachineForFocusedMachineHelper:
    """Helper class for GetConnectionsWithConnectedMachineForFocusedMachine"""


__all__ = ["GetConnectionsWithConnectedMachineForFocusedMachine"]
