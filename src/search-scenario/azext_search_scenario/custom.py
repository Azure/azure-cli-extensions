...
def _show_results(cmd, results, highlight_style=Style.ACTION):
    for idx, result in enumerate(results):
        ...
        if highlight_desc:
            print_styled_text(_style_highlight(highlight_desc, highlight_style))
            continue
        highlight_cmd = next(iter(result.get('highlights', {}).get('commandSet/command', [])), None)
        if highlight_cmd:
        ...
        if highlight_name:
            print_styled_text(_style_highlight(highlight_name, highlight_style))
            continue
...
def _show_detail(cmd, scenario):
    ...
    for idx, command in enumerate(scenario["commandSet"]):
        ...
        if 'arguments' in command and cmd.cli_ctx.config.getboolean('search_scenario', 'show_arguments', fallback=False):
            command_item = "{} {}".format(command_item, ' '.join(command['arguments']))
...
def _execute_scenario(ctx_cmd, scenario):
    for command in scenario["commandSet"]:
        ...
        if run_option == 1:
            ...
            execute_result = _execute_cmd(ctx_cmd, command['command'], nx_param, catch_exception=True)
            is_help_printed = False
            while execute_result != 0:
                if not ctx_cmd.cli_ctx.config.getboolean('search_scenario', 'print_help', fallback=False) and not is_help_printed:
                    _print_help_info(ctx_cmd, command["command"])
...
def _style_highlight(highlight_content: str, highlight_style=Style.ACTION) -> list:
    ...
    while remain:
        ...
        try:
            ...
            remain = None
        styled_text_object.append((Style.SECONDARY if not in_highlight else highlight_style, content))
...
def _execute_cmd(ctx_cmd, command, params, catch_exception=False):
    ...
    output_format = ctx_cmd.cli_ctx.config.get('next', 'output', fallback='status')
...
def _get_command_item_sample(command):
    ...
    if "arguments" in command and command["arguments"]:
        ...
        cmd_help = help_files._load_help_file(command['command'])
