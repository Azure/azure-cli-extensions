...
"12" --server-role-groups name="" enable-ha=true role="Coordinator" server-count=1 server-edition="GeneralPurpose" \
storage-quota-in-mb=524288 v-cores=4 --server-role-groups name="" enable-ha=false role="Worker" server-count=3 \
server-edition="MemoryOptimized" storage-quota-in-mb=524288 v-cores=4 --standby-availability-zone "2" --tags \
ElasticServer="1" --resource-group "TestGroup" --name "hsctestsg"
