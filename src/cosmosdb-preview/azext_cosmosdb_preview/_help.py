...
examples:
  ...
  - name: Create a new Azure Cosmos DB database account by restoring only the selected databases,collections or tables from an existing database account.
    text: az cosmosdb restore -g MyResourceGroup --target-database-account-name MyRestoredCosmosDBDatabaseAccount --account-name MySourceAccount --restore-timestamp 2020-07-13T16:03:41+0000 --location westus --databases-to-restore name=MyDB1 collections=collection1 collection2 --gremlin-databases-to-restore name=graphdb1 collections=graph1 graph2 --tables-to-restore tables=table1 table2
"""

# sql and mongodb restorable resources
...
# gremlin and table restorable resources
...
type: group
short-summary: Manage the tables that can be restored in the given account at the given timesamp and region.
"""

helps['cosmosdb table restorable-resource list'] = """
type: command
short-summary: List all the tables that can be restored in the given account at the given timesamp and region.
"""
