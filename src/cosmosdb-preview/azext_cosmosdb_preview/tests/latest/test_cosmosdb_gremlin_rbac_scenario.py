...
class Cosmosdb_previewgremlinRbacScenarioTest(ScenarioTest):
    ...
    def test_cosmosdb_gremlin_role(self, resource_group):
        ...
        fully_qualified_role_def_id1 = '/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.DocumentDB/databaseAccounts/{2}/gremlinRoleDefinitions/{3}'.format(
            subscription, resource_group, acc_name, role_def_id1)
        fully_qualified_role_def_id2 = '/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.DocumentDB/databaseAccounts/{2}/gremlinRoleDefinitions/{3}'.format(
        ...
        self.cmd('az cosmosdb gremlin role definition create -g {rg} -a {acc} -b "{empty_role_def_id_body}"', expect_failure=True)
        self.cmd('az cosmosdb gremlin role definition create -g {rg} -a {acc} -b "{inalid_id_role_def_create_body}"', expect_failure=True)
        self.cmd('az cosmosdb gremlin role definition create -g {rg} -a {acc} -b "{empty_role_name_create_body}"', expect_failure=True)
        self.cmd('az cosmosdb gremlin role definition create -g {rg} -a {acc} -b "{no_resource_role_def_body}"', expect_failure=True)
        self.cmd('az cosmosdb gremlin role definition create -g {rg} -a {acc} -b "{no_actions_role_def_body}"', expect_failure=True)
                
        # Make sure same role def does not exist
        self.cmd(
            'az cosmosdb gremlin role definition delete -g {rg} -a {acc} --role-definition-id {role_def_id1} --yes')
        role_definition_list = self.cmd(
            'az cosmosdb gremlin role definition list -g {rg} -a {acc}').get_output_in_json()
        ...
        # Success Tests
        self.cmd('az cosmosdb gremlin role definition create -g {rg} -a {acc} -b "{create_body}"', checks=[
            self.check('id', fully_qualified_role_def_id1),
            self.check('roleName', user_name)
        ])
        
        self.cmd('az cosmosdb gremlin role definition show -g {rg} -a {acc} --role-definition-id {role_def_id1}', checks=[
            self.check('roleName', user_name)
        ])

        self.cmd('az cosmosdb gremlin role definition update -g {rg} -a {acc} -b "{update_body}"', checks=[
            self.check('id', fully_qualified_role_def_id1),
        ...
        # Make sure same role def does not exist
        self.cmd(
            'az cosmosdb gremlin role definition delete -g {rg} -a {acc} --role-definition-id {role_def_id2} --yes')
        role_definition_list = self.cmd(
            'az cosmosdb gremlin role definition list -g {rg} -a {acc}').get_output_in_json()
        assert len(role_definition_list) == 3
        
        self.cmd('az cosmosdb gremlin role definition create -g {rg} -a {acc} -b "{create_body2}"', checks=[
