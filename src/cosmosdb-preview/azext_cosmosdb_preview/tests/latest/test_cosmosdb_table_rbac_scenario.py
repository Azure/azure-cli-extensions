...
class Cosmosdb_previewtableRbacScenarioTest(ScenarioTest):
    ...
    def test_cosmosdb_table_role(self, resource_group):
        ...
        fully_qualified_role_def_id1 = '/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.DocumentDB/databaseAccounts/{2}/tableRoleDefinitions/{3}'.format(
        ...
        self.cmd(
            'az cosmosdb table role definition delete -g {rg} -a {acc} --role-definition-id {role_def_id1} --yes')
        role_definition_list = self.cmd(
            'az cosmosdb table role definition list -g {rg} -a {acc}').get_output_in_json()
        ...
        # Success Tests
        self.cmd('az cosmosdb table role definition create -g {rg} -a {acc} -b "{create_body}"', checks=[
            self.check('id', fully_qualified_role_def_id1),
            self.check('roleName', user_name)
        ])
        
        self.cmd('az cosmosdb table role definition show -g {rg} -a {acc} --role-definition-id {role_def_id1}', checks=[
            self.check('roleName', user_name)
        ])

        self.cmd('az cosmosdb table role definition update -g {rg} -a {acc} -b "{update_body}"', checks=[
            self.check('id', fully_qualified_role_def_id1),
        ...
        # Make sure same role def does not exist
        self.cmd(
            'az cosmosdb table role definition delete -g {rg} -a {acc} --role-definition-id {role_def_id2} --yes')
        role_definition_list = self.cmd(
            'az cosmosdb table role definition list -g {rg} -a {acc}').get_output_in_json()
        assert len(role_definition_list) == 3
        
        self.cmd('az cosmosdb table role definition create -g {rg} -a {acc} -b "{create_body2}"', checks=[
