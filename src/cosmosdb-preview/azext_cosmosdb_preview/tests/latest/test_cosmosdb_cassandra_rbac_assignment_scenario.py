...
class Cosmosdb_previewcassandraRbacAssignmentScenarioTest(ScenarioTest):
    ...
    def test_cosmosdb_cassandra_role_assignment(self, resource_group):
        ...
        builtin_role_def_id_full = '/subscriptions/80be3961-0521-4a0a-8570-5cd5a4e2f98c/resourceGroups/cli_test_cosmosdb_cassandra_role_assignmentu4ss2n535bbvhe2wmufck5yfhyziujp77gg4/providers/Microsoft.DocumentDB/databaseAccounts/clikznzrccyiwbv/cassandraRoleDefinitions/00000000-0000-0000-0000-000000000001'
        
        builtin_role_def_id_full_update = '/subscriptions/80be3961-0521-4a0a-8570-5cd5a4e2f98c/resourceGroups/cli_test_cosmosdb_cassandra_role_assignmentu4ss2n535bbvhe2wmufck5yfhyziujp77gg4/providers/Microsoft.DocumentDB/databaseAccounts/clikznzrccyiwbv/cassandraRoleDefinitions/00000000-0000-0000-0000-000000000002'
        ...
        self.cmd(
            'az cosmosdb cassandra role assignment delete -g {rg} -a {acc} --role-assignment-id cb8ed2d7-2371-4e3c-bd31-6cc1560e84f8 --yes')
        role_assignment_list = self.cmd(
            'az cosmosdb cassandra role assignment list -g {rg} -a {acc}').get_output_in_json()
        assert len(role_assignment_list) == 0
        
        # Create a role assignment 
        self.cmd('az cosmosdb cassandra role assignment create -g {rg} -a {acc} --scope {scope} --principal-id {principal_id} --role-definition-id {builtin_role_def_id_full} --role-assignment-id {role_assignment_id}', checks=[
            self.check('scope', scope),
            self.check('principalId', principal_id)
        ])
        
        # Show/list role assignment
        self.cmd('az cosmosdb cassandra role assignment show -g {rg} -a {acc} --role-assignment-id cb8ed2d7-2371-4e3c-bd31-6cc1560e84f8', checks=[
        ...
        self.cmd('az cosmosdb cassandra role assignment update -g {rg} -a {acc} --scope {scope} --principal-id {principal_id} --role-definition-id {builtin_role_def_id_full_update} --role-assignment-id {role_assignment_id}', checks=[
            self.check('scope', scope),
            self.check('principalId', principal_id)
        ])
