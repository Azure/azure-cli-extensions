...
def datafactory_factory_create(cmd, client,
    ...
    if isinstance(properties_repo_configuration, str):
        properties_repo_configuration = json.loads(properties_repo_configuration)
    return client.create_or_update(resource_group_name=resource_group_name,
                                   ...
                                   tags=tags,
                                   identity=identity,
                                   repo_configuration=properties_repo_configuration)
...
def datafactory_pipeline_create(cmd, client,
    ...
    if isinstance(properties_activities, str):
        properties_activities = json.loads(properties_activities)
    if isinstance(properties_parameters, str):
        properties_parameters = json.loads(properties_parameters)
    if isinstance(properties_variables, str):
        properties_variables = json.loads(properties_variables)
    if isinstance(properties_annotations, str):
        properties_annotations = json.loads(properties_annotations)
    return client.create_or_update(resource_group_name=resource_group_name,
                                   factory_name=factory_name,
                                   pipeline_name=pipeline_name,
                                   if_match=if_match,
                                   description=properties_description,
                                   activities=properties_activities,
                                   parameters=properties_parameters,
                                   variables=properties_variables,
                                   concurrency=properties_concurrency,
                                   annotations=properties_annotations,
                                   run_dimensions=properties_run_dimensions,
                                   folder=properties_folder)
