...
def load_arguments(self, _):
    ...
    with self.argument_context('datafactory factory show') as c:
        ...
        c.argument('if_none_match', help='ETag of the factory entity. Should only be specified for get. If the ETag matches the existing entity tag, or if * was provided, then no content will be returned.')

    with self.argument_context('datafactory factory create') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('if_match', help='ETag of the factory entity. Should only be specified for update, for which it should match existing entity or can be * for unconditional update.')
        ...
        c.argument('properties_repo_configuration', arg_type=CLIArgumentType(options_list=['--properties-repo-configuration'], help='Git repo information of the factory.'))
    ...
    with self.argument_context('datafactory factory get-data-plane-access') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('permissions', help='The string with permissions for Data Plane access. Currently only \'r\' is supported which grants read only access.')
        c.argument('access_resource_path', help='The resource path to get access relative to factory. Currently only empty string is supported which corresponds to the factory resource.')
        c.argument('profile_name', help='The name of the profile. Currently only the default is supported. The default value is DefaultProfile.')
        c.argument('start_time', help='Start time for the token. If not specified the current time will be used.')
        c.argument('expire_time', help='Expiration time for the token. Maximum duration for the token is eight hours and by default the token will expire in eight hours.')
    ...
    with self.argument_context('datafactory factory configure-factory-repo') as c:
        ...
        c.argument('repo_configuration', arg_type=CLIArgumentType(options_list=['--repo-configuration'], help='Git repo information of the factory.'))
    ...
    with self.argument_context('datafactory integration-runtime show') as c:
        ...
        c.argument('if_none_match', help='ETag of the integration runtime entity. Should only be specified for get. If the ETag matches the existing entity tag, or if * was provided, then no content will be returned.')

    with self.argument_context('datafactory integration-runtime create') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('integration_runtime_name', help='The integration runtime name.')
        c.argument('if_match', help='ETag of the integration runtime entity. Should only be specified for update, for which it should match existing entity or can be * for unconditional update.')
        c.argument('properties', arg_type=CLIArgumentType(options_list=['--properties'], help='Integration runtime properties.'))

    with self.argument_context('datafactory integration-runtime update') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('integration_runtime_name', help='The integration runtime name.')
        c.argument('auto_update', arg_type=get_enum_type(['On', 'Off']), help='Enables or disables the auto-update feature of the self-hosted integration runtime. See https://go.microsoft.com/fwlink/?linkid=854189.')
        c.argument('update_delay_offset', help='The time offset (in hours) in the day, e.g., PT03H is 3 hours. The integration runtime auto update will happen on that time.')
    ...
    with self.argument_context('datafactory integration-runtime create-linked-integration-runtime') as c:
        ...
        c.argument('data_factory_name', help='The name of the data factory that the linked integration runtime belongs to.')
        c.argument('data_factory_location', help='The location of the data factory that the linked integration runtime belongs to.')
    ...
    with self.argument_context('datafactory integration-runtime-node update') as c:
        ...
        c.argument('integration_runtime_name', help='The integration runtime name.')
        c.argument('node_name', help='The integration runtime node name.')
        c.argument('concurrent_jobs_limit', help='The number of concurrent jobs permitted to run on the integration runtime node. Values between 1 and maxConcurrentJobs(inclusive) are allowed.')
    ...
    with self.argument_context('datafactory linked-service show') as c:
        ...
        c.argument('if_none_match', help='ETag of the linked service entity. Should only be specified for get. If the ETag matches the existing entity tag, or if * was provided, then no content will be returned.')

    with self.argument_context('datafactory linked-service create') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('linked_service_name', help='The linked service name.')
        c.argument('if_match', help='ETag of the linkedService entity.  Should only be specified for update, for which it should match existing entity or can be * for unconditional update.')
        c.argument('properties', arg_type=CLIArgumentType(options_list=['--properties'], help='Properties of linked service.'))
    ...
    with self.argument_context('datafactory dataset show') as c:
        ...
        c.argument('if_none_match', help='ETag of the dataset entity. Should only be specified for get. If the ETag matches the existing entity tag, or if * was provided, then no content will be returned.')

    with self.argument_context('datafactory dataset create') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('dataset_name', help='The dataset name.')
        c.argument('if_match', help='ETag of the dataset entity.  Should only be specified for update, for which it should match existing entity or can be * for unconditional update.')
    ...
    with self.argument_context('datafactory pipeline show') as c:
        ...
        c.argument('if_none_match', help='ETag of the pipeline entity. Should only be specified for get. If the ETag matches the existing entity tag, or if * was provided, then no content will be returned.')

    with self.argument_context('datafactory pipeline create') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('pipeline_name', help='The pipeline name.')
        c.argument('if_match', help='ETag of the pipeline entity.  Should only be specified for update, for which it should match existing entity or can be * for unconditional update.')
        c.argument('properties_description', help='The description of the pipeline.')
        c.argument('properties_activities', arg_type=CLIArgumentType(options_list=['--properties-activities'], help='List of activities in pipeline.'))
        c.argument('properties_parameters', arg_type=CLIArgumentType(options_list=['--properties-parameters'], help='List of parameters for pipeline.'))
        c.argument('properties_variables', arg_type=CLIArgumentType(options_list=['--properties-variables'], help='List of variables for pipeline.'))
        ...
        c.argument('properties_run_dimensions', action=AddRunDimensions, nargs='+', help='Dimensions emitted by Pipeline.')
        c.argument('properties_folder', action=AddFolder, nargs='+', help='The folder that this Pipeline is in. If not specified, Pipeline will appear at the root level.')
    ...
    with self.argument_context('datafactory pipeline create-run') as c:
        ...
        c.argument('parameters', action=AddParameters, nargs='+', help='Parameters of the pipeline run. These parameters will be used only if the runId is not specified.')
    ...
    with self.argument_context('datafactory trigger show') as c:
        ...
        c.argument('if_none_match', help='ETag of the trigger entity. Should only be specified for get. If the ETag matches the existing entity tag, or if * was provided, then no content will be returned.')

    with self.argument_context('datafactory trigger create') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('trigger_name', help='The trigger name.')
        c.argument('if_match', help='ETag of the trigger entity.  Should only be specified for update, for which it should match existing entity or can be * for unconditional update.')
        c.argument('properties', arg_type=CLIArgumentType(options_list=['--properties'], help='Properties of the trigger.'))
    ...
    with self.argument_context('datafactory trigger query-by-factory') as c:
        ...
        c.argument('factory_name', help='The factory name.')
        c.argument('continuation_token', help='The continuation token for getting the next page of results. Null for first page.')
        c.argument('parent_trigger_name', help='The name of the parent TumblingWindowTrigger to get the child rerun triggers')
    ...
    with self.argument_context('datafactory data-flow show') as c:
        ...
        c.argument('if_none_match', help='ETag of the data flow entity. Should only be specified for get. If the ETag matches the existing entity tag, or if * was provided, then no content will be returned.')

    with self.argument_context('datafactory data-flow create') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('data_flow_name', help='The data flow name.')
        c.argument('if_match', help='ETag of the data flow entity. Should only be specified for update, for which it should match existing entity or can be * for unconditional update.')
        c.argument('properties', arg_type=CLIArgumentType(options_list=['--properties'], help='Data flow properties.'))
    ...
    with self.argument_context('datafactory data-flow-debug-session create') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('compute_type', help='Compute type of the cluster. The value will be overwritten by the same setting in integration runtime if provided.')
        c.argument('core_count', help='Core count of the cluster. The value will be overwritten by the same setting in integration runtime if provided.')
        c.argument('time_to_live', help='Time to live setting of the cluster in minutes.')
        c.argument('integration_runtime', arg_type=CLIArgumentType(options_list=['--integration-runtime'], help='Set to use integration runtime setting for data flow debug session.'))
    ...
    with self.argument_context('datafactory data-flow-debug-session add-data-flow') as c:
        ...
        c.argument('linked_services', arg_type=CLIArgumentType(options_list=['--linked-services'], help='List of linked services.'))
        c.argument('staging', arg_type=CLIArgumentType(options_list=['--staging'], help='Staging info for debug session.'))
        c.argument('debug_settings', arg_type=CLIArgumentType(options_list=['--debug-settings'], help='Data flow debug settings.'))

    with self.argument_context('datafactory data-flow-debug-session execute-command') as c:
        c.argument('resource_group_name', resource_group_name_type, help='The resource group name.')
        c.argument('factory_name', help='The factory name.')
        c.argument('session_id', help='The ID of data flow debug session.')
        c.argument('command', arg_type=get_enum_type(['executePreviewQuery', 'executeStatisticsQuery', 'executeExpressionQuery']), help='The command type.')
