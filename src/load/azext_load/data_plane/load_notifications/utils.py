...
from azext_load.vendored_sdks.loadtesting.models import NotificationEventType, TestsNotificationRule, TestRunEndedNotificationEventFilter, TestRunStartedNotificationEventFilter, TriggerCompletedNotificationEventFilter, TriggerDisabledNotificationEventFilter, TestRunEndedEventCondition
...
def get_notification_rule_create_body(
    ...
    if not all_tests and not test_ids:
        InvalidArgumentValueError("No tests provided for notification rule. Please provide specific tests using --test-ids option or provide --all-tests flag to select all tests.")
    ...
    if not all_events and not event_filters:
        raise InvalidArgumentValueError("No events provided for notification rule. Please provide specific events using --events option or provide --all-events flag to select all events.")
...
def get_notification_rule_update_body(
    ...
    if all_events and remove_event:
        raise InvalidArgumentValueError("Can't provide --remove-event along with --all-event option.")
    ...
    if remove_event:
        for event in remove_event:
            if not event["event-id"] in event_filters:
                raise InvalidArgumentValueError("Invalid --remove-event value. Event-id: {} not present in existing event filters.".format(event["event-id"]))
...
def _maps_events_to_filters(events):
    ...
    for event in events:
        ...
        if event["type"] == NotificationEventType.TEST_RUN_STARTED:
            ...
            condition = TestRunEndedEventCondition(test_run_results=event["result"], test_run_statuses=event["status"])
            event_filters[event["event-id"]] = TestRunEndedNotificationEventFilter(condition=condition)
