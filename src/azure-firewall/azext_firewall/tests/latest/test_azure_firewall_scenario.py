...
class AzureFirewallScenario(ScenarioTest):
    ...
    def test_azure_policy_draft(self, resource_group, resource_group_location):
        ...
        self.cmd(
            'network firewall policy draft update -g {rg} --name {policy} --threat-intel-mode Deny --idps-mode Alert',
            checks=[
                self.check('type', 'Microsoft.Network/FirewallPolicies/FirewallPolicyDrafts'),
                self.check('threatIntelMode', 'Deny'),
                self.check('name', '{policy}'),
    ...
    def test_azure_policy_rcg_draft(self, resource_group, resource_group_location):
        ...
        self.cmd('network firewall policy rule-collection-group draft create -g {rg} --name {collectiongroup} --priority 150 --policy-name {policy}', checks=[
            self.check('type', 'Microsoft.Network/FirewallPolicies/RuleCollectionGroups/RuleCollectionGroupDrafts'),
            self.check('name', '{collectiongroup}'),
            self.check('priority', 150)
        ])

        self.cmd('network firewall policy rule-collection-group draft update -g {rg} --policy-name {policy}  --name {collectiongroup} --priority 12000', checks=[
            self.check('priority', 12000)
        ])

        self.cmd('network firewall policy rule-collection-group draft show -g {rg} --policy-name {policy} -n {collectiongroup}', checks=[
            self.check('priority', 12000)
        ])
        ...
        self.cmd('network firewall policy rule-collection-group draft show -g {rg} --policy-name {policy} --name {collectiongroup}',
                  checks=[
            self.check('length(ruleCollections[1].rules)', 2),
