...
class ConfigScenarioTest(ScenarioTest):
    ...
    def test_dataprotection_aks_backup_and_restore_initialize_configs(test):
        ...
        backup_config_json = test.cmd('az dataprotection backup-instance initialize-backupconfig --datasource-type AzureKubernetesService '
                                      '--backup-hook-refs "[{\'name\':\'name1\',\'namespace\':\'ns1\'},{\'name\':\'name2\',\'namespace\':\'ns2\'}]"', checks=[
            ...
            test.check('backup_hook_references.name', 'name1')
        ]).get_output_in_json()

        restore_config_json = test.cmd('az dataprotection backup-instance initialize-restoreconfig --datasource-type "{dataSourceType}" '
                                       '--restore-hook-refs "[{\'name\':\'restorehookname\',\'namespace\':\'default\'},{\'name\':\'restorehookname1\',\'namespace\':\'hrweb\'}]"', checks=[
            ...
            test.check('restore_hook_references.name', 'restorehookname')
        ]).get_output_in_json()