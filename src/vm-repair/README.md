...
so they require `--run-on-repair`. The detectors do not modify the attached source OS disk. Both write a
...
`Mode=Report` is the recovery default and makes no configuration changes. Continue to `Mode=Repair`
only when the Windows detector returns `NVME_REPAIR_APPLICABLE` for a Generation 2 guest. Stop when it
...
creates a registry backup and prints its full path. If rollback is needed,
run the same script with `--parameters Mode=Rollback BackupFile=<full-path>`. Use `--no-cleanup` when
restoring the source VM if the repair VM and its evidence or rollback files must be retained.
