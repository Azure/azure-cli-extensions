...
from azure.cli.command_modules.vm.custom import create_vm, create_managed_disk, get_vm, _is_linux_os
...
def swap_disk(cmd, vm_name, resource_group_name, rescue_password=None, rescue_username=None, rescue_vm_name=None):
    ...
    create_rescue_vm_command = 'az vm create -g {g} -n {n} --image {image} --attach-data-disks {disk_name} --admin-username {username} --admin-password {password}' \
                        .format(g=resource_group_name, n=rescue_vm_name, image=os_image_name, disk_name=copied_os_disk_name, username=rescue_username, password=rescue_password)    
