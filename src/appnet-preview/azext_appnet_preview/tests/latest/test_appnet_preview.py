...
class BaseScenario(ScenarioTest):
    ...
    def create_aks_cluster(self):
        ...
        # Install aks-preview via a subprocess so VCR doesn't intercept the
        # extension index lookup. The extension must be present during BOTH
        # record and replay so that `aks create` resolves to the same
        # api-version (otherwise the cassette won't match).
        subprocess.run(
