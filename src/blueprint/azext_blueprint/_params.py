...
def load_arguments(self, _):
    ...
    with self.argument_context('blueprint create') as c:
        ...
        c.argument('target_scope', arg_type=get_enum_type(BlueprintTargetScope), default='subscription',
    ...
    with self.argument_context('blueprint import') as c:
        ...
        c.argument('input_path', type=file_type,
                   help='The directory path for json definitions of the blueprint and artifacts.'
                        ' The blueprint definition file should be named blueprint.json.'
                        ' Artifacts json files should be in a subdirectory named artifacts.',
                   completer=FilesCompleter())
    ...
    with self.argument_context('blueprint export') as c:
        ...
        c.argument('output_path', type=file_type,
                   help='The directory path for json definitions of the blueprint and artifacts.'
                        ' The blueprint definition file will be named blueprint.json.'
                        ' Artifacts json files will be in a subdirectory named artifacts.',
                   completer=FilesCompleter())
        c.argument('skip_confirmation', action='store_true', options_list=['--yes', '-y'],
                   help='Skip user confirmation. When set, if directory does not exist, it will be created.'
                        ' If the directory exists and has contents, they will be overwritten.'
                        ' If not set, user will be prompted for permission to proceed')
    ...
    with self.argument_context('blueprint resource-group add') as c:
        c.argument('blueprint_name', help='Name of the blueprint definition.')
        c.argument('rg_name',
                   help='Name of this resource group.'
                        ' Leave empty if the resource group name will be specified during the blueprint assignment.')
        c.argument('rg_location',
                   help='Location of this resource group. Leave empty if the resource group location'
    ...
    with self.argument_context('blueprint resource-group update') as c:
        ...
        c.argument('depends_on', nargs='*',
                   help="Artifacts which need to be deployed before the specified artifact."
                        " Use '--depends-on' with no values to remove dependencies.")
    ...
    with self.argument_context('blueprint artifact policy create') as c:
        ...
        c.argument('resource_group_art',
                   help='Name of the resource group artifact to which the policy will be assigned.')
        c.argument('parameters', arg_type=parameter_type,
                   help='Parameters for policy assignment artifact. It can be a JSON string or JSON file path.')
    ...
    with self.argument_context('blueprint artifact role create') as c:
        ...
        c.argument('resource_group_art',
                   help='Name of the resource group artifact to which the policy will be assigned.')
        c.argument('principal_ids', nargs='+',
                   help='Array of user or group identities in'
                        ' Azure Active Directory or a reference to the corresponding parameter'
                        ' in blueprint definiton. The roleDefinition will apply to each identity.')
    ...
    with self.argument_context('blueprint artifact template create') as c:
        ...
        c.argument('resource_group_art',
                   help='Name of the resource group artifact to which the policy will be assigned.')
        c.argument('parameters', arg_type=parameter_type,
                   help='Parameters for ARM template artifact. It can be a JSON string or JSON file path.')
    ...
    with self.argument_context('blueprint assignment', validator=blueprint_assignment_validator) as c:
        ...
        c.argument('subscription',
                   help='Use subscription for the target scope of the blueprint assignment.'
                        ' Default susbcription will be used if option not specified.')
        c.argument('management_group',
                   help='Use management group for the target scope of the blueprint assignment.'
                        ' It is reserved for future use. Use --subscription instead.')
    ...
    for scope in ['create', 'update']:
        with self.argument_context('blueprint assignment ' + scope) as c:
            ...
            c.argument('user_assigned_identity', arg_group='Identity',
                       help='The user-assigned managed identity associated with the resource.')
            c.argument('display_name', help='One-liner string explain this resource.')
            c.argument('description', help='Multi-line explain this resource.')
            c.argument('blueprint_id', options_list=['--blueprint-version'],
                       help='Resource ID of the published version of a blueprint definition.')
            c.argument('parameters', arg_type=parameter_type,
                       help='Blueprint assignment parameter values. It can be a JSON string or JSON file path.')
            c.argument('resource_groups', options_list=['--resource-group-value'],
                       action=ResourceGroupAssignAddAction, nargs='+',
                       help="Key=Value pairs for a resource group."
                            " Keys include 'artifact_name'(required), 'name', 'location'.")
            c.argument('locks_mode', arg_type=get_enum_type(AssignmentLockMode), help='Lock mode.')
            c.argument('locks_excluded_principals',
    ...
    with self.argument_context('blueprint assignment create') as c:
        c.argument('identity_type', arg_type=get_enum_type(ManagedServiceIdentityType),
                   default='SystemAssigned', arg_group='Identity', help='Type of the managed identity.')

    with self.argument_context('blueprint assignment update') as c:
        c.argument('identity_type', arg_type=get_enum_type(ManagedServiceIdentityType), arg_group='Identity',
    ...
    with self.argument_context('blueprint assignment wait') as c:
        ...
        c.extra('subscription', arg_type=subscription_type,
                help='Use subscription for the target scope of the blueprint assignment.'
                     ' Default susbcription will be used if option not specified.')
        c.extra('management_group', arg_type=management_group_type,
                help='Use management group for the target scope of the blueprint assignment.'
                     ' It is reserved for future use. Use --subscription instead.')
        c.argument('assignment_name', options_list=['--name', '-n'], help='Name of the blueprint assignment.')
