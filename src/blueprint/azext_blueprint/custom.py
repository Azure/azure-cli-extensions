...
def import_blueprint_with_artifacts(cmd,
    ...
    blueprint_response = BlueprintCreate(cli_ctx=cmd.cli_ctx)(body)

    # delete old artifacts
    artifacts = BlueprintArtifactList(cli_ctx=cmd.cli_ctx)(command_args={'resource_scope': resource_scope,
                                                                         'blueprint_name': blueprint_name})
    for artifact in artifacts:
        BlueprintArtifactDelete(cli_ctx=cmd.cli_ctx)(command_args={'resource_scope': resource_scope,
                                                                   'blueprint_name': blueprint_name,
                                                                   'name': artifact['name']})
    # create new artifacts
    for artifact_name, artifact in art_dict.items():
