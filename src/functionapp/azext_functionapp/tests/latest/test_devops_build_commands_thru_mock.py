...
from ...azure_devops_build_interactive import (
    AzureDevopsBuildInteractive
)


def interactive_patch_path(interactive_module):
    return "azext_functionapp.azure_devops_build_interactive.{}".format(interactive_module)


class TestDevopsBuildCommandsMocked(unittest.TestCase):
    @patch("azext_functionapp.azure_devops_build_interactive.AzureDevopsBuildProvider", new=MagicMock())
    def setUp(self):
    ...
    def test_azure_devops_prechecks_no_file_with_runtime_language(
        self,
        _find_local_repository_runtime_language,
        exists,
        check_git
    ):
        check_git.return_value = True
