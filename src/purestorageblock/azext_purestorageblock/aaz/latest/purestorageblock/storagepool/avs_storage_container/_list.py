...
@register_command(
    "purestorageblock storagepool avs-storage-container list",
    is_preview=True,
)
class List(AAZCommand):
    """List AVS storage containers by storage pool

    :example: AvsStorageContainers_ListByStoragePool
        az purestorageblock storagepool avs-storage-container list --resource-group rgpurestorage --storage-pool-name spName
    """
    ...
    def _execute_operations(self):
        self.pre_operations()
        self.AvsStorageContainersListByStoragePool(ctx=self.ctx)()
    ...
    class AvsStorageContainersListByStoragePool(AAZHttpOperation):
        ...
        def _build_schema_on_200(cls):
            ...
            properties.datastore = AAZStrType(
                flags={"read_only": True},
            )
            properties.mounted = AAZBoolType(
                flags={"read_only": True},
            )
            properties.provisioned_limit = AAZIntType(
                serialized_name="provisionedLimit",
            )
