...
from ._validators import validate_date_format, validate_severity, validate_monitor_condition, validate_signal_type, validate_time_format
...
def load_arguments(self, _):
    ...
    with self.argument_context('monitor processing-rule create') as c:
        ...
        c.argument('enabled', nargs='+', arg_type=get_enum_type(['True', 'False']), help='Indicate if the given alert processing rule is enabled or disabled (values are True and False). Default True.')
        ...
        c.argument('schedule_start_date', help='Start date for the rule. Format: MM/DD/YYYY', validator = validate_date_format)
        c.argument('schedule_end_date', help='End date for the rule. Format: MM/DD/YYYY', validator = validate_date_format)
        c.argument('schedule_start_time', help='Start time for enabling the rule. Format: hh:mm:ss', validator = validate_time_format)
        c.argument('schedule_end_time', help='End time for disabling the rule. Format: hh:mm:ss', validator = validate_time_format)
        ...
        c.argument('schedule_recurrence_type_2', arg_type=get_enum_type(['Always', 'Once', 'Daily', 'Weekly', 'Monthly']), help='Specifies when the processing rule should be applied. Default to Always')
        c.argument('schedule_start_time_2', help='Start time for enabling the rule. Format: hh:mm:ss', validator = validate_time_format)
        c.argument('schedule_end_time_2', help='End time for disabling the rule. Format: hh:mm:ss', validator = validate_time_format)
