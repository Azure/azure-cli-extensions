...
helps['monitor alert-processing-rule create'] = """
    ...
    examples:
      ...
      - name: Create or update a rule that removes all action groups from alerts on a specific VM during a one-off maintenance window (1800-2000 at a specific date, Pacific Standard Time)
        text: |-
              az monitor alert-processing-rule create --name 'RemoveActionGroupsMaintenanceWindow' --rule-type RemoveAllActionGroups --scopes "/subscriptions/MySubscriptionId/resourceGroups/MyResourceGroup1/providers/Microsoft.Compute/virtualMachines/VMName" --resource-group alertscorrelationrg --schedule-start-datetime '2022-01-02 18:00:00' --schedule-end-datetime '2022-01-02 20:00:00' --schedule-time-zone 'Pacific Standard Time' --description "Removes all ActionGroups from all Alerts on VMName during the maintenance window"
      ...
      - name: Create or update a rule that removes all action groups from all alerts on any VM in two resource groups during a recurring maintenance window (2200-0400 every Sat and Sun, India Standard Time)
        text: |-
              az monitor alert-processing-rule create --name 'RemoveActionGroupsRecurringMaintenance' --rule-type RemoveAllActionGroups --scopes "/subscriptions/MySubscriptionId/resourceGroups/MyResourceGroup1" "/subscriptions/MySubscriptionId/resourceGroups/MyResourceGroup2" --resource-group alertscorrelationrg --filter-resource-type Equals "microsoft.compute/virtualmachines" --schedule-time-zone "India Standard Time" --schedule-recurrence-type Weekly --schedule-recurrence-start-time "22:00:00" --schedule-recurrence-end-time "04:00:00" --schedule-recurrence Sunday Saturday --description "Remove all ActionGroups from all Virtual machine Alerts during the recurring maintenance"
