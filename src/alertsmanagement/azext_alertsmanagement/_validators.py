...
def validate_full_operator(args):
    if len(args) < 2:
        raise CLIError('Filter Argument Error: values length can\'t be smaller than 2')
    if args[0].lower() not in ['equals', 'notequals', 'contains', 'doesnotcontain']:
        raise CLIError('Filter Argument Error: operator must be one of the follows: Equals, NotEquals, Contains, DoesNotContain')

def validate_only_equals_operator(args):
    if len(args) < 2:
        raise CLIError('Filter Argument Error: values length can\'t be smaller than 2')
    if args[0].lower() not in ['equals', 'notequals']:
        raise CLIError('Filter Argument Error: operator must be one of the follows: Equals, NotEquals')
