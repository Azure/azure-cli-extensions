...
class ChangeStateCreate(_ChangeStateCreate):
    ...
    def pre_operations(self):
        ...
        self._apply_stage_map_shortcut()
    ...
    def _apply_stage_map_shortcut(self):
        """Translate --stagemap-name into the stage_map resourceId payload."""
        stage_map_arg = getattr(self.ctx.args, "stage_map", None)
        stage_map_name_arg = getattr(self.ctx.args, "stagemap_name", None)
        ...
        if has_stage_map and has_stage_map_name:
            raise InvalidArgumentValueError("Use either --stage-map or --stagemap-name/--stage-map-name, not both.")

        if not has_stage_map_name:
            return

        scope_prefix = self._resolve_stage_map_scope()
        resource_id = f"{scope_prefix}/providers/Microsoft.ChangeSafety/stageMaps/{stage_map_name_arg.to_serialized_data()}"
        logger.debug("Resolved StageMap resourceId from name: %s", resource_id)
        self.ctx.args.stage_map = {
            "resource_id": resource_id
        }

    def _resolve_stage_map_scope(self):
        subscription_id = getattr(self.ctx, "subscription_id", None)
        if not subscription_id:
            raise InvalidArgumentValueError("A subscription is required to resolve the StageMap scope.")
...
class ChangeStateUpdate(_ChangeStateUpdate):
    ...
    def _apply_stage_map_shortcut(self):
        ...
        resource_id = f"/subscriptions/{subscription_id}/providers/Microsoft.ChangeSafety/stageMaps/{stage_map_name_arg.to_serialized_data()}"
        logger.debug("Resolved StageMap resourceId from name: %s", resource_id)
        self.ctx.args.stage_map = {
            "resource_id": resource_id
        }
