...
from azext_apr.commands import task
...
def test_helps_registered():
    ...
    assert "apr task" in helps
    for verb in VERBS:
        assert f"apr task {verb}" in helps
...
def test_command_table_registers_task_verbs():
    ...
    registered = {k for k in loader.command_table if k.startswith("apr task")}
    expected = {f"apr task {v}" for v in VERBS}
...
def test_arguments_load_and_expose_expected_options():
    ...
    loader.load_command_table(None)
    loader.load_arguments("apr task list")
    ...
    assert {"--registry", "-r"} <= options("apr task", "registry_name")
    assert {"--id"} <= options("apr task", "task_id")
    assert {"--state"} <= options("apr task list", "state")

    loader.load_arguments("apr task wait")
    assert {"--ids"} <= options("apr task wait", "task_ids")
    assert registry["apr task wait"]["task_ids"].settings.get("required") is True
