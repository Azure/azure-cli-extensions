...
def test_loader_instantiates():
    from azext_apr import COMMAND_LOADER_CLS, AprCommandsLoader

    assert COMMAND_LOADER_CLS is AprCommandsLoader


def test_client_factory_placeholder_raises(monkeypatch):
    from azext_apr._client_factory import cf_apr
    from azext_apr.server import _data_plane
...
def test_command_table_registers_registry_verbs():
    ...
    registered = {k for k in loader.command_table if k.startswith("apr registry")}
    expected = {
        f"apr registry {verb}" for verb in ["create", "delete", "show", "list", "update", "wait"]
    }
...
def test_data_plane_commands_resolve_to_own_module_without_control_plane_factory():
    ...
    # the control-plane `registry` module AND attaches `cf_apr` — so the command
    ...
    data_plane = {
        ...
        if name.startswith("apr ") and not name.startswith("apr registry")
    }
    ...
    for name, cmd in data_plane.items():
        ...
        assert kwargs["operations_tmpl"] != "azext_apr.commands.registry#{}", (
    ...
    for verb in ["create", "delete", "show", "list", "update", "wait"]:
        cmd = loader.command_table[f"apr registry {verb}"]
