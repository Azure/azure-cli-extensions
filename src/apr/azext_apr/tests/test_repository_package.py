...
from azext_apr.commands import repository_package
...
def test_helps_registered():
    ...
    assert "apr repository package" in helps
    for verb in VERBS:
        assert f"apr repository package {verb}" in helps
...
def test_command_table_registers_verbs():
    ...
    registered = {k for k in loader.command_table if k.startswith("apr repository package")}
    expected = {f"apr repository package {v}" for v in VERBS}
    assert registered == expected
    for verb in ("add", "remove"):
        assert loader.command_table[f"apr repository package {verb}"].supports_no_wait
...
def test_arguments_expose_plural_releases_option():
    ...
    loader.load_command_table(None)
    loader.load_arguments("apr repository package add")
    arguments = loader.argument_registry.arguments

    for verb in VERBS:
        scope = f"apr repository package {verb}"
