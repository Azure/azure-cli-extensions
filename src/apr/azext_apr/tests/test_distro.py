...
from azext_apr.commands import distro

VERBS = ["list", "create", "show", "update", "delete"]

# real-looking APR resource ids - match the resolver's regex, so they skip the name lookup
...
def test_helps_registered():
    ...
    assert "apr distro" in helps
    for verb in VERBS:
        assert f"apr distro {verb}" in helps
...
def test_command_table_registers_distro_verbs():
    ...
    registered = {k for k in loader.command_table if k.startswith("apr distro")}
    expected = {f"apr distro {v}" for v in VERBS}
...
def test_arguments_load_and_expose_expected_options():
    ...
    loader.load_command_table(None)
    loader.load_arguments("apr distro create")
    ...
    assert {"--registry", "-r"} <= options("apr distro", "registry_name")
    assert {"--name", "-n"} <= options("apr distro", "distro_name")
    assert "--type" in options("apr distro create", "distro_type")
    assert {"--base-path"} <= options("apr distro create", "base_path")
    assert {"--repository"} <= options("apr distro create", "repository")
    assert {"--publication"} <= options("apr distro create", "publication")
    # --repository is optional; a distribution may serve a repository, a
    # publication, or neither (backing can be set later).
    assert not registry["apr distro create"]["repository"].settings.get("required")
    # --type is an enum type (case-insensitive), not a raw choices= list
    assert registry["apr distro create"]["distro_type"].settings.get("choices")
    # --no-wait is wired (via supports_no_wait) on the long-running verbs only.
    for verb in ("create", "update", "delete"):
        assert loader.command_table[f"apr distro {verb}"].supports_no_wait
    for verb in ("show", "list"):
        assert not loader.command_table[f"apr distro {verb}"].supports_no_wait
