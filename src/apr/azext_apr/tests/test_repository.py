...
from azext_apr.commands import repository

VERBS = ["list", "create", "show", "update", "delete", "sync", "publish"]

# a real-looking APR resource id - matches the resolver's regex, so it skips the name lookup
...
def test_helps_registered():
    ...
    assert "apr repository" in helps
    for verb in VERBS:
        assert f"apr repository {verb}" in helps
...
def test_command_table_registers_repository_verbs():
    ...
    registered = {
        ...
        if k.startswith("apr repository")
        and not k.startswith("apr repository package")
        and not k.startswith("apr repository release")
    }
    expected = {f"apr repository {v}" for v in VERBS}
    assert registered == expected


def test_arguments_load_and_expose_expected_options():
    # In-process stand-in for `az apr repository ... --help`: running load_arguments
    ...
    loader.load_arguments("apr repository create")
    ...
    assert {"--registry", "-r"} <= options("apr repository", "registry_name")
    assert {"--name", "-n"} <= options("apr repository", "repository_name")
    assert "--type" in options("apr repository create", "repository_type")
    assert {"--retain-versions"} <= options("apr repository create", "retain_versions")
    assert {"--retain-all"} <= options("apr repository create", "retain_all")
    assert {"--remote"} <= options("apr repository update", "remote_name")
    assert {"--retain-versions"} <= options("apr repository update", "retain_versions")
    assert {"--retain-all"} <= options("apr repository update", "retain_all")
    assert {"--sync-mode"} <= options("apr repository sync", "sync_mode")
    assert {"--confirm"} <= options("apr repository sync", "confirm")
    assert loader.command_table["apr repository sync"].confirmation is (
    ...
    for verb in ("update", "delete", "sync", "publish"):
        assert loader.command_table[f"apr repository {verb}"].supports_no_wait
    for verb in ("create", "show", "list"):
        assert not loader.command_table[f"apr repository {verb}"].supports_no_wait
