...
from azext_apr.commands import remote

VERBS = ["list", "create", "show", "update", "delete"]

# real-looking APR resource id - matches the resolver's regex, so it skips the name lookup
...
def test_helps_registered():
    ...
    assert "apr remote" in helps
    for verb in VERBS:
        assert f"apr remote {verb}" in helps
...
def test_command_table_registers_remote_verbs():
    ...
    registered = {k for k in loader.command_table if k.startswith("apr remote")}
    expected = {f"apr remote {v}" for v in VERBS}
...
def test_arguments_load_and_expose_expected_options():
    ...
    loader.load_command_table(None)
    loader.load_arguments("apr remote create")
    ...
    assert {"--registry", "-r"} <= options("apr remote", "registry_name")
    assert {"--name", "-n"} <= options("apr remote", "remote_name")
    assert "--type" in options("apr remote create", "remote_type")
    assert {"--url"} <= options("apr remote create", "url")
    assert {"--policy"} <= options("apr remote create", "policy")
    assert {"--policy"} <= options("apr remote update", "policy")
    for scope in ("apr remote create", "apr remote update"):
        policy_help = registry[scope]["policy"].settings["help"]
        for policy in ("immediate", "on-demand", "streamed"):
            assert policy in policy_help
    assert {"--releases"} <= options("apr remote create", "releases")
    # --no-wait is wired (via supports_no_wait) on the mutating verbs only.
    for verb in ("create", "update", "delete"):
        assert loader.command_table[f"apr remote {verb}"].supports_no_wait
    for verb in ("show", "list"):
        assert not loader.command_table[f"apr remote {verb}"].supports_no_wait
