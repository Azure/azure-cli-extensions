...
def client(monkeypatch):
    ...
    return _data_plane.DataPlaneClient(
        cli_ctx=object(), base_url="https://r.eastus.api.apr.azure.net/"
    )


def test_base_url_normalized(client):
    assert client.base_url == "https://r.eastus.api.apr.azure.net"
...
def test_schemeless_endpoint_gets_https_scheme():
    ...
    client = _data_plane.DataPlaneClient(cli_ctx=object(), base_url="r.eastus.api.apr.azure.net")
    assert client.base_url == "https://r.eastus.api.apr.azure.net"
    assert client._api_root == "https://r.eastus.api.apr.azure.net/api/v1"
...
@pytest.mark.parametrize(
    "base_url",
    [
        "http://r.eastus.api.apr.azure.net",
        "ftp://r.eastus.api.apr.azure.net",
        "ftp://localhost:8100",
        "https://",
    ],
)
...
@pytest.mark.parametrize(
    "base_url",
    [
        "https://r.eastus.api.apr.azure.net?x=1",
        "https://r.eastus.api.apr.azure.net#fragment",
        "https://r.eastus.api.apr.azure.net?",
        "https://r.eastus.api.apr.azure.net#",
    ],
)
...
@pytest.mark.parametrize(
    "base_url",
    [
        "https://r.eastus.api.apr.azure.net/api",
        "https://r.eastus.api.apr.azure.net/other",
    ],
)
...
def test_request_prepends_base_url_and_attaches_auth(client):
    ...
    assert sent["url"] == "https://r.eastus.api.apr.azure.net/api/v1/repositories/"
    ...
    assert "apr-cli-version" in sent["headers"]
...
def test_streaming_multipart_body_is_reiterable(tmp_path):
    from azext_apr.server._data_plane import _StreamingMultipart
...
def test_streaming_multipart_prepares_as_chunked_transfer(tmp_path):
    ...
    prepared = requests.Request(
        "POST",
        "https://r.eastus.api.apr.azure.net/api/v1/packages/",
        data=body,
...
def test_resolve_endpoint_with_resource_group(monkeypatch):
    from azext_apr.server._data_plane import resolve_api_endpoint

    _patch_arm(monkeypatch, [{"properties": {"apiEndpoint": "https://r.eastus.api.apr.azure.net"}}])
    endpoint = resolve_api_endpoint(_cmd(), "r", resource_group_name="rg1")
    assert endpoint == "https://r.eastus.api.apr.azure.net"
...
def _patch_extensions(monkeypatch, ext_type, cover=True):
    ...
    # Parent-of-parent of .../azext_apr/server is the repo dir that contains the
...
def test_for_registry_installed_build_resolves_via_arm(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda cmd, name, rg=None: "https://r.eastus.api.apr.azure.net",
    )
    cmd = mock.Mock(cli_ctx=object())
