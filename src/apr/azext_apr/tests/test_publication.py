...
from azext_apr.commands import publication
...
def test_helps_registered():
    ...
    assert "apr publication" in helps
    for verb in VERBS:
        assert f"apr publication {verb}" in helps
...
def test_command_table_registers_publication_verbs():
    ...
    registered = {k for k in loader.command_table if k.startswith("apr publication")}
    expected = {f"apr publication {v}" for v in VERBS}
...
def test_arguments_load_and_expose_expected_options():
    ...
    loader.load_command_table(None)
    loader.load_arguments("apr publication list")
    ...
    assert {"--registry", "-r"} <= options("apr publication", "registry_name")
    assert {"--id"} <= options("apr publication", "publication_id")
    assert {"--repository"} <= options("apr publication list", "repository")
    # --no-wait is wired on delete only.
    assert loader.command_table["apr publication delete"].supports_no_wait
    for verb in ("show", "list"):
        assert not loader.command_table[f"apr publication {verb}"].supports_no_wait
