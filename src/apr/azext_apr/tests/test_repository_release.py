...
from azext_apr.commands import repository_release as rel

VERBS = ["list", "create", "delete"]

# real-looking APR resource ids - match the resolver regexes so they skip name lookups
...
def test_helps_registered():
    ...
    assert "apr repository release" in helps
    for verb in VERBS:
        assert f"apr repository release {verb}" in helps
...
def test_command_table_registers_release_verbs():
    ...
    registered = {
        ...
        if k.startswith("apr repository release ")
        and not k.startswith("apr repository release component")
    }
    expected = {f"apr repository release {v}" for v in VERBS}
...
def test_arguments_load_and_expose_expected_options():
    ...
    loader.load_command_table(None)
    loader.load_arguments("apr repository release create")
    ...
    grp = "apr repository release"
    create = "apr repository release create"
    ...
    for verb in ("create", "delete"):
        assert loader.command_table[f"apr repository release {verb}"].supports_no_wait
    assert not loader.command_table["apr repository release list"].supports_no_wait
...
def test_component_helps_registered():
    ...
    assert "apr repository release component" in helps
    for verb in COMPONENT_VERBS:
        assert f"apr repository release component {verb}" in helps
...
def test_command_table_registers_release_component_verbs():
    ...
    registered = {
        k for k in loader.command_table if k.startswith("apr repository release component")
    }
    expected = {f"apr repository release component {v}" for v in COMPONENT_VERBS}
    assert registered == expected
...
def test_component_arguments_load_and_expose_expected_options():
    ...
    loader.skip_applicability = True
    loader.load_command_table(None)
    loader.load_arguments("apr repository release component create")
    registry = loader.argument_registry.arguments

    def options(scope, dest):
        return set(registry[scope][dest].settings.get("options_list") or [])

    base = "apr repository release"
    grp = "apr repository release component"
    ...
    # --no-wait is wired on the long-running verbs only
    for verb in ("create", "delete"):
        assert loader.command_table[f"apr repository release component {verb}"].supports_no_wait
    assert not loader.command_table["apr repository release component list"].supports_no_wait
