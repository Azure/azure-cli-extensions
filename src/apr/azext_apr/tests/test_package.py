...
from azext_apr.commands import package
...
def test_helps_registered():
    ...
    assert "apr package" in helps
    assert "apr package upload" in helps
    for sub in SUBGROUPS:
        assert f"apr package {sub}" in helps
        assert f"apr package {sub} list" in helps
        assert f"apr package {sub} show" in helps


def test_command_table_registers_per_type_subgroups():
    from azure.cli.core.mock import DummyCli

    from azext_apr import AprCommandsLoader

    loader = AprCommandsLoader(cli_ctx=DummyCli())
    loader.load_command_table(None)

    registered = {k for k in loader.command_table if k.startswith("apr package")}
    expected = {"apr package upload"}
    for sub in SUBGROUPS:
        expected.add(f"apr package {sub} list")
        expected.add(f"apr package {sub} show")
...
def test_arguments_are_scoped_per_type():
    ...
    loader.load_command_table(None)
    loader.load_arguments("apr package upload")
    for sub in SUBGROUPS:
        loader.load_arguments(f"apr package {sub} list")
        loader.load_arguments(f"apr package {sub} show")
    ...
    for sub in SUBGROUPS:
        # registry_name/-r propagates from the top "apr package" scope
        assert {"--registry", "-r"} <= options("apr package", "registry_name")
        assert {"--id"} <= options(f"apr package {sub} show", "package_id")
        assert {"--repository"} <= options(f"apr package {sub} list", "repository")
        assert {"--name"} <= options(f"apr package {sub} list", "name")
        assert {"--sha256"} <= options(f"apr package {sub} list", "sha256")

    # --release only exists on the types that have a release concept
    for sub in ("deb", "debsrc", "rpm"):
        assert {"--release"} <= options(f"apr package {sub} list", "release")
    # file has no --release argument at all
    assert "release" not in registry.get("apr package file list", {})
    assert {"--file"} <= options("apr package upload", "package")
    assert {"--repository"} <= options("apr package upload", "repository")
    assert {"--type", "-t"} <= options("apr package upload", "file_type")
    assert {"--relative-path"} <= options("apr package upload", "relative_path")
    assert {"--release"} <= options("apr package upload", "release")
    assert {"--component"} <= options("apr package upload", "component")
    assert loader.command_table["apr package upload"].supports_no_wait
