...
helps["apr distro list"] = """
type: command
short-summary: List distributions.
examples:
  - name: List all distributions in a registry.
    text: az apr distro list -g myrg -r myregistry
  - name: List distributions serving a given repository.
    text: az apr distro list -g myrg -r myregistry --repository myrepo
"""

helps["apr distro create"] = """
type: command
short-summary: Create a distribution.
examples:
  - name: Create an apt distribution serving a repository.
    text: |
      az apr distro create -g myrg -r myregistry -n mydistro \
        --type apt --base-path dists/stable --repository myrepo
"""

helps["apr distro show"] = """
type: command
short-summary: Show details for a distribution.
examples:
  - name: Show a distribution by name.
    text: az apr distro show -g myrg -r myregistry -n mydistro
"""

helps["apr distro update"] = """
type: command
short-summary: Update a distribution.
examples:
  - name: Change the base path a distribution is served at.
    text: az apr distro update -g myrg -r myregistry -n mydistro --base-path dists/next
"""

helps["apr distro delete"] = """
type: command
short-summary: Delete a distribution.
examples:
  - name: Delete a distribution by name.
    text: az apr distro delete -g myrg -r myregistry -n mydistro
"""


def register_commands(loader):
    distro_type = CliCommandType(operations_tmpl="azext_apr.commands.distro#{}")
    with loader.command_group("apr distro", custom_command_type=distro_type, is_preview=True) as g:
...
def load_arguments(loader, _command):
    with loader.argument_context("apr distro") as c:
    ...
    with loader.argument_context("apr distro create") as c:
        c.argument(
            "distro_type",
            options_list=["--type"],
    ...
    for scope in ("apr distro create", "apr distro update"):
    ...
    with loader.argument_context("apr distro list") as c:
