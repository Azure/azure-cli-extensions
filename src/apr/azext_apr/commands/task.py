...
from azext_apr.server import DataPlaneClient, list_all, poll_task
...
examples:
  - name: List all tasks in a registry.
    text: az apr task list -g myrg -r myregistry
  - name: List only failed tasks.
    text: az apr task list -g myrg -r myregistry --state failed
"""
...
examples:
  - name: Show a task by id.
    text: az apr task show -g myrg -r myregistry --id mytaskid
"""
...
examples:
  - name: Wait for several tasks to finish.
    text: az apr task wait -g myrg -r myregistry --ids task1,task2
"""

helps["apr task cancel"] = """
type: command
short-summary: Cancel a task.
examples:
  - name: Cancel a running task by id.
    text: az apr task cancel -g myrg -r myregistry --id mytaskid
"""


def register_commands(loader):
    task_type = CliCommandType(operations_tmpl="azext_apr.commands.task#{}")
    with loader.command_group("apr task", custom_command_type=task_type, is_preview=True) as g:
...
def load_arguments(loader, _command):
    with loader.argument_context("apr task") as c:
    ...
    with loader.argument_context("apr task list") as c:
        c.ignore("task_id")
        c.argument(
            "state",
            options_list=["--state"],
    ...
    with loader.argument_context("apr task wait") as c:
        c.ignore("task_id")
        c.argument(
            "task_ids",
            options_list=["--ids"],
