...
helps["apr package upload"] = """
type: command
short-summary: Upload a package.
examples:
  - name: Upload a package without adding it to a repository.
    text: |
      az apr package upload -g myrg -r myregistry --file ./my-package.deb
  - name: Upload a deb package to a repository.
    text: |
      az apr package upload -g myrg -r myregistry --repository myrepo \
...
for _ptype, _summary in _TYPE_SUMMARY.items():
    helps[f"apr package {_ptype}"] = f"""
type: group
short-summary: Manage {_summary}.
"""
    helps[f"apr package {_ptype} list"] = f"""
type: command
short-summary: List {_summary}.
"""
    helps[f"apr package {_ptype} show"] = """
type: command
...
def register_commands(loader):
    package_type = CliCommandType(operations_tmpl="azext_apr.commands.package#{}")
    with loader.command_group(
        "apr package", custom_command_type=package_type, is_preview=True
    ) as g:
    ...
    for ptype in PACKAGE_TYPES:
        with loader.command_group(
            f"apr package {ptype}", custom_command_type=package_type, is_preview=True
        ) as g:
...
def load_arguments(loader, _command):
    with loader.argument_context("apr package") as c:
    ...
    with loader.argument_context("apr package upload") as c:
    ...
    for ptype in PACKAGE_TYPES:
        with loader.argument_context(f"apr package {ptype} show") as c:
            c.argument("package_id", options_list=["--id"], help="Package id.")

        with loader.argument_context(f"apr package {ptype} list") as c:
            c.argument(
                "repository",
                options_list=["--repository"],
    ...
    with loader.argument_context("apr package rpm list") as c:
        c.argument(
            "release",
            options_list=["--release"],
