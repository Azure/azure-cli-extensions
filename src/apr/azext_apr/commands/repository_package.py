...
helps["apr repository package"] = """
type: group
short-summary: Manage the packages contained in a repository.
"""

helps["apr repository package add"] = """
type: command
short-summary: Add packages to a repository.
examples:
  - name: Add packages to a repository.
    text: az apr repository package add -g myrg -r myregistry -n myrepo --packages pkg1,pkg2
  - name: Add packages to a specific release of an apt repository.
    text: |
      az apr repository package add -g myrg -r myregistry -n myrepo \
...
helps["apr repository package remove"] = """
type: command
short-summary: Remove packages from a repository.
examples:
  - name: Remove packages from a repository.
    text: az apr repository package remove -g myrg -r myregistry -n myrepo --packages pkg1,pkg2
  - name: Remove packages from a specific component of an apt release.
    text: |
      az apr repository package remove -g myrg -r myregistry -n myrepo \
        --packages pkg1 --releases jammy --component contrib
"""
...
def register_commands(loader):
    repository_package_type = CliCommandType(
        operations_tmpl="azext_apr.commands.repository_package#{}"
    )
    with loader.command_group(
        "apr repository package", custom_command_type=repository_package_type, is_preview=True
    ) as g:
...
def load_arguments(loader, _command):
    with loader.argument_context("apr repository package") as c:
    ...
    for scope in ("apr repository package add", "apr repository package remove"):
