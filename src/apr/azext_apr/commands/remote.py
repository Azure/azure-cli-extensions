...
helps["apr remote list"] = """
type: command
short-summary: List remotes.
examples:
  - name: List all remotes in a registry.
    text: az apr remote list -g myrg -r myregistry
"""

helps["apr remote create"] = """
type: command
short-summary: Create a remote.
examples:
  - name: Create an apt remote syncing selected distributions and components.
    text: |
      az apr remote create -g myrg -r myregistry -n myremote --type apt \
        --url http://archive.ubuntu.com/ubuntu \
        --releases jammy,focal --components main,universe
  - name: Create a yum remote.
    text: |
      az apr remote create -g myrg -r myregistry -n myremote --type yum \
        --url https://packages.microsoft.com/rhel/9/prod --policy on-demand
"""

helps["apr remote show"] = """
type: command
short-summary: Show details for a remote.
examples:
  - name: Show a remote by name.
    text: az apr remote show -g myrg -r myregistry -n myremote
"""

helps["apr remote update"] = """
type: command
short-summary: Update a remote.
examples:
  - name: Change the upstream URL a remote syncs from.
    text: az apr remote update -g myrg -r myregistry -n myremote --url http://archive.ubuntu.com/ubuntu
  - name: Download packages only when clients request them.
    text: az apr remote update -g myrg -r myregistry -n myremote --policy on-demand
"""

helps["apr remote delete"] = """
type: command
short-summary: Delete a remote.
examples:
  - name: Delete a remote by name.
    text: az apr remote delete -g myrg -r myregistry -n myremote
"""


def register_commands(loader):
    remote_type = CliCommandType(operations_tmpl="azext_apr.commands.remote#{}")
    with loader.command_group("apr remote", custom_command_type=remote_type, is_preview=True) as g:
...
def load_arguments(loader, _command):
    with loader.argument_context("apr remote") as c:
    ...
    with loader.argument_context("apr remote create") as c:
        c.argument(
            "remote_type",
            options_list=["--type"],
    ...
    with loader.argument_context("apr remote update") as c:
        c.argument("url", options_list=["--url"], help="Upstream URL to sync from.")
    ...
    with loader.argument_context("apr remote list") as c:
