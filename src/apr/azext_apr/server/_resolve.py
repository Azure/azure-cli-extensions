...
Users can pass either an APR resource id or a plain name. If it's a name, we
look it up by filtering the list endpoint on an exact name
(/<resource>/?name=<value>), which returns 0 or 1 matches.

Note: these ids are the APR data-plane's own single-segment ids (they slot
...
from azext_apr.server._paging import iter_results

_UUID = r"[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"

# Apt releases (deb + debsrc share them) translate from Pulp's
# /content/deb/releases/<uuid>/ href, so their APR id is content-deb-releases-<uuid>.
...
# their APR id is content-deb-release_components-<uuid>.
_RELEASE_COMPONENT_ID_PREFIX = "content-deb-release_components"


def _looks_like_id(resource: str, value: str) -> bool:
    # an APR resource id is a single path segment ending in a uuid, optionally
