"""Shared HTTP client for the APR data plane.
...
class DataPlaneClient:
    ...
    def for_registry(
        ...
        if is_dev_extension():
            logger.warning(
                "Dev build detected (azdev); targeting local APR server %s and "
...
def raise_if_dev_extension() -> None:
    ...
    Control-plane verbs (``az apr registry ...``) manage the Azure resource via
    ...
    if is_dev_extension():
        raise ValidationError(
            "Control-plane commands (az apr registry ...) are not available in a "
            "local dev build of this extension. A dev build targets the local APR "
...
def resolve_api_endpoint(
    ...
    ARM (``send_raw_request``) until the azure-mgmt-apr SDK ships.
    """
