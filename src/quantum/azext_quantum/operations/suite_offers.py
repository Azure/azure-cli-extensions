...
def suite_offer_quotas(cmd, provider_id):
    ...
    canonical_provider_id = offer.properties.provider_id
    ...
    return _merge_suite_offer_quotas(offer, usages, canonical_provider_id)
...
def suite_offer_targets(cmd, provider_id):
    ...
    client = cf_suite_offers_data_plane(cmd.cli_ctx, subscription_id, offer.properties.location)
    try:
        status = client.get_provider_status(subscription_id, canonical_provider_id)
    except AzureResourceNotFoundError as ex:
        raise ResourceNotFoundError(
...
def _merge_suite_offer_quotas(offer, usages, provider_id):
    ...
    usage_by_target = {
        usage.target_id.lower(): usage
        for usage in (usages or [])
        if usage.target_id is not None and usage.scope == _SUITE_OFFER_QUOTA_SCOPE
    }

    rows = []
    for target_quota in offer.properties.target_quotas or []:
        usage = usage_by_target.get(target_quota.target_id.lower())
