...
class ProgramArgsAction(argparse._AppendAction):
    def __call__(self, parser, namespace, values, option_string=None):
        action = self.get_action(values)
        namespace.optional_program_args = action
...
def load_arguments(self, _):
    ...
    optional_program_args_type = CLIArgumentType(options_list=['--program-args'], help='Program arguments passed to the Q# compiler as a list of key=value pairs separated by spaces. Omit the leading "--" on argument names.', action=ProgramArgsAction, nargs='+')
    ...
    with self.argument_context('quantum job submit') as c:
        ...
        c.argument('optional_program_args', optional_program_args_type)
