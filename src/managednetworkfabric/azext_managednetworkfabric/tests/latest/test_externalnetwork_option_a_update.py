...
def step_show(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd(
        "az networkfabric externalnetwork show --resource-name {name} --l3-isolation-domain-name {l3Domain} --resource-group {rg}"
    )
...
def step_update_s1(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd(
        "az networkfabric externalnetwork update --resource-group {rg} --l3domain {l3Domain} --resource-name {name} "
        " --peering-option {s2PeeringOption} --option-a-properties {updatedOptionAProperties} --annotation {annotation} "
        " --static-route-config {staticRouteConfig} --network-to-network-interconnect-id {nniId}",
        checks=checks,
    )
...
def step_list_resource_group(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd(
        "az networkfabric externalnetwork list --resource-group {rg} --l3-isolation-domain-name {l3Domain}"
    )
...
class GA_ExternalNetworkOptionAUpdateScenarioTest1(ScenarioTest):
    ...
    def test_GA_externalnetwork_option_a_update_scenario1(self):
