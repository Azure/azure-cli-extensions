...
def step_create_s1(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd(
        "az networkfabric externalnetwork create --resource-group {rg} --l3domain {l3Domain} --resource-name {name} --peering-option {s1PeeringOption} --option-b-properties {optionBProperties}"
        " --import-route-policy {importRoutePolicy} --export-route-policy {exportRoutePolicy} --annotation {annotation} --nni-id {nniId} --static-route-configuration {staticRouteConfig}",
        checks=checks,
    )
...
def step_delete(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd(
        "az networkfabric externalnetwork delete --resource-name {name} --l3-isolation-domain-name {l3Domain} --resource-group {rg}"
    )
...
class GA_ExternalNetworkOptionBScenarioTest1(ScenarioTest):
    ...
    def test_GA_externalnetwork_option_b_scenario1(self):
