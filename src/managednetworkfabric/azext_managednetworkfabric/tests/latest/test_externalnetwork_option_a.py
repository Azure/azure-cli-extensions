...
def step_create_s1(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd(
        "az networkfabric externalnetwork create --resource-group {rg} --l3-isolation-domain-name {l3Domain} --resource-name {name}"
        " --peering-option {s2PeeringOption} --option-a-properties {optionAProperties} --network-to-network-interconnect-id {nniId}"
        " --static-route-config {staticRouteConfig} --annotation {annotation}",
        checks=checks,
    )
...
class GA_ExternalNetworkOptionAScenarioTest1(ScenarioTest):
    ...
    def test_GA_externalnetwork_option_a_scenario1(self):
