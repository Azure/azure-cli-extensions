...
def step_create_s1(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd(
        'az networkfabric routepolicy create --resource-group {rg} --resource-name {name} --location {location} --default-action {defaultAction} --nf-id {nfId} --address-family-type {addressFamilyType} --statements {statements_with_ipcommunity}', checks=checks)
...
def step_create_s2(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd(
        'az networkfabric routepolicy create --resource-group {rg} --resource-name {name} --location {location} --default-action {defaultAction} --nf-id {nfId} --address-family-type {addressFamilyType} --statements {statements_with_ipextcommunity}', checks=checks)
