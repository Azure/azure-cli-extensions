...
def radius_policy_insert(
    ...
    # Handles both:
    #   ccePolicy: 'value'  (direct property)
    #   'microsoft.containerinstance.virtualnode.ccepolicy': 'value'  (annotation)
    def replace_cce_policy(match):
        full_match = match.group(0)
        # Find where the colon is to split key from value
    ...
    # Match any key containing 'ccepolicy' (case-insensitive) followed by a quoted value
    pattern = r'["\']?[^"\']*[cC][cC][eE][pP][oO][lL][iI][cC][yY][^"\']*["\']?\s*:\s*["\'][^"\']*["\']'
