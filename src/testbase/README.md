...
##### Create #####
```
az testbase account create --location "westus" --sku-name "S0" --resource-group "contoso-rg1" \
    --name "contoso-testBaseAccount1" 

az testbase account wait --created --resource-group "{rg}" --name "{myTestBaseAccount}"
```
##### Show #####
```
...
az testbase package wait --created --name "{myPackage}" --resource-group "{rg}" --account-name "{myTestBaseAccount}"
```
##### Show #####
```
