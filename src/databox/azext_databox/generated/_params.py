...
def load_arguments(self, _):
    ...
    with self.argument_context('databox job create') as c:
        ...
        c.argument('type_', options_list=['--type'], type=str, help='Identity type', arg_group='Identity')
        c.argument('user_assigned_identities', type=validate_file_or_dict, help='User Assigned Identities Expected '
                   'value: json-string/@json-file.', arg_group='Identity')
        ...
        c.argument('scheduled_date_time', help='Scheduled date time.', arg_group='Delivery Info')
    ...
    with self.argument_context('databox job update') as c:
        ...
        c.argument('shipping_address', action=AddShippingAddress, nargs='+', help='Shipping address of the customer.',
                   arg_group='Details')
        c.argument('kek_type', arg_type=get_enum_type(['MicrosoftManaged', 'CustomerManaged']), help='Type of '
                   'encryption key used for key encryption.', arg_group='Details Key Encryption Key')
        c.argument('kek_url', type=str, help='Key encryption key. It is required in case of Customer managed KekType.',
                   arg_group='Details Key Encryption Key')
        c.argument('kek_vault_resource_id', type=str, help='Kek vault resource id. It is required in case of Customer '
                   'managed KekType.', arg_group='Details Key Encryption Key')
        c.argument('type_', options_list=['--type'], type=str, help='Managed service identity type.',
                   arg_group='Details Key Encryption Key Identity Properties')
        c.argument('user_assigned', action=AddUserAssigned, nargs='+', help='User assigned identity properties.',
                   arg_group='Details Key Encryption Key Identity Properties')
        c.argument('contact_name', type=str, help='Contact name of the person.', arg_group='Details Contact Details')
        c.argument('phone', type=str, help='Phone number of the contact person.', arg_group='Details Contact Details')
        c.argument('phone_extension', type=str, help='Phone extension number of the contact person.',
                   arg_group='Details Contact Details')
        c.argument('mobile', type=str, help='Mobile number of the contact person.',
                   arg_group='Details Contact Details')
        c.argument('email_list', nargs='+', help='List of Email-ids to be notified about job progress.',
                   arg_group='Details Contact Details')
        c.argument('notification_preference', action=AddNotificationPreference, nargs='+', help='Notification '
                   'preference for a job stage.', arg_group='Details Contact Details')
        c.argument('resource_identity_type', type=str, help='Identity type', arg_group='Identity')
        c.argument('user_assigned_identities', type=validate_file_or_dict, help='User Assigned Identities Expected '
                   'value: json-string/@json-file.', arg_group='Identity')
    ...
    with self.argument_context('databox mitigate') as c:
        c.argument('job_name', type=str, help='The name of the job Resource within the specified resource group. job '
                   'names must be between 3 and 24 characters in length and use any alphanumeric and underscore only',
                   id_part='name')
        c.argument('resource_group_name', resource_group_name_type)
        c.argument('customer_resolution_code', arg_type=get_enum_type(['None', 'MoveToCleanUpDevice', 'Resume']),
    ...
    with self.argument_context('databox service region-configuration') as c:
        ...
        c.argument('data_box_schedule_availability_request', action=AddDataBoxScheduleAvailabilityRequest, nargs='+',
                   help='Request body to get the availability for scheduling data box orders orders.',
                   arg_group='ScheduleAvailabilityRequest')
        c.argument('disk_schedule_availability_request', action=AddDiskScheduleAvailabilityRequest, nargs='+',
                   help='Request body to get the availability for scheduling disk orders.',
                   arg_group='ScheduleAvailabilityRequest')
        c.argument('heavy_schedule_availability_request', action=AddHeavyScheduleAvailabilityRequest, nargs='+',
                   help='Request body to get the availability for scheduling heavy orders.',
                   arg_group='ScheduleAvailabilityRequest')
        c.argument('sku_name', arg_type=get_enum_type(['DataBox', 'DataBoxDisk', 'DataBoxHeavy']), help='Type of the '
                   'device.', arg_group='Transport Availability Request')
    ...
    with self.argument_context('databox service validate-address') as c:
        ...
        c.argument('preferred_shipment_type', arg_type=get_enum_type(['CustomerManaged', 'MicrosoftManaged']),
                   help='Indicates Shipment Logistics type that the customer preferred.', arg_group='Transport '
                   'Preferences')
