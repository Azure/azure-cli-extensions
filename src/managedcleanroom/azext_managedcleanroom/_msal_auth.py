...
def get_config_value(cmd, config_key, env_var_name, default_value):
    ...
    if env_value:
        logger.debug(f"Using {config_key} from environment variable: {env_var_name}")
        return env_value
    ...
    if config_value:
        logger.debug(f"Using {config_key} from Azure CLI config")
        return config_value
    
    # Priority 3: Default value
    logger.debug(f"Using default {config_key}")
...
def get_msal_config(cmd):
    ...
    """
    client_id = get_config_value(cmd, 'client_id', 'MANAGEDCLEANROOM_CLIENT_ID', DEFAULT_MS_CLIENT_ID)
    tenant_id = get_config_value(cmd, 'tenant_id', 'MANAGEDCLEANROOM_TENANT_ID', DEFAULT_MS_TENANT_ID)
    scopes_str = get_config_value(cmd, 'scopes', 'MANAGEDCLEANROOM_SCOPES', DEFAULT_MS_SCOPES)
...
def get_msal_token(cmd):
    ...
    try:
        ...
        account = accounts[0]
        logger.debug(f"Attempting to acquire token for account: {account.get('username', 'unknown')}")
        ...
        if result and "access_token" in result:
            ...
            tenant_id = result.get("id_token_claims", {}).get("tid", config['tenant_id'])
            ...
            logger.debug(f"Failed to acquire token silently: {result.get('error', 'Unknown error')}")
            return None
            
    except Exception as ex:
        logger.debug(f"Error retrieving MSAL token: {ex}")
        return None
...
def clear_msal_cache():
    ...
    if cache_file.exists():
        try:
            cache_file.unlink()
            logger.info(f"Cleared MSAL token cache: {cache_file}")
        except Exception as ex:
            raise Exception(f"Failed to clear token cache: {ex}")
    else:
