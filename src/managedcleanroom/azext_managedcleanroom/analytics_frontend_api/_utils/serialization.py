...
class RawDeserializer:
    ...
    def deserialize_from_text(
            cls, data: Optional[Union[AnyStr, IO]], content_type: Optional[str] = None) -> Any:
        ...
        if cls.JSON_REGEXP.match(content_type):
            try:
                ...
                raise DeserializationError(
                    "JSON is invalid: {}".format(err), err) from err
    ...
    def deserialize_from_http_generics(
            cls, body_bytes: Optional[Union[AnyStr, IO]], headers: Mapping) -> Any:
...
class Model:
    ...
    def _create_xml_node(cls):
        ...
        return _create_xml_node(
            xml_map.get(
                "name", cls.__name__), xml_map.get(
                "prefix", None), xml_map.get(
                "ns", None))
    ...
    def _infer_class_models(cls):
        try:
            ...
            client_models = {
                k: v for k,
                v in models.__dict__.items() if isinstance(
                    v,
                    type)}
            if cls.__name__ not in client_models:
                raise ValueError("Not Autorest generated code")
        except Exception:  # pylint: disable=broad-exception-caught
            # Assume it's not Autorest generated (tests?). Add ourselves as
            # dependencies.
    ...
    def deserialize(
            cls,
            data: Any,
            content_type: Optional[str] = None) -> Self:
    ...
    def _flatten_subtype(cls, key, objects):
        ...
        for valuetype in cls._subtype_map[key].values():
            result |= objects[valuetype]._flatten_subtype(
                key, objects)  # pylint: disable=protected-access
        return result
        ...
        for subtype_key in cls.__dict__.get("_subtype_map", {}).keys():
            ...
            if not isinstance(response, ET.Element):
                ...
                subtype_value = response.get(
                    rest_api_response_key, None) or response.get(
                    subtype_key, None)
            else:
    ...
    def _get_rest_key_parts(cls, attr_key):
        ...
        return [_decode_attribute_map_key(key_part)
                for key_part in rest_split_key]
...
class Serializer:  # pylint: disable=too-many-public-methods
                ...
                try:
                    ...
                    if is_xml_model_serialization:
                        ...
                        if isinstance(new_attr, list):
                            ...
                            # we MUST replace the tag with the local tag. But
                            # keeping the namespaces.
        ...
        if internal_data_type and not isinstance(internal_data_type, Enum):
            try:
                ...
                data = deserializer._deserialize(
                    data_type, data)  # pylint: disable=protected-access
            except DeserializationError as err:
                raise SerializationError(
                    "Unable to build a model: " + str(err)) from err
    ...
    def query(self, name, data, data_type, **kwargs):
        ...
        """
        try:
            # Treat the list aside, since we don't want to encode the div
            # separator
    ...
    def serialize_data(self, data, data_type, **kwargs):
        ...
        try:
            ...
            enum_type = self.dependencies.get(
                data_type, cast(type, data.__class__))
    ...
    def serialize_iter(self, data, iter_type, div=None, **kwargs):
        ...
        if kwargs.get("do_quote", False):
            serialized = [
                "" if s is None else quote(
                    str(s), safe="") for s in serialized]
        ...
        if "xml" in serialization_ctxt or is_xml:
            ...
            # Create a wrap node if necessary (use the fact that Element and
            # list have "append")
    ...
    def serialize_rfc(attr, **kwargs):  # pylint: disable=unused-argument
        ...
        try:
            ...
            raise TypeError(
                "RFC1123 object must be valid Datetime object.") from exc
    ...
    def serialize_unix(attr, **kwargs):  # pylint: disable=unused-argument
        ...
        try:
            ...
            raise TypeError(
                "Unix time object must be valid Datetime object.") from exc
...
def rest_key_extractor(attr, attr_desc, data):  # pylint: disable=unused-argument
    ...
    while "." in key:
        # Need the cast, as for some reasons "split" is typed as list[str |
        # Any]
...
def rest_key_case_insensitive_extractor(  # pylint: disable=unused-argument, inconsistent-return-statements
    ...
    if working_data:
        return attribute_key_case_insensitive_extractor(
            key, None, working_data)
...
def xml_key_extractor(attr, attr_desc, data):  # pylint: disable=unused-argument,too-many-return-statements
    ...
    if is_wrapped or (
        internal_type and (
            issubclass(
            internal_type,
            Enum) or "name" not in internal_type_xml_map)):
...
class Deserializer:
    ...
    def _build_additional_properties(self, attribute_map, data):
        if not self.additional_properties_detection:
            return None
        if "additional_properties" in attribute_map and attribute_map.get(
                "additional_properties", {}).get("key") != "":
            # Check empty string. If it's not empty, someone has a real
            # "additionalProperties"
            return None
    ...
    def _unpack_content(raw_data, content_type=None):
        ...
        """
        # Assume this is enough to detect a Pipeline Response without importing
        # it
        ...
        # Assume this is enough to recognize universal_http.ClientResponse
        # without importing it
        if hasattr(raw_data, "body"):
            return RawDeserializer.deserialize_from_http_generics(
                raw_data.text(), raw_data.headers)

        # Assume this enough to recognize requests.Response without importing
        # it.
        ...
        if isinstance(raw_data, (str, bytes)) or hasattr(raw_data, "read"):
            return RawDeserializer.deserialize_from_text(
                raw_data, content_type)  # type: ignore
        return raw_data
    ...
    def _instantiate_model(self, response, attrs, additional_properties=None):
        ...
        if callable(response):
            ...
            try:
                ...
                kwargs = {
                    k: v for k,
                    v in attrs.items() if k not in subtype and k not in readonly +
                    const}
    ...
    def deserialize_data(self, data, data_type):  # pylint: disable=too-many-return-statements
        ...
        try:
            ...
            if data_type in self.deserialize_type:
                if isinstance(
                    data,
                    self.deserialize_expected_types.get(
                        data_type,
                        tuple())):
                    return data
    ...
    def deserialize_iter(self, attr, iter_type):
        ...
        if isinstance(
                attr,
                ET.Element):  # If I receive an element here, get the children
            attr = list(attr)
    ...
    def deserialize_dict(self, attr, dict_type):
        ...
        if isinstance(attr, list):
            return {
                x["key"]: self.deserialize_data(
                    x["value"],
                    dict_type) for x in attr}

        if isinstance(attr, ET.Element):
            # Transform <Key>value</Key> into {"Key": "value"}
            attr = {el.tag: el.text for el in attr}
        return {k: self.deserialize_data(v, dict_type)
    ...
    def deserialize_enum(data, enum_obj):
        ...
        if isinstance(data, int):
            ...
            try:
                ...
                raise DeserializationError(
                    error.format(data, enum_obj)) from exc
        try:
            ...
            for enum_value in enum_obj:
                if enum_value.value.lower() == str(data).lower():
                    return enum_value
            # We don't fail anymore for unknown value, we deserialize as a
            # string
            _LOGGER.warning(
    ...
    def deserialize_date(attr):
        ...
        if re.search(r"[^\W\d_]", attr, re.I + re.U):  # type: ignore
            raise DeserializationError(
                ...
                attr)
        # This must NOT use defaultmonth/defaultday. Using None ensure this
        # raises an exception.
