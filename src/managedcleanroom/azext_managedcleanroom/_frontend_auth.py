...
def get_frontend_client(cmd, endpoint=None):
    ...
    logger.debug(f"Creating Analytics Frontend API client for endpoint: {api_endpoint}")
    
    # Check if this is a local development endpoint
    is_local = api_endpoint.startswith('http://localhost') or api_endpoint.startswith('http://127.0.0.1')
    ...
    if is_local:
        # For local development, create a custom auth policy that bypasses HTTPS check
        class LocalBearerTokenPolicy(SansIOHTTPPolicy):
            ...
            def on_request(self, request):
                ...
                # The token might be a tuple ('Bearer', 'token_string') or just the token string
                ...
                logger.debug(f"Setting Authorization header: Bearer {token_string[:50]}...")
