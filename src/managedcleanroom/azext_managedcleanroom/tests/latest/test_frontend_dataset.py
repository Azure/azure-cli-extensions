...
class TestFrontendDataset(unittest.TestCase):
    ...
    def test_list_datasets_success(self, mock_get_client):
        ...
        mock_client.collaboration.analytics_datasets_list_get.assert_called_once_with("test-collab-123")
    ...
    def test_publish_dataset_failure(self, mock_get_client):
        ...
        # Mock error
        mock_client = Mock()
        mock_client.collaboration.analytics_dataset_id_publish_post.side_effect = Exception("Dataset validation failed")
        mock_get_client.return_value = mock_client
