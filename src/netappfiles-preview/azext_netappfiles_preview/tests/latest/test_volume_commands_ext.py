...
class AzureNetAppFilesExtVolumeServiceScenarioTest(ScenarioTest):
    ...
    def create_volume(self, account_name, pool_name, volume_name1, rg, tags=None, volume_name2=None):
        ...
        if volume_name2:
            ...
            self.cmd("az netappfiles volume create -g %s -a %s -p %s -v %s -l 'westus2' %s --creation-token %s --subnet-id %s --tags '%s'" % (rg, account_name, pool_name, volume_name2, VOLUME_DEFAULT, creation_token, subnet_id, tags)).get_output_in_json()
