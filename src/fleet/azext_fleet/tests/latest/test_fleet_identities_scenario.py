...
class FleetIdentitiesScenarioTest(ScenarioTest):
    ...
    def test_fleet_identities(self):
        ...
        # add a user-assigned identity to the fleet.
        identity = self.cmd('identity create -n {identity_name} -g {rg}', checks=[self.check('name', '{identity_name}')]).get_output_in_json()
        ...
        self.cmd('identity delete -g {rg} -n {fleet_name}')
