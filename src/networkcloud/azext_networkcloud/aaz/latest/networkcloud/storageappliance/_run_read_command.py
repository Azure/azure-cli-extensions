...
@register_command(
    "networkcloud storageappliance run-read-command",
    is_preview=True,
)
class RunReadCommand(AAZCommand):
    """Run and retrieve output from read only commands on the provided storage appliance.

    :example: Run and retrieve output from read only commands on storage appliance.
        az networkcloud storageappliance run-read-command --resource-group "resourceGroupName" --name "storageApplianceName" --commands "[{command:purealert,arguments:[list,'--filter',state='/open/']}]" --limit-time-seconds 60
    """
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        # define Arg Group "StorageApplianceRunReadCommandsParameters"

        _args_schema = cls._args_schema
        _args_schema.commands = AAZListArg(
            options=["--commands"],
            arg_group="StorageApplianceRunReadCommandsParameters",
            help="The list of read-only commands to run.",
            required=True,
        ...
        _element.command = AAZStrArg(
            options=["command"],
            help="The read-only command to execute against the storage appliance.",
            required=True,
        )
    ...
    def _execute_operations(self):
        self.pre_operations()
        yield self.StorageAppliancesRunReadCommands(ctx=self.ctx)()
    ...
    class StorageAppliancesRunReadCommands(AAZHttpOperation):
