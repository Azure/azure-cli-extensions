...
@register_command(
    "networkcloud baremetalmachine validate-hardware",
    is_experimental=True,
)
class ValidateHardware(AAZCommand):
    """Validate the hardware of the provided bare metal machine.

    :example: Validate the bare metal machine hardware
        az networkcloud baremetalmachine validate-hardware --bare-metal-machine-name "bareMetalMachineName" --resource-group "resourceGroupName" --validation-category "validation-category"
    """
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        # define Arg Group "BareMetalMachineValidateHardwareParameters"

        _args_schema = cls._args_schema
        _args_schema.validation_category = AAZStrArg(
            options=["--validation-category"],
            arg_group="BareMetalMachineValidateHardwareParameters",
            help="The category of hardware validation to perform.",
            required=True,
            enum={"BasicValidation": "BasicValidation"},
        )
        return cls._args_schema

    def _execute_operations(self):
        self.pre_operations()
        yield self.BareMetalMachinesValidateHardware(ctx=self.ctx)()
    ...
    class BareMetalMachinesValidateHardware(AAZHttpOperation):
        ...
        def content(self):
            ...
            _builder.set_prop("validationCategory", AAZStrType, ".validation_category", typ_kwargs={"flags": {"required": True}})
        ...
        def _build_schema_on_200(cls):
            ...
            _ValidateHardwareHelper._build_schema_operation_status_result_read(cls._schema_on_200)
...
class _ValidateHardwareHelper:
    """Helper class for ValidateHardware"""
