...
def step_create(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd('az networkcloud clustermanager create --name {name} '
...
def step_delete(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd('az networkcloud clustermanager delete --name {name} '
             '--resource-group {rg} -y', checks=checks)
...
class ClusterManagerScenarioTest(ScenarioTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.kwargs.update({
            ...
            'mrg_name': self.create_random_name(prefix="cli-test-cm-mrg-", length=24),
            'location': CONFIG.get('CLUSTER_MANAGER', 'location'),
            'analyticsWorkspaceId': CONFIG.get('CLUSTER_MANAGER', 'analytics_workspace_id'),
            'fabricControllerId': CONFIG.get('CLUSTER_MANAGER', 'fabric_controller_id'),
