...
def step_create(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd('az networkcloud virtualmachine console create '
...
def step_update(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd('az networkcloud virtualmachine console update '
...
def step_show(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd('az networkcloud virtualmachine console show '
             '--resource-group {resourceGroup} --virtual-machine-name {virtualMachineName}',
             checks=checks)
...
def step_list(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd('az networkcloud virtualmachine console list '
             '--resource-group {resourceGroup} --virtual-machine-name {virtualMachineName}', checks=checks)
...
def step_delete(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd('az networkcloud virtualmachine console delete --resource-group {resourceGroup} '
             '--virtual-machine-name {virtualMachineName} --yes', checks=checks)
...
class VirtualMachineConsoleScenarioTest(ScenarioTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.kwargs.update({
            ...
            'extendedLocation': CONFIG.get('VIRTUALMACHINE_CONSOLE', 'extended_location'),
            ...
            'virtualMachineName': CONFIG.get('VIRTUALMACHINE_CONSOLE', 'virtual_machine_name'),
        })
