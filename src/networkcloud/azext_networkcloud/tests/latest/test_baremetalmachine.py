...
def step_show(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine show --name {nameCrud} --resource-group {rgCrud}')
...
def step_run_command(test, checks=None):
    ...
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine run-command --name {nameRunCommand} --resource-group {rgCommands} --arguments {runCommandArguments} --limit-time-seconds {limitTimeSeconds} --script {script}')

def step_run_data_extract(test, checks=None):
    '''BareMetalMachine run data extract operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine run-data-extract --name {nameRunDataExtract} --resource-group {rgCommands} --limit-time-seconds {limitTimeSeconds} --commands {dataExtractCommands}')

def step_run_read_command(test, checks=None):
    '''BareMetalMachine run read command operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine run-read-command --name {nameRunReadCommand} --resource-group {rgCommands} --limit-time-seconds {limitTimeSeconds} --commands {runReadCommands}')

def step_cordon(test, checks=None):
    '''BareMetalMachine cordon operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine cordon --evacuate {cordonEvacuate} --name {nameCordon} --resource-group {rgCordon}')

def step_uncordon(test, checks=None):
    '''BareMetalMachine uncordon operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine uncordon --name {nameCordon} --resource-group {rgCordon}')

def step_restart(test, checks=None):
    '''BareMetalMachine restart operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine restart --name {nameRestart} --resource-group {rgPower}')

def step_power_off(test, checks=None):
    '''BareMetalMachine power off operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine power-off --name {namePower} --skip-shutdown {skipShutdown} --resource-group {rgPower}')

def step_start(test, checks=None):
    '''BareMetalMachine start operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine start --name {namePower} --resource-group {rgPower}')

def step_reimage(test, checks=None):
    '''BareMetalMachine reimage operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine reimage --name {nameReimage} --resource-group {rgValidate}')

def step_replace(test, checks=None):
    '''BareMetalMachine replace operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine replace --name {nameReplace} --resource-group {rgReplace} --bmc-credentials {bmcCreds} --bmc-mac-address {bmcMacAddress} --boot-mac-address {bootMacAddress} --machine-name {newBmmName} --serial-number {serialNumber}')

def step_validate_hardware(test, checks=None):
    '''BareMetalMachine validate hardware operation'''
    if checks is None:
        checks = []
    test.cmd('az networkcloud baremetalmachine validate-hardware --name {nameValidate} --resource-group {rgValidate} --validation-category {validationCategory}')
