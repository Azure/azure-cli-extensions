...
1.0.0b1
+++++++
* Initial preview release. Supports the ``Microsoft.CloudHealth`` resource provider at API version ``2026-01-01-preview``.
...
* Adds ``monitor health-models entity`` commands for entities (nodes in the graph): ``create``, ``show``, ``list``, ``update``, ``delete``, ``wait``, plus ``get-history``, ``get-signal-history``, and ``ingest-health-report``.
