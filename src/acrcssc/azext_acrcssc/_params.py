...
def load_arguments(self: AzCommandsLoader, _):
    ...
    with self.argument_context("acr supply-chain workflow") as c:
        ...
        c.argument("type", arg_type=get_enum_type(CSSCTaskTypes), options_list=['--type', '-t'], help='Allowed values: ContinuousPatchV1')
        c.argument('cmd', options_list=['--__cmd__'], required=False )
