...
from .example_steps import step_cluster_create_identity
...
from .example_steps import step_arc_setting_update
from .example_steps import step_arc_setting_create_identity
from .example_steps import step_arc_setting_generate_password
...
def call_scenario(test):
    ...
    step_cluster_create_identity(test, checks=[])
    step_arc_setting_create(test, checks=[])
    step_arc_setting_show(test, checks=[])
    step_arc_setting_list(test, checks=[
        test.check('length(@)', 1),
    ])
    step_arc_setting_update(test, checks=[
        test.check("connectivityProperties.enabled", True),
    ])
    step_arc_setting_create_identity(test, checks=[])
    step_arc_setting_generate_password(test, checks=[])
    step_extension_create(test, checks=[
        test.check("publisher", "Microsoft.Compute", case_sensitive=False),
        test.check("settings.workspaceId", "xx", case_sensitive=False),
