...
@try_manual
def step__dedicatedhsm_put(test, rg):
...
@try_manual
def step__dedicatedhsm_get(test, rg):
...
@try_manual
def step__dedicatedhsm_get2(test, rg):
    test.cmd('az hardware-security-modules dedicated-hsm list '
             '-g ""',
             checks=[])
...
@try_manual
def step__dedicatedhsm_patch(test, rg):
...
def call_scenario(test, rg):
    setup(test, rg)
    step__dedicatedhsm_put(test, rg)
    step__dedicatedhsm_get_get_a_dedicated_hsm(test, rg)
    step__dedicatedhsm_get(test, rg)
    step__dedicatedhsm_get2(test, rg)
    step__dedicatedhsm_patch(test, rg)
...
class AzureDedicatedHSMResourceProviderScenarioTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(name_prefix='clitesthardware_security_modules_hsm-group'[:7], key='rg',
                           parameter_name='rg')
    @VirtualNetworkPreparer(name_prefix='clitesthardware_security_modules_stamp01'[:7], key='vn', resource_group_key=''
                            'rg')
