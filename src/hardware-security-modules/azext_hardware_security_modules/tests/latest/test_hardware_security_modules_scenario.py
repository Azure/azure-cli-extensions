...
class HardwareSecurityModulesScenario(ScenarioTest):
    ...
    @ResourceGroupPreparer(name_prefix='cli_test_hardware_security_modules_v2', location='westus')
    def test_hardware_security_modules_v2(self, resource_group):
        ...
        self.cmd('network vnet create --name {vnet} --resource-group {rg} --address-prefix 10.3.0.0/16 '
                 '--subnet-name compute --subnet-prefix 10.3.0.0/24')
        self.cmd('network vnet subnet create --vnet-name {vnet} --resource-group {rg} --name {subnet1} '
                 '--address-prefixes 10.3.1.0/24 --delegations Microsoft.HardwareSecurityModules/dedicatedHSMs')
        # self.cmd('network vnet subnet create --vnet-name {vnet} --resource-group {rg} --name {subnet2} '
        # '--address-prefixes 10.3.2.0/24 --delegations Microsoft.HardwareSecurityModules/dedicatedHSMs')
        self.cmd('network vnet subnet create --vnet-name {vnet} --resource-group {rg} --name {subnet3} '
        ...
        self.cmd('dedicated-hsm create '
                 '--name hsm1 '
                 '--location westus '
                 '--management-network-profile-interfaces private-ip-address="10.3.1.5" '
                 '--management-network-profile-subnet '
                 'id=/subscriptions/{subscription_id}/resourceGroups/{rg}/providers/Microsoft.Network/virtualNetworks/{vnet}/subnets/{subnet1} '
                 '--stamp-id "stamp1" '
