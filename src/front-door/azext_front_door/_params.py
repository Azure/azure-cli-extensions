...
def load_arguments(self, _):
    ...
    for scope in ['backend-pool', 'backend-pool backend']:
        ...
        with self.argument_context('network front-door {}'.format(scope), arg_group=arg_group) as c:
            ...
            c.argument('private_link_resource_id', options_list='--resource-id', help='The Resource Id of the Private Link. Populating this optional field indicates that this backend is \'Private\'.')
            ...
            c.argument('private_link_approval_message', options_list='--approval-message', help='A custom message to be included in the approval request to connect to the Private Link.')
            c.argument('backend_host_header', help='Host header sent to the backend.')
