...
# For db(mysqlFlex/psql/psqlFlex/sql/fabricSql) linker with auth type=systemAssignedIdentity, enable Microsoft Entra auth and create db user on data plane
...
def get_enable_mi_for_db_linker_func(yes=False, new=False):
    def enable_mi_for_db_linker(cmd, source_id, target_id, auth_info, client_type, connection_name,
        ...
        target_handler = getTargetHandler(
            cmd, target_id, target_type, auth_info, client_type, connection_name,
            connstr_props,
            skip_prompt=yes, new_user=new)
...
def getTargetHandler(cmd, target_id, target_type, auth_info, client_type, connection_name,
                     connstr_props,
                     skip_prompt, new_user):
    ...
    if target_type in {RESOURCE.FabricSql}:
        return FabricSqlHandler(cmd, target_id, target_type, auth_info, connection_name,
                                connstr_props, skip_prompt, new_user)
    return None
