...
def configure_common_settings(cli_ctx, client):
    ...
    try:
        ...
        command_ext_version = cli_ctx.data['command_extension_version']
        if command_ext_name:
            client.config.add_user_agent(" CliExtension/{}({})".format(command_ext_name, command_ext_version))
    except KeyError:
        pass
...
def _prepare_client_kwargs_track2(cli_ctx):
    ...
    try:
        ...
        command_ext_version = cli_ctx.data['command_extension_version']
        if command_ext_name:
            client_kwargs['user_agent'] += " CliExtension/{}({})".format(command_ext_name, command_ext_version)
    except KeyError:
        pass
