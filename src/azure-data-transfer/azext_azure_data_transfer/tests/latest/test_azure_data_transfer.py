...
class ConnectionAndFlowOperations(ScenarioTest):
    ...
    @record_only()
    def test_rejected_connection_operations(self):
        self.kwargs.update({
            'rg': 'rpaas-rg',
            'subscriptionId': '389ff96a-b137-405b-a3c8-4d22514708b5',
        ...
        id = self.cmd('az azure-data-transfer connection create --resource-group {rg} --connection-name {rejectedConnection} --direction Receive --location {location} --flow-types ["Mission"] --pipeline {pipeline} --justification required --remote-subscription-id {subscriptionId}  --requirement-id 1234 --primary-contact lasuredd@microsoft.com --secondary-contacts lasuredd@microsoft.com').get_output_in_json().get('id')
        self.kwargs.update({'id': id})

        self.cmd('az azure-data-transfer pipeline reject-connection --resource-group {rg} --pipeline-name {pipeline} --id {id}', checks=[
            self.check('properties.status', 'Rejected'),
        ])

        result = self.cmd('az azure-data-transfer pipeline approve-connection --resource-group {rg} --pipeline-name {pipeline} --id {id}', expect_failure=True)

        self.cmd('az azure-data-transfer connection delete --yes --resource-group {rg} --connection-name {rejectedConnection}')
    ...
    def test_update_connection_operations(self):
        ...
        self.cmd('az azure-data-transfer connection create --resource-group {rg} --connection-name {connectionName} --direction Receive --location {location} --flow-types ["Mission"] --pipeline {pipeline} --justification required --remote-subscription-id {subscriptionId}  --requirement-id 1234 --primary-contact lasuredd@microsoft.com --secondary-contacts lasuredd@microsoft.com').get_output_in_json().get('id')

        self.cmd('az azure-data-transfer connection update --resource-group {rg} --connection-name {connectionName} --tags testAutomation=true key2=value2', checks=[
            self.check('tags.testAutomation', 'true'),
        ])

        self.cmd('az azure-data-transfer connection delete --yes --resource-group {rg} --connection-name {connectionName}')
    ...
    def test_list_operations_with_pagination(self):
        ...
        self.cmd('az azure-data-transfer connection create --resource-group {rg} --connection-name {sendConnection} --direction Send --location {location} --flow-types ["Mission"] --pipeline {pipeline} --primary-contact lasuredd@microsoft.com --secondary-contacts lasuredd@microsoft.com --pin 12345')

        count = len(self.cmd('az azure-data-transfer connection list --resource-group {rg} --max-items 3').get_output_in_json())
        ...
        for _ in range(3):
            ...
            self.cmd('az azure-data-transfer connection flow create --resource-group {rg} --connection-name {sendConnection} --name {name} --flow-type "Mission" --location {location} --status "Enabled" --storage-account {storageAccountId} --storage-container-name {storageContainer} --data-type "Blob"')
    ...
    def test_enable_disable_flow_operations(self):
        ...
        self.cmd('az azure-data-transfer connection flow disable --resource-group {rg} --connection-name {sendConnection} --name {flowName}', checks=[
        ...
        self.cmd('az azure-data-transfer connection flow enable --resource-group {rg} --connection-name {sendConnection} --name {flowName}',  checks=[
            self.check('properties.status', 'Enabled'),
        ])
        
        self.cmd('az azure-data-transfer connection flow update --resource-group {rg} --name {flowName} --connection-name {sendConnection} --tags testAutomation=true flowUpdate=value', checks=[
            self.check('tags.testAutomation', 'true'),
        ])
    ...
    def test_connection_flow_link_operations(self):
        ...
        self.cmd('az azure-data-transfer connection flow create --resource-group {rg} --connection-name {receiveConnection} --name {receiveFlow} --flow-type "Mission" --location {location} --status "Enabled" --storage-account {storageAccountId} --storage-container-name {storageContainer} --data-type "Blob"', expect_failure=True)
        
        self.cmd('az azure-data-transfer pipeline approve-connection --resource-group {rg} --pipeline-name {pipeline} --id {id}')
        ...
        flowId = self.cmd('az azure-data-transfer connection flow create --resource-group {rg} --connection-name {receiveConnection} --name {receiveFlow} --flow-type "Mission" --location {location} --status "Enabled" --storage-account {storageAccountId} --storage-container-name {storageContainer} --data-type "Blob"').get_output_in_json().get('id')
        self.kwargs.update({'flowId': flowId})

        sendFlowId = self.cmd('az azure-data-transfer connection flow create --resource-group {rg} --connection-name {sendConnection} --name {sendFlow} --flow-type "Mission" --location {location} --status "Enabled" --storage-account {storageAccountId} --storage-container-name {storageContainer} --data-type "Blob"').get_output_in_json().get('id')
