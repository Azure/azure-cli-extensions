...
def load_arguments(self, _):
    ...
    # Telemetry 
    with self.argument_context('containerapp env', arg_group='Telemetry') as c:
        c.argument('app_insights_connection_string', options_list=['--app-insights-connection-string'], help='Application Insights connection string used by container apps environment', is_preview=True)

    with self.argument_context('containerapp env', arg_group='Telemetry') as c:
        c.argument('open_telemetry_traces_destinations', options_list=['--open-telemetry-traces-destinations', '--otel-traces-destinations'], nargs='+', help='Specify the destinations of open telemetry trace signal', is_preview=True)
    
    with self.argument_context('containerapp env', arg_group='Telemetry') as c:
        c.argument('open_telemetry_logs_destinations', options_list=['--open-telemetry-logs-destinations', '--otel-logs-destinations'], nargs='+', help='Specify the destinations of open telemetry log signal', is_preview=True)

    with self.argument_context('containerapp env', arg_group='Telemetry') as c:
        c.argument('open_telemetry_metrics_destinations', options_list=['--open-telemetry-metrics-destinations', '--otel-metrics-destinations'], nargs='+', help='Specify the destinations of open telemetry metric signal', is_preview=True)

    with self.argument_context('containerapp env', arg_group='Telemetry') as c:
        c.argument('open_telemetry_include_system_telemetry', options_list=['--open-telemetry-include-system-telemetry', '--otel-include-system-telemetry'], help='Include telemetry data generated by system components', is_preview=True)

    with self.argument_context('containerapp env', arg_group='Telemetry') as c:
        c.argument('open_telemetry_dataDog_site', options_list=['--open-telemetry-dataDog-site', '--otel-dataDog-site'], help='Specify the data dog site', is_preview=True)

    with self.argument_context('containerapp env', arg_group='Telemetry') as c:
        c.argument('open_telemetry_dataDog_key', options_list=['--open-telemetry-dataDog-key', '--otel-dataDog-key'], help='Specify the data dog api key', is_preview=True)
