...
class ContainerappOtherPropertyTests(ScenarioTest):
    ...
    def test_containerapp_kind_functionapp(self, resource_group):
        ...
        self.cmd(f'containerapp create -g {resource_group} -n {app} --image {image} --ingress external --target-port 80 --environment {env} --cpu 0.5 --memory 1Gi')
        self.cmd(f'containerapp show -g {resource_group} -n {app}', checks=[JMESPathCheck("kind", None)])

        self.cmd(f'containerapp update -g {resource_group} -n {app} --cpu 0.5 --memory 1Gi --kind functionapp')
        self.cmd(f'containerapp show -g {resource_group} -n {app}', checks=[JMESPathCheck("kind", "functionapp")])

        self.cmd(f'containerapp create -g {resource_group} -n {app} --image {image} --ingress external --target-port 80 --environment {env} --kind functionapp')
        self.cmd(f'containerapp show -g {resource_group} -n {app}', checks=[JMESPathCheck("kind", "functionapp")])
