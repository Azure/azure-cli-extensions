...
def prepare_containerapp_env_for_app_e2e_tests(test_cls, location=TEST_LOCATION):
    ...
    rg_name = f'client.env_rg_{location}'.lower().replace(
        " ", "").replace("(", "").replace(")", "")
    env_name = f'env-{location}'.lower().replace(" ",
                                                 "").replace("(", "").replace(")", "")
    managed_env = None
    try:
        managed_env = test_cls.cmd(
            'containerapp env show -g {} -n {}'.format(rg_name, env_name)).get_output_in_json()
        ...
        if e.error_msg.__contains__('ResourceGroupNotFound') or e.error_msg.__contains__('ResourceNotFound'):
            ...
            test_cls.cmd(
                f'containerapp env create -g {rg_name} -n {env_name} --logs-destination none')
            managed_env = test_cls.cmd(
                'containerapp env show -g {} -n {}'.format(rg_name, env_name)).get_output_in_json()
...
def create_containerapp_env(test_cls, env_name, resource_group, location=None):
    logs_workspace_name = test_cls.create_random_name(
    ...
    logs_workspace_id = test_cls.cmd('monitor log-analytics workspace create -g {} -n {} -l {}'.format(
        resource_group, logs_workspace_name, logs_workspace_location)).get_output_in_json()["customerId"]
    logs_workspace_key = test_cls.cmd('monitor log-analytics workspace get-shared-keys -g {} -n {}'.format(
        resource_group, logs_workspace_name)).get_output_in_json()["primarySharedKey"]

    if location:
        test_cls.cmd(
            f'containerapp env create -g {resource_group} -n {env_name} --logs-workspace-id {logs_workspace_id} --logs-workspace-key {logs_workspace_key} -l {location}')
    else:
        test_cls.cmd(
            f'containerapp env create -g {resource_group} -n {env_name} --logs-workspace-id {logs_workspace_id} --logs-workspace-key {logs_workspace_key}')

    containerapp_env = test_cls.cmd(
...
def create_and_verify_containerapp_up(
    ...
    if app_name is None:
        ...
        app_name = test_cls.create_random_name(
    ...
    if requires_acr_prerequisite:
        # Create ACR
        registry_name = test_cls.create_random_name(
            prefix='containerapp', length=24)
        acr = test_cls.cmd('acr create -g {} -n {} --sku Basic --admin-enabled'.format(
            resource_group, registry_name)).get_output_in_json()
        registry_server = acr["loginServer"]
        acr_credentials = test_cls.cmd(
...
def create_and_verify_containerapp_up_with_multiple_environments(
    ...
    test_cls.cmd(
        f'containerapp env create -g {resource_group} -n {first_env_name} -l {location}')

    second_env_name = test_cls.create_random_name(prefix='env', length=24)
    test_cls.cmd(
        f'containerapp env create -g {resource_group} -n {second_env_name} -l {location}')
    ...
    app = test_cls.cmd(
        f"containerapp show -g {resource_group} -n {app_name}").get_output_in_json()
    url = app["properties"]["configuration"]["ingress"]["fqdn"]
    containerapp_env = test_cls.cmd('containerapp env show -g {} -n {}'.format(
        resource_group, second_env_name)).get_output_in_json()
    test_cls.assertEqual(
        app["properties"]["environmentId"], containerapp_env["id"])
    url = url if url.startswith("http") else f"http://{url}"
    resp = requests.get(url)
    test_cls.assertTrue(resp.ok)
    ...
    test_cls.cmd(
        'containerapp delete -g {} -n {} --yes --no-wait'.format(resource_group, app_name))
    test_cls.cmd(
        'containerapp env delete -g {} -n {} --yes --no-wait'.format(resource_group, first_env_name))


def create_extension_and_custom_location(test_cls, resource_group, connected_cluster_name, custom_location_name):
    try:
        connected_cluster = test_cls.cmd(
        ...
        test_cls.cmd(
            f'az customlocation create -g {resource_group} -n {custom_location_name} -l {location} --host-resource-id {connected_cluster_id} --namespace appplat-ns -c {extension["id"]}')
    except:
        pass
...
def verify_containerapp_create_exception(
    ...
    test_cls.cmd(
        'containerapp delete -g {} -n {} --yes'.format(resource_group, app_name))
...
def create_and_verify_containerapp_create_and_update(
    ...
    app = test_cls.cmd(f"containerapp show -g {resource_group} -n {app_name}", checks=[
        ...
        JMESPathCheck(
            'properties.configuration.registries[0].username', registry_user),
        JMESPathCheck('properties.provisioningState', 'Succeeded'),
    ]).get_output_in_json()
    test_cls.assertEqual(app["properties"]["template"]
                         ["containers"][0]["image"].split(":")[0], image_name)
    ...
    test_cls.assertEqual(app["properties"]["template"]
                         ["containers"][0]["image"].split(":")[0], image_name)
    test_cls.assertNotEqual(
        app["properties"]["template"]["containers"][0]["image"], old_image)
