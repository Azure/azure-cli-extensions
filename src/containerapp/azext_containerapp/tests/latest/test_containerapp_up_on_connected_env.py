...
class ContainerAppUpConnectedEnvImageTest(ScenarioTest):
    ...
    def test_containerapp_up_on_arc_e2e(self, resource_group, connected_cluster_name):
        ...
        custom_location_name = self.create_random_name(
            prefix="my-custom-location", length=24)
        ...
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --connected-cluster-id {connected_cluster_id} --image {image}')
        env_list = self.cmd(
            f'containerapp connected-env list -g {resource_group}').get_output_in_json()
        self.assertEqual(env_list[0]["extendedLocation"]
                         ["name"].lower(), custom_location_id.lower())
        if format_location(connected_cluster["location"]) == format_location('eastus2euap'):
            self.assertEqual(format_location(
                env_list[0]["location"]), format_location(TEST_LOCATION))
        else:
            self.assertEqual(format_location(env_list[0]["location"]), format_location(
                connected_cluster["location"]))
        env_id = env_list[0]["id"]
        env_name = env_list[0]["name"]
        self._validate_app(resource_group, app_name,
                           custom_location_id, env_id)

        # -n {appname}
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --image {image}', expect_failure=False)
        self._validate_app(resource_group, app_name,
                           custom_location_id, env_id)
    ...
    def test_containerapp_up_on_arc_auto_install_extension_e2e(self, resource_group, connected_cluster_name):
        ...
        # -n {appname} --connected-cluster-id
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --connected-cluster-id {connected_cluster_id} --image {image}', expect_failure=False)

        extension_type = 'microsoft.app.environment'
        installed_exts = self.cmd(
            f'k8s-extension list -c {connected_cluster_name} -g {resource_group} --cluster-type connectedClusters').get_output_in_json()
        ...
        app_name = 'mycontainerapp2'
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --environment {env_id} --image {image}', expect_failure=False)
        self._validate_app(resource_group, app_name,
                           custom_location_id, env_id)

        # -n {appname} --environment {env_id}
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --environment {env_name} --image {image}', expect_failure=False)
        self._validate_app(resource_group, app_name,
                           custom_location_id, env_id)

        # -n {appname} --connected-cluster-id --environment {env_name}
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --connected-cluster-id {connected_cluster_id} --environment {env_id} --image {image}', expect_failure=False)
        self._validate_app(resource_group, app_name,
                           custom_location_id, env_id)

        # -n {appname} --connected-cluster-id --environment {env_id}
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --connected-cluster-id {connected_cluster_id} --environment {env_name} --image {image}', expect_failure=False)
        self._validate_app(resource_group, app_name,
                           custom_location_id, env_id)
        ...
        app_name = 'mycontainerapp3'
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --custom-location {custom_location_id} --image {image}', expect_failure=False)
        self._validate_app(resource_group, app_name,
                           custom_location_id, env_id)

        # -n {appname} --custom-location --environment {env_id}
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --custom-location {custom_location_id} --environment {env_id} --image {image}', expect_failure=False)
        self._validate_app(resource_group, app_name,
                           custom_location_id, env_id)

        # -n {appname} --custom-location --environment {env_name}
        self.cmd(
            f'containerapp up -n {app_name} -g {resource_group} --custom-location {custom_location_id} --environment {env_name} --image {image}', expect_failure=False)
        self._validate_app(resource_group, app_name,
                           custom_location_id, env_id)
    ...
    def _validate_app(self, resource_group, app_name, custom_location_id, environment_id):
        ...
        self.assertEqual(app["extendedLocation"]
                         ["name"].lower(), custom_location_id.lower())
        self.assertEqual(app["properties"]
                         ["environmentId"].lower(), environment_id.lower())
