...
class ContainerAppWorkloadProfilesTest(ScenarioTest):
    ...
    def test_containerapp_env_workload_profiles_e2e(self, resource_group):
        ...
        self.cmd('containerapp env show -n {} -g {}'.format(env, resource_group), checks=[
            ...
            JMESPathCheck(
                'properties.workloadProfiles[0].name', "Consumption", case_sensitive=False),
            JMESPathCheck(
                'properties.workloadProfiles[0].workloadProfileType', "Consumption", case_sensitive=False),
        ])

        self.cmd(
            "az containerapp env workload-profile list-supported -l {}".format(location))

        profiles = self.cmd("az containerapp env workload-profile list -g {} -n {}".format(
            resource_group, env)).get_output_in_json()
        self.assertEqual(len(profiles), 1)
        self.assertEqual(profiles[0]["properties"]
                         ["name"].lower(), "consumption")
        self.assertEqual(profiles[0]["properties"]
                         ["workloadProfileType"].lower(), "consumption")
    ...
    def test_containerapp_env_workload_profiles_delete(self, resource_group):
        ...
        self.cmd("az network vnet create -l {} --address-prefixes '14.0.0.0/16' -g {} -n {}".format(
            location, resource_group, vnet))
    ...
    def test_containerapp_create_enable_workload_profiles_three_state_flag(self, resource_group):
        ...
        self.cmd('containerapp env delete -g {} -n {} --yes --no-wait'.format(
            resource_group, env1), expect_failure=False)
    ...
    def test_containerapp_create_with_workloadprofile_yaml(self, resource_group):
        ...
        self.assertContainerappProperties(
            containerapp_env, resource_group, app, workload_profile_name, revision_01, "10.0", "5Gi")

        revision_02 = "revision02"
        containerapp_yaml_text_02 = containerapp_yaml_text_01.replace(
            workload_profile_name, "Consumption")
        containerapp_yaml_text_02 = containerapp_yaml_text_02.replace(
            "cpu: 10.0", "cpu: 0.5")
        containerapp_yaml_text_02 = containerapp_yaml_text_02.replace(
        ...
        self.cmd(
            f'containerapp update -n {app} -g {resource_group} --yaml {containerapp_file_name_02}')
        ...
        self.assertContainerappProperties(
            containerapp_env, resource_group, app, "Consumption", revision_02, "0.5", "1Gi")
    ...
    def assertContainerappProperties(self, containerapp_env, rg, app, workload_profile_name, revision, cpu, mem):
        self.cmd(f'containerapp show -g {rg} -n {app}', checks=[
            ...
            JMESPathCheck("properties.workloadProfileName",
                          workload_profile_name),
            JMESPathCheck(
                "properties.template.containers[0].resources.cpu", cpu),
            JMESPathCheck(
                "properties.template.containers[0].resources.memory", mem)
        ])
    ...
    def test_containerapp_create_enable_dedicated_gpu(self, resource_group):
        ...
        self.cmd('containerapp env create -g {} -n {} --logs-destination none --enable-dedicated-gpu'.format(
            ...
            JMESPathCheck(
                'properties.workloadProfiles[0].workloadProfileType', "Consumption", case_sensitive=False),
            JMESPathCheck(
                'properties.workloadProfiles[1].name', gpu_default_name, case_sensitive=False),
            JMESPathCheck(
                'properties.workloadProfiles[1].workloadProfileType', gpu_default_type, case_sensitive=False),
        ...
        self.cmd(f'containerapp create -n {app1} -g {resource_group} --image mcr.microsoft.com/azuredocs/samples-tf-mnist-demo:gpu --environment {env} -w {gpu_default_name} --min-replicas 1 --cpu 0.1 --memory 0.1', checks=[
            ...
            JMESPathCheck(
                'properties.template.containers[0].resources.cpu', '0.1'),
            JMESPathCheck(
                'properties.template.containers[0].resources.memory', '0.1Gi'),
            JMESPathCheck('properties.template.scale.minReplicas', '1'),
