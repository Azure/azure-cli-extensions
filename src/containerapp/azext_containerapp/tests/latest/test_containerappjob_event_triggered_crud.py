...
class ContainerAppJobsCRUDOperationsTest(ScenarioTest):
    ...
    def test_containerapp_eventjob_crudoperations_e2e(self, resource_group):
        ...
        self.cmd("az containerapp job show --resource-group {} --name {}".format(resource_group, job), checks=[
            ...
            JMESPathCheck(
                'properties.configuration.eventTriggerConfig.scale.rules[0].metadata.queueLength', "1"),
        ])
        ...
        self.cmd("az containerapp job show --resource-group {} --name {}".format(resource_group, job), checks=[
            ...
            JMESPathCheck('properties.configuration.triggerType',
                          "event", case_sensitive=False),
            JMESPathCheck(
                'properties.configuration.eventTriggerConfig.scale.maxExecutions', 9),
            JMESPathCheck(
