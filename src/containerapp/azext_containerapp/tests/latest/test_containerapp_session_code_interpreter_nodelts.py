...
class ContainerAppSessionCodeInterperterNodeLTSTests(ScenarioTest):
    ...
    @AllowLargeResponse(8192)
    @ResourceGroupPreparer()
    def test_containerapp_session_code_interpreter_nodelts_e2e_registry(self, resource_group):
        location = TEST_LOCATION
        self.cmd('configure --defaults location={}'.format(location))
        ...
        # Create NodeLTS SessionPool
        sessionpool_name_nodelts = self.create_random_name(prefix='spnodeltsregistry', length=24)
