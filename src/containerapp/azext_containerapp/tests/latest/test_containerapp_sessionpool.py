...
class ContainerappSessionPoolTests(ScenarioTest):
    ...
    def test_containerapp_sessionpool(self, resource_group):
        ...
        sessionpool_name = "spjupyterpython"
        self.cmd('containerapp sessionpool create -g {} -n {}'.format(
            resource_group, sessionpool_name), checks=[
            JMESPathCheck('name', sessionpool_name),
            JMESPathCheck('properties.containerType', "JupyterPython"),
        ...
        # sessionpool_list = self.cmd("containerapp env java-component list -g {} --environment {}".format(resource_group, env_name)).get_output_in_json()
        # self.assertTrue(len(sessionpool_list) == 0)
        #
