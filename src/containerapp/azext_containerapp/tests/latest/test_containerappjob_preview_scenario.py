...
class ContainerappJobPreviewScenarioTest(ScenarioTest):
    ...
    def test_containerappjob_preview_environment_type(self, resource_group, infra_cluster, connected_cluster_name):
        ...
        app2 = self.cmd('containerapp job show -n {} -g {}'.format(ca_name2,
                        resource_group)).get_output_in_json()
        self.cmd('containerapp job delete --ids {} --yes'.format(app2['id']))

        self.cmd(
            'containerapp job delete -n {} -g {} --yes'.format(ca_name, resource_group))
    ...
    def test_containerappjob_preview_e2e(self, resource_group):
        ...
        app = self.cmd(
            'containerapp job show -n {} -g {}'.format(
                ca_name, resource_group),
            checks=[
                JMESPathCheck('properties.environmentId',
                              containerapp_env['id']),
                JMESPathCheck('properties.provisioningState', "Succeeded"),
