...
class SubnetPreparer(NoTrafficRecordingPreparer, SingleValueReplacer):
    def __init__(self, name_prefix='vnet', location='eastus2euap', resource_group_parameter_name='resource_group', vnet_name=None, vnet_address_prefixes='14.0.0.0/23', subnet_address_prefixes='14.0.0.0/23',
                 delegations='Microsoft.App/environments', subnet_name="sub", service_endpoints=None, skip_delete=False):
    ...
    def create_resource(self, name, **kwargs):
        ...
        subnet_id = None
        try:
            self.live_only_execute(self.cli_ctx, f"az network vnet create --address-prefixes {self.vnet_address_prefixes} -g {resource_group} -n {self.vnet_name} --location {self.location}")
            subnet_command = f"az network vnet subnet create --address-prefixes {self.subnet_address_prefixes} " \
