...
class ConnectedClusterPreparer(NoTrafficRecordingPreparer, SingleValueReplacer):
    ...
    def create_resource(self, name, **kwargs):
        ...
        try:
            self.live_only_execute(self.cli_ctx, f'az aks create --resource-group {group} --name {self.infra_cluster} --enable-aad --generate-ssh-keys --enable-cluster-autoscaler --min-count 4 --max-count 10 --node-count 4 --location {self.location}')
            ...
            self.live_only_execute(self.cli_ctx, f'az connectedk8s connect --resource-group {group} --name {self.connected_cluster_name} --location {self.location}')
