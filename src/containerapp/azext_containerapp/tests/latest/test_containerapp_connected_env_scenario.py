...
class ContainerappPreviewScenarioTest(ScenarioTest):
    ...
    def test_containerapp_preview_connected_env_e2e(self, resource_group, connected_cluster_name):
        ...
        self.cmd(
            f'containerapp connected-env delete -g {resource_group} --name {env_name} --yes', expect_failure=True)

        self.cmd(
            'containerapp delete -n {} -g {} --yes'.format(ca_name1, resource_group))

        self.cmd(
            f'containerapp connected-env delete -g {resource_group} --name {env_name} --yes', expect_failure=False)
    ...
    def test_containerapp_preview_connected_env_storage(self, resource_group, connected_cluster_name):
        ...
        self.cmd('containerapp connected-env storage set -g {} -n {} --storage-name {} --azure-file-account-name {} --azure-file-account-key {} --access-mode ReadOnly --azure-file-share-name {}'
                 .format(resource_group, env_name, storage_name, storage_name, storage_keys["value"], shares_name), checks=[
                     ...
                     JMESPathCheck(
                         'properties.azureFile.accountName', storage_name),
                     JMESPathCheck(
                         'properties.azureFile.shareName', shares_name),
                     JMESPathCheck(
                         'properties.azureFile.accessMode', 'ReadOnly'),
                 ])
    ...
    def test_containerapp_preview_connected_env_dapr_components(self, resource_group, connected_cluster_name):
        ...
        containerapp_env = self.cmd('containerapp connected-env show -g {} -n {}'.format(
            resource_group, env_name)).get_output_in_json()
        ...
        self.cmd('containerapp connected-env dapr-component remove -n {} -g {} --dapr-component-name {}'.format(
    ...
    def test_containerapp_preview_connected_env_certificate(self, resource_group, connected_cluster_name):
        ...
        self.cmd('containerapp connected-env certificate upload -g {} -n {} --certificate-file "{}"'.format(
        ...
        self.cmd('containerapp connected-env certificate delete -n {} -g {} --thumbprint {} --certificate {} --yes'.format(
            env_name, resource_group, cert_thumbprint, cert_name), expect_failure=False)
        self.cmd('containerapp connected-env certificate list -g {} -n {}'.format(resource_group, env_name), checks=[
            JMESPathCheck('length(@)', 0),
        ])
        self.cmd('containerapp connected-env delete -g {} -n {} --yes'.format(
