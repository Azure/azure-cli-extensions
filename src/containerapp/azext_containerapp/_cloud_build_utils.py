...
def run_cloud_build(cmd, source, location, resource_group_name, environment_name, run_full_id, logs_file, logs_file_path):
    ...
    log_in_file(
        f"Starting the Cloud Build for build of id '{generated_build_name}'\n", logs_file, no_print=True)
    ...
    try:
        ...
        def display_spinner(task_title):
            ...
            def spin():
                ...
                while done_spinner is False and fail_spinner is False and cancel_spinner is False:
                    ...
                    print(
                        f"\r    {spinner} {task_title} ({time_elapsed:.1f}s)", end="", flush=True)
                    time.sleep(0.15)
                if cancel_spinner:
                    print(
                        f"\r    {font_bold_yellow}Canceled:{font_default} {task_title} ({time_elapsed:.1f}s)")
                    log_in_file(
                        f"Canceled: {task_title} ({time_elapsed:.1f}s)", logs_file, no_print=True)
                elif fail_spinner:
                    print(
                        f"\r    {font_bold_red}(X) Fail:{font_default} {task_title} ({time_elapsed:.1f}s)")
                    log_in_file(
                        f"Fail: {task_title} ({time_elapsed:.1f}s)", logs_file, no_print=True)
                else:
                    print(
                        f"\r    {font_bold_green}(✓) Done:{font_default} {task_title} ({time_elapsed:.1f}s)")
                    log_in_file(
                        f"Done: {task_title} ({time_elapsed:.1f}s)", logs_file, no_print=True)
                # Display the cursor
        ...
        thread = display_spinner(
            "Listing the builders available in the Container Apps environment")
        builders_list_json_content = BuilderClient.list(
            cmd, resource_group_name)
        filtered_builders_list = [builder for builder in builders_list_json_content["value"]
                                  if builder["properties"]["environmentId"].endswith(f"/{environment_name}")]
        ...
        if builder_available and filtered_builders_list[0]["properties"]["provisioningState"] != "Succeeded":
            ...
            raise ValidationError(
                f"The builder selected, {non_ready_builder_name}, isn't ready to build (current state: {non_ready_provisioning_state})")
        done_spinner = True
        thread.join()
        ...
        if builder_available:
            ...
            log_in_file(
                f"{substatus_indentation}Builder selected: {builder_name}", logs_file)
        else:
            log_in_file(
                f"{substatus_indentation}No builder available in environment {font_bold}{environment_name}{font_default}", logs_file)
            # Builder creation
            done_spinner = False
            thread = display_spinner(
                "Creating the builder in the Container Apps environment")
            builder_name = f"builder{run_full_id}"[:12]
            BuilderClient.create(
                cmd, builder_name, resource_group_name, environment_name, location)
            done_spinner = True
            thread.join()
            log_in_file(
                f"{substatus_indentation}Builder created: {builder_name}", logs_file)
        ...
        done_spinner = False
        thread = display_spinner(
            "Starting the Container Apps Cloud Build agent")
        build_create_json_content = BuildClient.create(
            cmd, builder_name, generated_build_name, resource_group_name, location, True)
        ...
        thread.join()
        log_in_file(
            f"{substatus_indentation}Cloud Build agent started: {build_name}", logs_file)

        # Token retrieval
        done_spinner = False
        thread = display_spinner("Retrieving the authentication token")
        token_retrieval_json_content = BuildClient.list_auth_token(
            cmd, builder_name, build_name, resource_group_name, location)
        ...
        done_spinner = False
        thread = display_spinner(
            f"Compressing data: {font_bold}{source}{font_default}")
        tar_file_path = os.path.join(
            tempfile.gettempdir(), f"{build_name}.tar.gz")
        ...
        try:
            ...
            files = [
                ("file", ("build_data.tar.gz", tar_file, "application/x-tar"))]
        ...
        if not response_file_upload.ok:
            raise ValidationError(
                f"Error when uploading the file, request exited with {response_file_upload.status_code}")
        done_spinner = True
        thread.join()
        ...
        done_spinner = False
        thread = display_spinner(
            "Waiting for the Cloud Build agent to report status")
        build_provisioning = True
        while build_provisioning:
            build_json_content = BuildClient.get(
                cmd, builder_name, build_name, resource_group_name)
            if build_json_content["properties"]["provisioningState"] == "Succeeded":
                ...
                if build_status == "InProgress":
                    ...
                    raise ValidationError(
                        f"The build {build_name} was provisioned properly but its build status is {build_status}")
            time.sleep(1)
        done_spinner = True
        thread.join()
        ...
        while logs_stream_retries < maximum_logs_stream_retries:
            ...
            if not response_log_streaming.ok:
                raise ValidationError(
                    f"Error when streaming the logs, request exited with {response_log_streaming.status_code}")
        ...
        for line in response_log_streaming_lines:
            ...
            log_execution_phase_match = re.search(
                log_execution_phase_pattern, log_line)
        ...
        log_in_file(
            f"{substatus_indentation}{font_bold}Successfully built and containerized image: {final_image}{font_default}", logs_file)
        log_in_file(
            f"{substatus_indentation}Full logs: {logs_file_path}\n", logs_file)
        logs_file.close()
        return final_image
        ...
        thread.join()
        log_in_file(
            f"{substatus_indentation}{font_bold_red}{str(cloud_build_error)}{font_default}", logs_file)
        log_in_file(
            f"{substatus_indentation}Full logs: {logs_file_path}\n", logs_file)
        raise ValidationError(
            "The Cloud Build failed because of an error during the build process.") from cloud_build_error
    except (ValidationError, Exception) as error:
        fail_spinner = True
        thread.join()
        log_in_file(
            f"{substatus_indentation}{font_bold_red}{str(error)}{font_default}", logs_file)
        log_in_file(
            f"{substatus_indentation}Full logs: {logs_file_path}\n", logs_file)
        raise error
