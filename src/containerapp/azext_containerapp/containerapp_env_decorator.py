...
class ContainerappEnvPreviewCreateDecorator(ContainerAppEnvCreateDecorator):
    ...
    def set_up_dynamic_json_columns(self):
        if self.get_argument_logs_destination() == "log-analytics" and self.get_argument_logs_dynamic_json_columns() is not None:
            self.managed_env_def["properties"]["appLogsConfiguration"]["logAnalyticsConfiguration"][
                "dynamicJsonColumns"] = self.get_argument_logs_dynamic_json_columns()
    ...
    def set_up_workload_profiles(self):
        if self.get_argument_enable_workload_profiles():
            ...
            try:
                existing_environment = self.client.show(
            ...
            workload_profiles = get_default_workload_profiles(
                self.cmd, self.get_argument_location())
...
class ContainerappEnvPreviewUpdateDecorator(ContainerAppEnvUpdateDecorator):
    def set_up_app_log_configuration(self):
        ...
        if logs_destination:
            ...
            safe_set(self.managed_env_def, "properties",
                     "appLogsConfiguration", "destination", value=logs_destination)

        if logs_destination == "azure-monitor":
            safe_set(self.managed_env_def, "properties",
                     "appLogsConfiguration", "logAnalyticsConfiguration", value=None)
        ...
        if self.get_argument_logs_dynamic_json_columns() is not None:
            safe_set(self.managed_env_def, "properties", "appLogsConfiguration", "logAnalyticsConfiguration",
                     "dynamicJsonColumns", value=self.get_argument_logs_dynamic_json_columns())
