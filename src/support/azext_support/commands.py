...
from azext_support._client_factory import (
    cf_communications,
    cf_support,
    cf_support_tickets,
)
...
def load_command_table(self, _):
    ...
    with self.command_group(
        "support tickets", support_tickets, client_factory=cf_support_tickets
    ) as g:
        ...
        g.custom_command(
            ...
            validator=validate_tickets_create,
        )
    ...
    with self.command_group(
        ...
        g.custom_command(
            "create", "create_support_tickets_communications", supports_no_wait=False
        )
    ...
    self.command_table["support in-subscription communication create"] = (
        CommunicationCreate(loader=self)
    )
    self.command_table["support no-subscription communication create"] = (
        CommunicationNoSubscriptionCreate(loader=self)
    )
    self.command_table["support no-subscription tickets update"] = (
        TicketUpdateNoSubscription(loader=self)
    )
    self.command_table["support no-subscription tickets create"] = (
        TicketCreateNoSubscription(loader=self)
    )
    self.command_table["support in-subscription tickets list"] = TicketList(loader=self)
    self.command_table["support no-subscription tickets list"] = (
        TicketListNoSubscription(loader=self)
    )
    ...
    self.command_table["support in-subscription file-workspace create"] = (
        FileWorkspaceCreateSubscription(loader=self)
    )
    self.command_table["support no-subscription file-workspace create"] = (
