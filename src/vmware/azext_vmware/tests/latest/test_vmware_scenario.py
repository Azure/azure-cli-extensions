...
class VmwareScenarioTest(ScenarioTest):
    ...
    #     # add identity source
    #     self.cmd(
    #         'vmware private-cloud identity-source create -g {rg} -c {privatecloud} -n groupName --alias groupAlias --domain domain --base-user-dn "ou=baseUser" --base-group-dn "ou=baseGroup" --primary-server ldaps://1.1.1.1:636 --username someone --password something')
    #
    #     # delete identity source
    #     self.cmd(
    #         'vmware private-cloud identity-source delete -g {rg} -c {privatecloud} -n groupName --alias groupAlias --domain domain --yes')
    #
    # @ResourceGroupPreparer(name_prefix='cli_test_vmwarecluster', location='brazilsouth')
    # def test_vmware_cluster(self, resource_group):
    #     # cluster list should report 0
    #     self.cmd('vmware cluster list -g {rg} -c {privatecloud}', checks=[
    #         self.check('length(@)', 0)
    #     ])
    #
    #     # cluster create
    #     self.cmd('vmware cluster create -g {rg} -c {privatecloud} -n {cluster} --sku {sku} --size 3 --hosts {hosts}')
    #
    #     # cluster create without --hosts
    #     self.cmd('vmware cluster create -g {rg} -c {privatecloud} -n {cluster} --sku {sku} --size 3')
    #
