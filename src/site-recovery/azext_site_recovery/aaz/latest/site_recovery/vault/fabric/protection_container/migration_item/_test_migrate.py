...
@register_command(
    "site-recovery vault fabric protection-container migration-item test-migrate",
)
class TestMigrate(AAZCommand):
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        v_mware_cbt.network_id = AAZStrArg(
            options=["network-id"],
            help="The test network Id.",
            required=True,
        )
        v_mware_cbt.recovery_point_id = AAZStrArg(
            options=["recovery-point-id"],
            help="The recovery point Id.",
            required=True,
        )
        v_mware_cbt.vm_nics = AAZListArg(
            options=["vm-nics"],
            help="The list of NIC details.",
        )

        vm_nics = cls._args_schema.provider_specific_details.v_mware_cbt.vm_nics
        vm_nics.Element = AAZObjectArg()

        _element = cls._args_schema.provider_specific_details.v_mware_cbt.vm_nics.Element
        _element.is_primary_nic = AAZStrArg(
            options=["is-primary-nic"],
            help="A value indicating whether this is the primary NIC.",
            required=True,
        )
        _element.is_selected_for_migration = AAZStrArg(
            options=["is-selected-for-migration"],
            help="A value indicating whether this NIC is selected for migration.",
        )
        ...
        _element.target_nic_name = AAZStrArg(
            options=["target-nic-name"],
            help="Target NIC name.",
        )
        _element.target_static_ip_address = AAZStrArg(
            options=["target-static-ip-address"],
            help="The static IP address.",
        )
        _element.target_subnet_name = AAZStrArg(
            options=["target-subnet-name"],
            help="Target subnet name.",
        )
        _element.test_static_ip_address = AAZStrArg(
            options=["test-static-ip-address"],
            help="The test static IP address.",
        )
        _element.test_subnet_name = AAZStrArg(
            options=["test-subnet-name"],
            help="The test subnet name.",
        )
        return cls._args_schema

    def _execute_operations(self):
        yield self.ReplicationMigrationItemsTestMigrate(ctx=self.ctx)()

    def _output(self, *args, **kwargs):
        result = self.deserialize_output(self.ctx.vars.instance, client_flatten=True)
        return result

    class ReplicationMigrationItemsTestMigrate(AAZHttpOperation):
        ...
        def content(self):
            ...
            if disc_v_mware_cbt is not None:
                disc_v_mware_cbt.set_prop("networkId", AAZStrType, ".v_mware_cbt.network_id", typ_kwargs={"flags": {"required": True}})
                disc_v_mware_cbt.set_prop("recoveryPointId", AAZStrType, ".v_mware_cbt.recovery_point_id", typ_kwargs={"flags": {"required": True}})
                disc_v_mware_cbt.set_prop("vmNics", AAZListType, ".v_mware_cbt.vm_nics")

            vm_nics = _builder.get(".properties.providerSpecificDetails{instanceType:VMwareCbt}.vmNics")
            if vm_nics is not None:
                vm_nics.set_elements(AAZObjectType, ".")

            _elements = _builder.get(".properties.providerSpecificDetails{instanceType:VMwareCbt}.vmNics[]")
            if _elements is not None:
                _elements.set_prop("isPrimaryNic", AAZStrType, ".is_primary_nic", typ_kwargs={"flags": {"required": True}})
                _elements.set_prop("isSelectedForMigration", AAZStrType, ".is_selected_for_migration")
                _elements.set_prop("nicId", AAZStrType, ".nic_id", typ_kwargs={"flags": {"required": True}})
                _elements.set_prop("targetNicName", AAZStrType, ".target_nic_name")
                _elements.set_prop("targetStaticIPAddress", AAZStrType, ".target_static_ip_address")
                _elements.set_prop("targetSubnetName", AAZStrType, ".target_subnet_name")
                _elements.set_prop("testStaticIPAddress", AAZStrType, ".test_static_ip_address")
                _elements.set_prop("testSubnetName", AAZStrType, ".test_subnet_name")
