    ...
    def _replace_string_keys(self, val):
        ...
        if 'secondaryKey' in val:
            val = re.sub(r'"secondaryKey":( ?)"([^"]+)"', r'"secondaryKey":"{}"'
                         .format(MOCK_KEY), val, flags=re.IGNORECASE)
        if 'primaryConnectionString' in val:
            val = re.sub(r'"primaryConnectionString":( ?)"([^"]+)"', r'"primaryConnectionString":"{}"'
                         .format(MOCK_KEY), val, flags=re.IGNORECASE)
        if 'secondaryConnectionString' in val:
            val = re.sub(r'"secondaryConnectionString":( ?)"([^"]+)"', r'"secondaryConnectionString":"{}"'
                         .format(MOCK_KEY), val, flags=re.IGNORECASE)
        return val
    ...
    def _replace_byte_keys(self, val):
        ...
        if b'secondaryKey' in val:
            val = re.sub(b'"secondaryKey":( ?)"([^"]+)"', '"secondaryKey":"{}"'
                         .format(MOCK_KEY).encode(), val, flags=re.IGNORECASE)
        if b'primaryConnectionString' in val:
            val = re.sub(b'"primaryConnectionString":( ?)"([^"]+)"', '"primaryConnectionString":"{}"'
                         .format(MOCK_KEY).encode(), val, flags=re.IGNORECASE)
        if b'secondaryConnectionString' in val:
            val = re.sub(b'"secondaryConnectionString":( ?)"([^"]+)"', '"secondaryConnectionString":"{}"'
                         .format(MOCK_KEY).encode(), val, flags=re.IGNORECASE)
        return val
