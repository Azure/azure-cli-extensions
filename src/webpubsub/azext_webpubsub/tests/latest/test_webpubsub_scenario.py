...
class WebpubsubScenarioTest(ScenarioTest):
    ...
    def test_webpubsub(self, resource_group):
        ...
        # Test update with client cert enabled
        self.cmd('webpubsub update -g {rg} -n {name} --client-cert-enabled true', checks=[
            self.check('name', '{name}'),
            self.check('location', '{location}'),
            self.check('provisioningState', 'Succeeded'),
            self.check('tls.clientCertEnabled', 'true'),
        ])

        # Test update with client cert disabled
        # self.cmd('webpubsub update -g {rg} -n {name} --client-cert-enabled false', checks=[
        #     self.check('name', '{name}'),
        #     self.check('location', '{location}'),
        #     self.check('provisioningState', 'Succeeded'),
        #     self.check('tls.clientCertEnabled', 'false'),
        # ])
