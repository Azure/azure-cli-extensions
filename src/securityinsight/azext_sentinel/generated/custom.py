...
def sentinel_alert_rule_create(client,
    ...
    if len(all_alert_rule) > 1:
        raise CLIError('at most one of  fusion_alert_rule, microsoft_security_incident_creation_alert_rule, scheduled_a'
                       'lert_rule is needed for alert_rule!')
...
def sentinel_bookmark_create(client,
    ...
    return client.create_or_update(resource_group_name=resource_group_name,
                                   ...
                                   user_info_object_id=created_by_object_id)
...
def sentinel_data_connector_create(client,
    ...
    if m_c_a_s_data_connector is not None:
        all_data_connector.append(m_c_a_s_data_connector)
    ...
    if t_i_data_connector is not None:
        all_data_connector.append(t_i_data_connector)
    if len(all_data_connector) > 1:
        raise CLIError('at most one of  aad_data_connector, aatp_data_connector, asc_data_connector, aws_cloud_trail_da'
                       'ta_connector, m_c_a_s_data_connector, mdatp_data_connector, office_data_connector, t_i_data_con'
                       'nector is needed for data_connector!')
    if len(all_data_connector) != 1:
        raise CLIError('data_connector is required. but none of aad_data_connector, aatp_data_connector, asc_data_conne'
                       'ctor, aws_cloud_trail_data_connector, m_c_a_s_data_connector, mdatp_data_connector, office_data'
                       '_connector, t_i_data_connector is provided!')
