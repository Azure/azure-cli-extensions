...
class ResourceMoverMoveCollectionScenarioTest(ScenarioTest):
    ...
    def test_resourcemover2_movecollection_basic(self):
        self.kwargs.update({
        ...
        self.cmd('az resource-mover-2 move-collection create --location {location} --source-region {source_region} '
                 '--target-region {target_region} --name {collection_name} --resource-group {rg}', checks=basic_checks)

        # check updating a move-collection
        self.cmd('az resource-mover-2 move-collection update --name {collection_name} '
        ...
        self.cmd('az resource-mover-2 move-collection show --resource-group {rg} --name {collection_name}',
                 checks=basic_checks)
        ...
        # self.cmd('az resource-mover-2 move-collection list',checks=[
        #     self.check('length(@)', 1)
        # ])

        # check deleting a move-collection
        self.cmd('az resource-mover-2 move-collection delete --resource-group {rg} --name {collection_name} --yes')

        # check listing a move-collection by resource group
        self.cmd('az resource-mover-2 move-collection list --resource-group {rg}', checks=[
                 self.check('length(@)', 0)])

    @ResourceGroupPreparer(name_prefix="clitest_resourcemover_collection_", location="eastus2")
    def test_resourcemover2_movecollection_e2e(self):

        self.kwargs.update({
            'collection_name': 'clitest-movecollection-39',
            'location': 'eastus2',
            'source_region': 'eastus',
            'target_region': 'westus',
        ...
        move_collection = self.cmd('az resource-mover-2 move-collection create --location {location} --source-region {source_region} '
        ...
        self.cmd('az resource-mover-2 move-resource add --resource-group {rg} --move-collection-name {collection_name} '
                 '--name {move_resource_vnet} --source-id {source_vnet_id} --resource-settings {vnet_resource_settings}')

        # list-required-for
        self.cmd('az resource-mover-2 move-collection list-required-for --resource-group {rg} --move-collection-name '
                 '{collection_name} --source-id {source_vnet_id}', checks=[
                     self.check('length(sourceIds)', 0)])

        # list-unresolved-dependency
        self.cmd('az resource-mover-2 move-collection list-unresolved-dependency --resource-group {rg} '
                 '--move-collection-name {collection_name}', checks=[
                     self.check('length(@)', 0)])

        # resolve-dependency
        self.cmd('az resource-mover-2 move-collection resolve-dependency --resource-group {rg} '
                 '--move-collection-name {collection_name}', checks=[
                     self.check('resourceGroup', '{rg}'),
                     self.check('status', 'Succeeded')])

        # list all the move-resources in move-collection
        move_resources = self.cmd('az resource-mover-2 move-resource list --resource-group {rg} '
                                  '--move-collection-name {collection_name}', checks=[
        ...
        def check_move_state(target_state):
            ...
            self.cmd('az resource-mover-2 move-resource show --resource-group {rg} '
        ...
        # prepare
        self.cmd('az resource-mover-2 move-collection prepare --move-resources {move_resource_vnet_id} '
        ...
        # initiate-move
        self.cmd('az resource-mover-2 move-collection initiate-move --move-resources {move_resource_vnet_id} '
                 '--name {collection_name} --resource-group {rg}', checks=[
                     self.check('resourceGroup', '{rg}'),
                     self.check('status', 'Succeeded')])
        ...
        # commit
        self.cmd('az resource-mover-2 move-collection commit --move-resources {move_resource_vnet_id} ' 
                 '--name {collection_name} --resource-group {rg}', checks=[
                     self.check('resourceGroup', '{rg}'),
                     self.check('status', 'Succeeded')])
        ...
        self.cmd('az resource-mover-2 move-collection bulk-remove --move-resources {move_resource_vnet_id} '
                 '--name {collection_name} --resource-group {rg}', checks=[
                     self.check('resourceGroup', '{rg}'),
                     self.check('status', 'Succeeded')])
        self.cmd('az resource-mover-2 move-collection delete --name {collection_name} --resource-group '
                 '{rg} --yes')
...
class ResourceMoverMoveResourceScenarioTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(name_prefix="clitest_resourcemover_collection_", location="eastus2")
    def test_resourcemover2_moveresource_basic(self):
        self.kwargs.update({
            'collection_name': 'clitest-movecollection-99',
            'location': 'eastus2',
            'source_region': 'eastus',
            'target_region': 'westus',
            'source_vnet': 'vnet-in-source-region',
        ...
        # add the vNet as a move-resource to the move-collection
        self.cmd('az resource-mover-2 move-resource add --resource-group {rg} --move-collection-name '
                 '{collection_name} --name {move_resource_vnet} --source-id {source_vnet_id} --resource-settings {resource_settings}')

        # list all the move-resources in move-collection
        self.cmd('az resource-mover-2 move-resource list --resource-group {rg} '
                 '--move-collection-name {collection_name}', checks=[
                     self.check('length(@)', 1)])

        # test showing the move-resource in move-collection
        self.cmd('az resource-mover-2 move-resource show --resource-group {rg} --move-collection-name {collection_name} '
        ...
        self.cmd('az resource-mover-2 move-resource delete --resource-group {rg} --move-collection-name {collection_name} '
                 '--name {move_resource_vnet} --yes')

        # delete the move-collection
        self.cmd('az resource-mover-2 move-collection delete --name {collection_name} --resource-group {rg} --yes')
