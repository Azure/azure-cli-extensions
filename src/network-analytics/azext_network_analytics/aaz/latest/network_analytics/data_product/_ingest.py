...
class Ingest(AAZCommand):
    ...
    class DataProductsIngest(object):
        ...
        def __init__(self, ctx):
            ...
            self.resources_client = get_mgmt_service_client(self.ctx.cli_ctx, ResourceType.MGMT_RESOURCE_RESOURCES, subscription_id=self.subscription_id).resources
            self.container_client = get_sdk(self.ctx.cli_ctx, ResourceType.DATA_STORAGE_BLOB, '_container_client#ContainerClient')
        ...
        def get_keyvault_url(self, data_product):
            consumption_endpoints = data_product.properties['consumptionEndpoints']
            ingestion_url = consumption_endpoints['ingestionUrl']
