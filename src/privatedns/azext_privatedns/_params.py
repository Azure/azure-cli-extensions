...
def load_arguments(self, _):
    ...
    with self.argument_context('network privatedns record-set') as c:
    ...
    for item in ['a', 'aaaa', 'cname', 'mx', 'ptr', 'srv', 'txt']:
        with self.argument_context('network privatedns record-set {} add-record'.format(item)) as c:
        ...
        with self.argument_context('network privatedns record-set {} remove-record'.format(item)) as c:
    ...
    with self.argument_context('network privatedns record-set cname set-record') as c:
        c.argument('relative_record_set_name', options_list=['--record-set-name', '-n'], help='The name of the record set relative to the zone. Creates a new record set if one does not exist.')

    with self.argument_context('network privatedns record-set soa') as c:
        c.argument('relative_record_set_name', ignore_type, default='@')

    with self.argument_context('network privatedns record-set a') as c:
        c.argument('ipv4_address', options_list=('--ipv4-address', '-a'), help='IPV4 address in string notation.')

    with self.argument_context('network privatedns record-set aaaa') as c:
        c.argument('ipv6_address', options_list=('--ipv6-address', '-a'), help='IPV6 address in string notation.')

    with self.argument_context('network privatedns record-set cname') as c:
        c.argument('cname', options_list=('--cname', '-c'), help='Canonical name.')

    with self.argument_context('network privatedns record-set mx') as c:
    ...
    with self.argument_context('network privatedns record-set ptr') as c:
    ...
    with self.argument_context('network privatedns record-set srv') as c:
    ...
    with self.argument_context('network privatedns record-set txt') as c:
