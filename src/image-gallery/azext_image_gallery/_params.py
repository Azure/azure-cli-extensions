...
def load_arguments(self, _):
    ...
    pagination_limit_type = CLIArgumentType(
        options_list=['--max-items'], type=int, arg_group="Pagination",
        help="Total number of items to return in the command's output. If the total number of items available is more "
             "than the value specified, a token is provided in the command's output. To resume pagination, provide the "
             "token value in `--next-token` argument of a subsequent command."
    )
    ...
    with self.argument_context('sig image-definition list-community') as c:
        c.argument('location', arg_type=get_location_type(self.cli_ctx), id_part='name')
        c.argument('public_gallery_name', public_gallery_name_type)
        c.argument('pagination_limit', pagination_limit_type)
        c.argument('pagination_token', pagination_token_type)
