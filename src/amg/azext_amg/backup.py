...
def _save_dashboards(grafana_url, backup_dir, timestamp, http_headers, **kwargs):
    ...
    while True:
        ...
        if folders_to_exclude:
            ...
            dashboards = [d for d in dashboards if (d.get('folderTitle', '') and d.get('folderTitle', '').lower() not in folders_to_exclude or
                                                    (not d.get('folderTitle', '')) and 'general' not in folders_to_exclude)]
