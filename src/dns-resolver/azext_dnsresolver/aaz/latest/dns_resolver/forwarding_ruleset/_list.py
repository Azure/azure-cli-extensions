...
class List(AAZCommand):
    ...
    :example: List DNS forwarding rulesets by subscription
        az dns-resolver forwarding-ruleset list

    :example: List DNS forwarding rulesets by resource group
        az dns-resolver forwarding-ruleset list --resource-group "sampleResourceGroup"

    :example: List DNS forwarding rulesets by virtual network
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        _args_schema.top = AAZIntArg(
            options=["--top"],
            help="The maximum number of results to return. If not specified, returns up to 100 results.",
        )
        return cls._args_schema
    ...
    def _execute_operations(self):
        ...
        if condition_0:
            self.DnsForwardingRulesetsListByResourceGroup(ctx=self.ctx)()
        if condition_1:
            self.DnsForwardingRulesetsList(ctx=self.ctx)()
    ...
    class DnsForwardingRulesetsListByResourceGroup(AAZHttpOperation):
        ...
        def _build_schema_on_200(cls):
            ...
            dns_resolver_outbound_endpoints = cls._schema_on_200.value.Element.properties.dns_resolver_outbound_endpoints
            dns_resolver_outbound_endpoints.Element = AAZObjectType()

            _element = cls._schema_on_200.value.Element.properties.dns_resolver_outbound_endpoints.Element
            _element.id = AAZStrType(
                flags={"required": True},
            )
    ...
    class DnsForwardingRulesetsList(AAZHttpOperation):
        CLIENT_TYPE = "MgmtClient"
