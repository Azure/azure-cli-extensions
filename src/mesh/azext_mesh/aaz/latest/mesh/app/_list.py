...
class List(AAZCommand):
    ...
    def _execute_operations(self):
        ...
        if condition_0:
            self.ApplicationListBySubscription(ctx=self.ctx)()
        if condition_1:
            self.ApplicationListByResourceGroup(ctx=self.ctx)()
    ...
    class ApplicationListBySubscription(AAZHttpOperation):
        ...
        def _build_schema_on_200(cls):
            ...
            disc_azure_internal_monitoring_pipeline = cls._schema_on_200.value.Element.properties.diagnostics.sinks.Element.discriminate_by("kind", "AzureInternalMonitoringPipeline")
            ...
            properties.description = AAZStrType()
            properties.diagnostics = AAZObjectType()
            _ListHelper._build_schema_diagnostics_ref_read(properties.diagnostics)
            properties.health_state = AAZStrType(
                serialized_name="healthState",
            )
            ...
            disc_add_remove_replica = cls._schema_on_200.value.Element.properties.services.Element.properties.auto_scaling_policies.Element.mechanism.discriminate_by("kind", "AddRemoveReplica")
            ...
            disc_average_load = cls._schema_on_200.value.Element.properties.services.Element.properties.auto_scaling_policies.Element.trigger.discriminate_by("kind", "AverageLoad")
            ...
            metric = cls._schema_on_200.value.Element.properties.services.Element.properties.auto_scaling_policies.Element.trigger.discriminate_by("kind", "AverageLoad").metric
            metric.kind = AAZStrType(
                flags={"required": True},
            )

            disc_resource = cls._schema_on_200.value.Element.properties.services.Element.properties.auto_scaling_policies.Element.trigger.discriminate_by("kind", "AverageLoad").metric.discriminate_by("kind", "Resource")
            ...
            _ListHelper._build_schema_diagnostics_ref_read(_element.diagnostics)
            ...
            _ListHelper._build_schema_container_state_read(instance_view.current_state)
            instance_view.events = AAZListType()
            instance_view.previous_state = AAZObjectType(
                serialized_name="previousState",
            )
            _ListHelper._build_schema_container_state_read(instance_view.previous_state)
            ...
            disc_service_fabric_volume_disk = cls._schema_on_200.value.Element.properties.services.Element.properties.code_packages.Element.volumes.Element.creation_parameters.discriminate_by("kind", "ServiceFabricVolumeDisk")
