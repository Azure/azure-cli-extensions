...
class List(AAZCommand):
    ...
    def _execute_operations(self):
        self.pre_operations()
        condition_0 = has_value(self.ctx.subscription_id) and has_value(self.ctx.args.category) and has_value(self.ctx.args.resource_group) is not True
        condition_1 = has_value(self.ctx.args.resource_group) and has_value(self.ctx.subscription_id) and has_value(self.ctx.args.category)
        if condition_0:
            self.WorkbooksListBySubscription(ctx=self.ctx)()
        if condition_1:
            self.WorkbooksListByResourceGroup(ctx=self.ctx)()
    ...
    class WorkbooksListBySubscription(AAZHttpOperation):
    ...
    class WorkbooksListByResourceGroup(AAZHttpOperation):
        CLIENT_TYPE = "MgmtClient"
