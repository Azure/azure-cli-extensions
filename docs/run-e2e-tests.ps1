...
function Deactivate-AllPmRoles {
    ...
    $rawAssign = az role assignment list --assignee $userOid --scope $EdgeMachine --include-inherited --query "[?contains(roleDefinitionName, 'Provisioned Machine')]" -o json 2>$null
    $allAssignments = @()
    if ($rawAssign) { try { $allAssignments = $rawAssign | ConvertFrom-Json -EA SilentlyContinue } catch { $allAssignments = @() } }
...
# ═════════════════════════════════════════════════════════════
# 7. ROLE-BASED ACCESS: SUDO VERIFICATION (requires Docker + PIM)
#    Grouped by role — user activates each role manually before its tests.
#    Order: Admin → Contributor → Reader
# ═════════════════════════════════════════════════════════════
Write-Host "`n─── Phase 7: Role-Based Sudo Access ───" -ForegroundColor Magenta

if ($dockerOk -and $pimOk) {
    ...
    # ─── Admin Role Group (runs immediately — Admin already active from PIM section) ───
    Write-Host "`n  [Admin] Running 6.5 — Admin role already active from PIM activation" -ForegroundColor Cyan

    Run-Test "6.5" "Admin role has sudo access" "SSH with Admin cert, verify sudo whoami = root" `
        "ssh with Admin cert -> sudo whoami" "sudo works - returns root" {
        # Generate cert (Admin already active from PIM activation section)
        $o = az provisionedmachine ssh-cert-create --vault-name $VaultName --resource-id $EdgeMachine 2>&1 | Out-String
        if ($o -notmatch "certificatePath") { throw "Cert gen failed (is Admin role active?): $o" }
        $m = [regex]::Match($o, '\{[^}]+\}')
        $j = $m.Value | ConvertFrom-Json
        $adminKey = $j.privateKeyPath; $adminCert = $j.certificatePath

        # SSH and test sudo
        $ju = "${userAlias}_jit"
        $sshOut = ssh -i $adminKey -o "CertificateFile=$adminCert" -o UserKnownHostsFile=NUL -o StrictHostKeyChecking=no -o BatchMode=yes -p $SshPort "${ju}@localhost" 'sudo whoami 2>&1; echo EXIT_CODE=$?' 2>&1 | Out-String
        Remove-Item -Recurse -Force (Split-Path $adminKey) -EA SilentlyContinue

        if ($sshOut -match "root" -and $sshOut -match "EXIT_CODE=0") {
            "sudo works - returns root"
        } else {
            throw "Admin should have sudo. Got: $sshOut"
        }
    }

    # ─── Contributor Role Group ───────────────────────────────
    Write-Host "`n╔══════════════════════════════════════════════════╗" -ForegroundColor Yellow
    Write-Host "║  ROLE GROUP: Provisioned Machine Contributor      ║" -ForegroundColor Yellow
    Write-Host "║  Deactivate Admin, activate Contributor, then     ║" -ForegroundColor Yellow
    Write-Host "║  press ENTER (or 'skip' to skip)                  ║" -ForegroundColor Yellow
    Write-Host "╚══════════════════════════════════════════════════╝" -ForegroundColor Yellow
    $contribReady = Read-Host "Contributor role active? [ENTER/skip]"

    if ($contribReady -ne "skip") {
        Run-Test "6.6" "Contributor role has NO sudo access" "SSH with Contributor cert, verify sudo denied" `
            "ssh with Contributor cert -> sudo whoami" "sudo denied" {
            # Generate cert (Contributor already active)
            $o = az provisionedmachine ssh-cert-create --vault-name $VaultName --resource-id $EdgeMachine 2>&1 | Out-String
            if ($o -notmatch "certificatePath") { throw "Cert gen failed (is Contributor role active?): $o" }
            $m = [regex]::Match($o, '\{[^}]+\}')
            $j = $m.Value | ConvertFrom-Json
            $contribKey = $j.privateKeyPath; $contribCert = $j.certificatePath

            # SSH and test sudo
            $ju = "${userAlias}_jit"
            $sshOut = ssh -i $contribKey -o "CertificateFile=$contribCert" -o UserKnownHostsFile=NUL -o StrictHostKeyChecking=no -o BatchMode=yes -p $SshPort "${ju}@localhost" 'sudo whoami 2>&1; echo EXIT_CODE=$?' 2>&1 | Out-String
            Remove-Item -Recurse -Force (Split-Path $contribKey) -EA SilentlyContinue

            if ($sshOut -match "root" -and $sshOut -match "EXIT_CODE=0") {
                throw "BUG: Contributor should NOT have sudo access! Got: $sshOut"
            }
            if ($sshOut -match "not allowed|denied|not permitted|password is required|not in the sudoers") {
                "sudo denied - Contributor has no sudo"
            } else {
                throw "Unexpected sudo output for Contributor: $sshOut"
            }
        }
    } else {
        Skip-Test "6.6" "Contributor sudo access" "User skipped Contributor role group"
    }

    # ─── Reader Role Group ────────────────────────────────────
    Write-Host "`n╔══════════════════════════════════════════════════╗" -ForegroundColor Magenta
    Write-Host "║  ROLE GROUP: Provisioned Machine Reader           ║" -ForegroundColor Magenta
    Write-Host "║  Deactivate Contributor, activate Reader, then    ║" -ForegroundColor Magenta
    Write-Host "║  press ENTER (or 'skip' to skip)                  ║" -ForegroundColor Magenta
    Write-Host "╚══════════════════════════════════════════════════╝" -ForegroundColor Magenta
    $readerReady = Read-Host "Reader role active? [ENTER/skip]"

    if ($readerReady -ne "skip") {
        Run-Test "6.7" "Reader role has NO sudo access" "SSH with Reader cert, verify sudo denied" `
            "ssh with Reader cert -> sudo whoami" "sudo denied" {
            # Generate cert (Reader already active)
            $o = az provisionedmachine ssh-cert-create --vault-name $VaultName --resource-id $EdgeMachine 2>&1 | Out-String
            if ($o -notmatch "certificatePath") { throw "Cert gen failed (is Reader role active?): $o" }
            $m = [regex]::Match($o, '\{[^}]+\}')
            $j = $m.Value | ConvertFrom-Json
            $readerKey = $j.privateKeyPath; $readerCert = $j.certificatePath

            # SSH and test sudo
            $ju = "${userAlias}_jit"
            $sshOut = ssh -i $readerKey -o "CertificateFile=$readerCert" -o UserKnownHostsFile=NUL -o StrictHostKeyChecking=no -o BatchMode=yes -p $SshPort "${ju}@localhost" 'sudo whoami 2>&1; echo EXIT_CODE=$?' 2>&1 | Out-String
            Remove-Item -Recurse -Force (Split-Path $readerKey) -EA SilentlyContinue

            if ($sshOut -match "root" -and $sshOut -match "EXIT_CODE=0") {
                throw "BUG: Reader should NOT have sudo access! Got: $sshOut"
            }
            if ($sshOut -match "not allowed|denied|not permitted|password is required|not in the sudoers") {
                "sudo denied - Reader has no sudo"
            } else {
                throw "Unexpected sudo output for Reader: $sshOut"
            }
        }
    } else {
        Skip-Test "6.7" "Reader sudo access" "User skipped Reader role group"
    }
} elseif (-not $dockerOk) {
    "6.5","6.6","6.7" | % { Skip-Test $_ "Sudo access test" "Docker not available" }
} else {
    "6.5","6.6","6.7" | % { Skip-Test $_ "Sudo access test" "PIM not activated" }
}
